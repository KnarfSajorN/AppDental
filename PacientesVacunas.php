                ...
                <tbody>
                 ...
                 {    
                  ...
                  while ($fila = mysqli_fetch_array($resultado)) {
                    echo '<tr>';                
                    echo'<td><a href="vacunacionAplicar?cI='.encrypt($fila['cliente_id']).'" title="Aplicar Vacuna">'.$fila['nombre_cliente'].' </a></td>';
                    echo '<td>'.$fila['CODI_CLIENTE'].'</td>    
                    <td>'.funcionMaster($fila[20],'ID','nombre','usuariosInterconsulta').'</td>
                    <td>'.$fila['whatsapp'].'</td>
                    <td>';
                    echo '<a href="vacunacionAplicar?cI='.encrypt($fila['cliente_id']).'" title="Aplicar Vacuna"><i class="fa fa-eyedropper"></i> </a>';  
                    echo '</td>
                    </tr>';
