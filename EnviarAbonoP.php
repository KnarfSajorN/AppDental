...
$queryList=mysqli_query($conn3,"SELECT * FROM  sOperacionInv where idOperacion = $idOperacion");
            ...
            $queryList=mysqli_query($conn3,"SELECT * FROM  abonoP where  numero_operacion = '$idOperacion' AND id='$idAbono' ");
            $nrowl=mysqli_num_rows($queryList);
            while($rowMotorizado=mysqli_fetch_array($queryList))
            {
              $id=$rowMotorizado['id'];
...
$mensajeW = ' Sr(a) *'.$nombre_cliente .'* Se le ha registrado un abono por el valor de : *'.$valor_abonado.'* *'.$Moneda.'* 
por favor firmar en el siguiente link para confirmar el monto cancelado,
Link:  '.$Base.'firma/FirmarAbonoP/'.$idOperacion.'/'.$id.' ';
$accion = 0;
