...
<div class="content-wrapper">
  ...
  <section class="content">
    <div class="">
      <div class="col-md-12">
        <h4 class="Titulo_Pagina">Pacientes Pediatría</h4>
        <?php
        $msg = $_GET['msg'];
        if ($msg == '1') {
        ...
        <div class="box">
          <div class="box-header">
            <a href="CrearPaciente.php">
              <button class="btn btn-block btn-outline-info rounded-pill mb-2">
...
<script>
  ...
  var titulo_tabla = "Pacientes";
     ...
     <?php } else if (($_SESSION['vista'] == 0 || $_SESSION['vista'] == 1)) {
       ...
       query_tabla_ajax = "<?php echo "SELECT * FROM WHERE 1=1 cliente {$filtro} $queryCliente ORDER BY cliente_id"; ?>";
     <?php } ?>
  ...
  columnastablas = [{
    ...
    },
    {
      "data": function(row, type, set) {
        botones = "";
        botones += "<a href='hcp?cI=<?=salt()?>" + btoa(row.cliente_id) + "' title='Agregar Historia'><i class='fas fa-file-medical'></i> </a>";
        ...
        botones += "<a href='hcpVerPaciente?cI=<?=salt()?>" + btoa(row.cliente_id) + "' title='Ver historial'><i class='fas fa-book-medical'></i> </a>";
        ...
        botones += "<a href='HC_Examenes?cI=<?=salt()?>" + btoa(row.cliente_id) + "'title='Exámenes'><i class='fa-solid fa-briefcase-medical'></i> </a>";
        return botones;
      }
    }
  ];
