...
<aside class="main-sidebar sidebar-light-primary elevation-4">
  ...
  <a href="portada" class="brand-link bg-light" id="logoNav">
    <img src="img/logoSolo.png" alt="Logo" class="brand-image img-circle" style="opacity: .8">
    <span class="brand-text font-weight-light">Medicalsoft +</span>
  </a>
    ...
    <nav class="mt-2">
      <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
        ...
        <li style="padding: 10px;text-align: center;">
          ...
          if ($_SESSION['POS'] != "") {
            $Nombre_POS_Menu = "<br>[" . funcionMaster($_SESSION['POS'], 'id', 'Nombre', 'PuntoPOS') . "]";
          }
          ?>
          <span id="tituloMenu" class="brand-text font-weight-light"><strong>
              <?= $tituloMenu . $Nombre_POS_Menu ?>
            </strong></span>
        </li>
        <?php
        // query
          ...
          foreach ($Arreglo_Grupos as $key => $value) {
            ...
            foreach ($ArregloMenuFinal as $key => $value) {
              ...
              if ($numeroCampos == 1) {
                ...
                if ($Ruta != "#") { ?>
                  <li class="nav-item">
                    <a class="nav-link " <?= (substr($Ruta, 0, 4) == "http" || substr($Ruta, 0, 3) == "www" ? 'target="_blank"' : '') ?>
                      href='<?= (substr($Ruta, 0, 4) == "http" || substr($Ruta, 0, 3) == "www" ? '' : $Base) ?><?= $Ruta ?>'
                      style='color:black; background: <?= $Color ?>47;'
                      onMouseOver="this.style.color='white';this.style.background='<?= $Color ?>'"
                      onMouseOut="this.style.color='black' ;this.style.background='<?= $Color ?>47'">
                ...
                <?php } else { ?>
                  <li class="nav-item">
                    <a class="nav-link " href='#' style='color:black; background: <?= $Color ?>47;'
                      onMouseOver="this.style.color='white';this.style.background='<?= $Color ?>'"
                      onMouseOut="this.style.color='black' ;this.style.background='<?= $Color ?>47'">
                      <i style='height:1rem; width:auto; margin-right:8px;' class='nav-icon <?= $Icono ?> fa-2x'></i>
                      <p>
                ...
                <li class="nav-item">
                  ...
                  <ul class='nav nav-treeview'>
                    ...
                    foreach ($value as $subkey => $subvalue) {
                      ...
                      <li class="nav-item">
                        <a class="nav-link " href='<?= $Base ?><?= $SubRuta ?>'
                          style='color:black; background: <?= $SubColor ?>47;  padding: 12px 5px 12px 15px;'
                          onMouseOver="this.style.background='<?= $SubColor ?>'"
                          onMouseOut="this.style.color='black' ;this.style.background='<?= $SubColor ?>47'">
