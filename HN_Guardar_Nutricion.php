<?php
 ...
 //  include 'menu.php'; 
    ...
    date_default_timezone_set('America/Bogota');
        ...
        $enfermedadActual         = reem($_POST['enfermedad']);
...
$edadMeses  = $_POST['edadMeses'];
  ...
  if ($peso == '') {$peso = 0;}
        ...
        $notasadicionales           = reem($_POST['notasadicionales']);
        ...
        $Antecedentes_Ginecologicos = $_POST['Antecedentes_Ginecologicos'];
        ...
        $otros_examenesRealizar                 = reem($_POST['otros_examenesRealizar']);
...
*/
function recorrerArregloYGuardarDatos($arreglo,$Titulo) {
  $final = "";
  ...
  $ArregloP_Bioquimicos_Iniciales['Trae?']=reem($_POST['TRAE']);
  $ArregloP_Bioquimicos_Iniciales['Fecha']=reem($_POST['fecha1']);
  $ArregloP_Bioquimicos_Iniciales['Hb']=reem($_POST['hb']);
  $ArregloP_Bioquimicos_Iniciales['Htcto']=reem($_POST['Htcto']);
  $ArregloP_Bioquimicos_Iniciales['G. blancos']=reem($_POST['blancos']);
  
  //////////////////////////////////////////////////

  $ArregloP_Bioquimicos_Lipidico['Col Total']=reem($_POST['Coltotal']);
  $ArregloP_Bioquimicos_Lipidico['TG']=reem($_POST['TG']);
  $ArregloP_Bioquimicos_Lipidico['Col LDL']=reem($_POST['ColLDL']);
  $ArregloP_Bioquimicos_Lipidico['Col HDL']=reem($_POST['COLHDL']);

  //////////////////////////////////////////////////

  $ArregloP_Bioquimicos_Glucemia['Ayunas']=reem($_POST['Ayunas']);
  $ArregloP_Bioquimicos_Glucemia['Pre Carga (Curva)']=reem($_POST['curva']);
  $ArregloP_Bioquimicos_Glucemia['Post']=reem($_POST['Post']);
  $ArregloP_Bioquimicos_Glucemia['A1C']=reem($_POST['A1C']);
  $ArregloP_Bioquimicos_Glucemia['Acido']=reem($_POST['ACIDO']);
  
  //////////////////////////////////////////////////
  
  $ArregloP_Bioquimicos_Tiroidea['TSH']=reem($_POST['TSH']);
  $ArregloP_Bioquimicos_Tiroidea['T3']=reem($_POST['T3']);
  $ArregloP_Bioquimicos_Tiroidea['T4']=reem($_POST['T4']);

  //////////////////////////////////////////////////
  
  $ArregloP_Bioquimicos_Insulina['Ayunas']=reem($_POST['Ayunasi']);
  $ArregloP_Bioquimicos_Insulina['Pre']=reem($_POST['pre']);
  $ArregloP_Bioquimicos_Insulina['Post']=reem($_POST['post1']);

  //////////////////////////////////////////////////
  
  $ArregloP_Bioquimicos_Hepatico['AST']=reem($_POST['AST']);
  $ArregloP_Bioquimicos_Hepatico['ALT']=reem($_POST['ALT']);
  $ArregloP_Bioquimicos_Hepatico['TGO']=reem($_POST['TGO']);

  //////////////////////////////////////////////////
  
  $ArregloP_Bioquimicos_Vitaminas['Calcio']=reem($_POST['Calcio']);
  $ArregloP_Bioquimicos_Vitaminas['Vit D']=reem($_POST['VitD']);
  $ArregloP_Bioquimicos_Vitaminas['Vit B12']=reem($_POST['VitB12']);

  //////////////////////////////////////////////////
  
  $ArregloP_Bioquimicos_Gestante['Toxop']=reem($_POST['Toxop']);
  $ArregloP_Bioquimicos_Gestante['IgG']=reem($_POST['IgG']);
  $ArregloP_Bioquimicos_Gestante['IgM']=reem($_POST['IgM']);

  //////////////////////////////////////////////////

  $ArregloP_Bioquimicos_Otros['Otros']=reem($_POST['otros']);
  $ArregloP_Bioquimicos_Otros['Observaciones']=reem($_POST['obser']);

  

  $TextoParametrosBioquimicos = recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Iniciales,"");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Lipidico,"Perfil Lipídico");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Glucemia,"Glucemia");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Tiroidea,"Función Tiroidea");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Insulina,"Insulina");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Hepatico,"Perfil Hepático");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Vitaminas,"Perfil Vitaminas");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Gestante,"Perfil Gestante");
  $TextoParametrosBioquimicos .= recorrerArregloYGuardarDatos($ArregloP_Bioquimicos_Otros,"");

  $parametros = $TextoParametrosBioquimicos;
 ...
 $hrasS             = reem($_POST['hrasS']);
  if ($hrasS <> ''  ) {$hrasS1   = 'Horas de Sueño/Día (Horas): '.$hrasS."<br>";}

 $SUEÑO              = reem($_POST['SUEÑO']);
  if ($SUEÑO <> '') {$SUEÑO1   = 'Calidad del Sueño: '.$SUEÑO."<br>";} 

 $actividadF              = reem($_POST['actividadF']);
  if ($actividadF  <> '') {$actividadF1   = 'Actividad Física: '.$actividadF."<br>";} 

 $tipoA               = reem($_POST['tipoA']);
  if ($tipoA <> '') {$tipoA1   = 'Tipo de Actividad: '.$tipoA."<br>";} 

//$titulo9='<br> <i>Estrés Laboral:</i>';
 $alto              = reem($_POST['alto']);
if ($alto  <> ''  ) {$alto1   = 'Estrés: '.$alto."<br>";}
...
$titulo10='<br> <i>Vicios:</i> <br>';
  ...
  */
   ...
   $ArregloP_Vicios['Alcohol']=reem($_POST['alcohol']);
   $ArregloP_Vicios['Cantidad de Alcohol']=reem($_POST['cantidadalcohol']);
   $ArregloP_Vicios['Frecuencia de Alcohol']=reem($_POST['frecuencia']);
   $ArregloP_Vicios['Cigarrillo']=reem($_POST['cigarrillo']);
   $ArregloP_Vicios['Cantidad de Cigarrillo']=reem($_POST['cantidad1']);
   $ArregloP_Vicios['Frecuencia de Cigarrillo']=reem($_POST['frecuencia1']);
   
   $TextoParametrosVicios .= recorrerArregloYGuardarDatos($ArregloP_Vicios,"<br>Vicios");
...
//$estiloVida=$hrasS1.$SUEÑO1.$actividadF1.$tipoA1.$alto1.$moderado1.$bajo1.$titulo10.$alcohol1.$cantidadalcohol1.$frecuencia1.$cigarrillo1.$cantidad11.$frecuencia11; 
  $estiloVida=$hrasS1.$SUEÑO1.$actividadF1.$tipoA1.$alto1.$TextoParametrosVicios; 



 $Fuerza             = reem($_POST['Fuerza']);
  if ($Fuerza  <> ''  ) {$Fuerza1   = 'Fuerza: '.$Fuerza."<br>";}

 $Dinamometro             = reem($_POST['Dinamometro']);
  if ( $Dinamometro   <> '') { $Dinamometro1   = 'Dinamómetro: '. $Dinamometro."<br>";} 

 $masa             = reem($_POST['masa']);
  if ($masa  <> '') {$masa1   = 'Masa: '.$masa."<br>" ;} 

 $compocorporal             = reem($_POST['compocorporal']);
  if ($compocorporal   <> '') {$compocorporal1   = 'Composición Corporal: '.$compocorporal."<br>";} 

 $Rendimientomuscular               = reem($_POST['Rendimientomuscular']);
  if ($Rendimientomuscular<> ''  ) {$Rendimientomuscular1   = 'Rendimiento Muscular: '.$Rendimientomuscular."<br>";}

 $velocidad                = reem($_POST['velocidad']);
  if ($velocidad   <> '') {$velocidad1   = 'Velocidad: '.$velocidad."<br>";} 

 $marcha             = reem($_POST['marcha']);
  if ( $marcha   <> '') { $marcha1   = 'Marcha: '. $marcha."<br>";} 

 $equilibrio               = reem($_POST['equilibrio']);
  if ($equilibrio   <> '') {$equilibrio1   = 'Equilibrio: '.$equilibrio."<br>";} 

 $silla             = reem($_POST['silla']);
  if ($silla  <> '') {$silla1   = 'Silla: '.$silla."<br>";} 
 ...
 $alergiasA            = reem($_POST['alergiasA']);
  if ($alergiasA  <> ''  ) {$alergiasA1   = 'Alergias: '.$alergiasA."<br>"  ;}

 $cuales1           = reem($_POST['cuales1']);
  if ($cuales1  <> '') {$cuales11   = 'Cuales?: '.$cuales1."<br>";} 
  $titulo11='<br> <i>Síntomas GI:</i>';

 $gastritis             = reem($_POST['gastritis']);
  if ($gastritis  <> '') {$gastritis1   = 'Síntomas Gastrointestinales: '.$gastritis."<br>" ;} 

$rge              = reem($_POST['varios']);
  if ($rge   <> '') {$rge1   = 'Varios: '.$rge."<br>" ;} 
 ...
 $adecuado             = reem($_POST['adecuado']);
  if ($adecuado    <> '') { $adecuado1   = 'Hábito Intestinal: '. $adecuado."<br>"    ;} 

   $bristol             = reem($_POST['bristol']);
  if ($bristol   <> '') {$bristol1   = 'Bristol: '.$bristol."<br>" ;}
 ...
 $estreñ             = reem($_POST['estreñ']);
  ...
  $ArregloP_Alimentaria['Presenta Atracones o Sobreingestas?']=reem($_POST['sobreI']);
  $ArregloP_Alimentaria['Restricciones Alimentarias']=reem($_POST['RESTRICCIONE']);
  $ArregloP_Alimentaria['Alimentos que Producen Temor']=reem($_POST['alimentos']);
  $ArregloP_Alimentaria['Intolerancias']=reem($_POST['intorelancias']);
  $ArregloP_Alimentaria['Rechazos']=reem($_POST['Rechazos']);
  $ArregloP_Alimentaria['Preferencias']=reem($_POST['Preferencias']);
  $ArregloP_Alimentaria['Consumo de Suplementos']=reem($_POST['Suplementos']);

  $TextoParametrosAlimentaria .= recorrerArregloYGuardarDatos($ArregloP_Alimentaria,"<br>Conducta Alimentaria");
...
$Ensaladas             = reem($_POST['Ensaladas']);
  ...
  */
  ...
  $ArregloP_Alimentaria_1['Bebidas Azucaradas']=reem($_POST['bebidas']);
  $ArregloP_Alimentaria_1['Productos de Panaderia']=reem($_POST['panaderia']);
  $ArregloP_Alimentaria_1['Dulces y Postres']=reem($_POST['dulces']);
  $ArregloP_Alimentaria_1['Comidas Rápidas']=reem($_POST['comidasR']);
  $ArregloP_Alimentaria_1['Fritos']=reem($_POST['fritos']);
  $ArregloP_Alimentaria_1['Ensaladas']=reem($_POST['Ensaladas']);
  $ArregloP_Alimentaria_1['Frutas']=reem($_POST['Frutas']);
  $ArregloP_Alimentaria_1['Granos']=reem($_POST['Granos']);
  $ArregloP_Alimentaria_1['Embutidos']=reem($_POST['embutidos']);
  $ArregloP_Alimentaria_1['Sal']=reem($_POST['sal']);
  $ArregloP_Alimentaria_1['Agua']=reem($_POST['agua']);

  $TextoParametrosAlimentaria_1 .= recorrerArregloYGuardarDatos($ArregloP_Alimentaria_1,"<br>Frecuencia de Consumo");
...
$anamnesis=$alergiasA1.$cuales11.$gastritis1.$rge1.$colon1.$adecuado1.$estreñ1.$bristol1.$TextoParametrosAlimentaria.$TextoParametrosAlimentaria_1;

//?Importante si modifican y agregan o quitan algun caracteres del modulo de consumoH en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de consumoH para evitar que se visualize la tabla si esta vacia
//?Importante si modifican y agregan o quitan algun caracteres del modulo de consumoH en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de consumoH para evitar que se visualize la tabla si esta vacia
//?Importante si modifican y agregan o quitan algun caracteres del modulo de consumoH en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de consumoH para evitar que se visualize la tabla si esta vacia

$hora1           = reem($_POST['hora1']);
  if ($hora1 <> '') { $hora11  ='<tr> <td> Hora:'.$hora1.'</td>';}else{$hora11 = '<tr> <td>&nbsp;</td>';} 

$preparacion1           = reem($_POST['preparacion1']);
  if ($preparacion1 <> '') { $preparacion11P  =' <td>'.$preparacion1.'</td>' ;}else{$preparacion11P = ' <td>&nbsp;</td>';} 

 $cantidad01            = reem($_POST['cantidad01']);
  if ($cantidad01 <> '') {$cantidad11P  ='<td>'.$cantidad01.'</td>';}else{$cantidad11P = ' <td>&nbsp;</td>';} 
...
$preparacion51           = reem($_POST['preparacion51']);
 ...
 $ic51             = reem($_POST['ic51']);
  if ($ic51 <> '') {$ic5151  =' <td>'.$ic51.'</td></tr>';}else{$ic5151 = ' <td></td></tr>';} 
...
$preparacion52           = reem($_POST['preparacion52']);
 ...
 $ic54             = reem($_POST['ic54']);
  if ($ic54 <> '') {$ic5454  =' <td>'.$ic54.'</td></tr>';}else{$ic5454 = ' <td></td></tr>';} 
...
$titulo15= '<table class= "table table-bordered"><tr> </tr>';
$titulos= '<tr><td> <b>Alimento/Preparación </b></td><td> Cantidad</td> <td> #Ic </td></tr>';
$comida1='<tr><td><b>Desayuno</b></td> </tr>';
$comida2='<tr><td><b>Media Mañana</b></td> </tr>';
$comida3='<tr><td><b>Almuerzo</td></b> </tr>';
...
$ec1    = $_POST['PesoActual'];
       ...
       $pesoM= $_POST['pesom'];
        ...
        //?Importante si modifican y agregan o quitan algun caracteres del modulo de antropometria en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de antropometria para evitar que se visualize la tabla si esta vacia
      //?Importante si modifican y agregan o quitan algun caracteres del modulo de antropometria en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de antropometria para evitar que se visualize la tabla si esta vacia
      //?Importante si modifican y agregan o quitan algun caracteres del modulo de antropometria en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de antropometria para evitar que se visualize la tabla si esta vacia
       
        //Datos iniciales de la 
        if(strlen($TipoSeguimiento) <> ''){$expTipoSeguimiento= 'Tipo de Seguimiento: '.$TipoSeguimiento.'<br>';}

        $contadorCampos=0;
        if(strlen($ec1) <> ''){$exp1= '<tr><td>Peso Actual(kg):'.$ec1.'</td>';$contadorCampos++;}
        if(strlen($ec2) <> ''){$exp2= '<td>Peso Usual (kg): '.$ec2.'</td>';$contadorCampos++;}
        if(strlen($ec3) <> ''){$exp3= '<td>Peso Minimo: '.$ec3.'</td>';$contadorCampos++;}
        if(strlen($ec4) <> ''){$exp4= '<td>Peso Maximo:'.$ec4.'</td>';$contadorCampos++;}
        if(strlen($ec5) <> ''){$exp5= '<td>Talla(cm):'.$ec5.'</td></tr>';$contadorCampos++;}
        if(strlen($ec7) <> ''){$exp7= '<tr><td>Observaciones Para Historia del Peso y Talla: '.$ec7.'</td></tr>';$contadorCampos++;}

        if($contadorCampos>0){$titulo1= '<tr><th colspan="4" class="text-center"> Peso, Historia del Peso y Talla</th></tr>';}else{$titulo1="";}

        $contadorCampos=0;
        if(strlen($ec8) <> ''){$exp8= '<tr><td>Cuello:'.$ec8.'</td>';$contadorCampos++;}
        if(strlen($ec9) <> ''){$exp9= '<td>Pecho/Tórax: '.$ec9.'</td>';$contadorCampos++;}
        if(strlen($ec10) <> ''){$exp10= '<td>Brazo Relajado: '.$ec10.'</td>';$contadorCampos++;}
        if(strlen($ec11) <> ''){$exp11= '<td>Brazo Contraído:'.$ec11.'</td></tr>';$contadorCampos++;}
        if(strlen($ec12) <> ''){$exp12= '<tr><td> Antebrazo:'.$ec12.'</td>';$contadorCampos++;}
       if(strlen($ec13) <> ''){$exp13= '<td>Muñeca: '.$ec13.'</td>';$contadorCampos++;}
        if(strlen($ec14) <> ''){$exp14= '<td>Cintura:'.$ec14.'</td>';$contadorCampos++;}
        if(strlen($ec15) <> ''){$exp15= '<td>Abdómen:'.$ec15.'</td></tr>';$contadorCampos++;}
         if(strlen($ec16) <> ''){$exp16= '<tr><td>Cadera: '.$ec16.'</td>';$contadorCampos++;}
         if(strlen($ec16a) <> ''){$exp16a= '<td>Muslo: '.$ec16a.'</td>';$contadorCampos++;}

        if(strlen($ec17) <> ''){$exp17= '<td>Pantorrilla: '.$ec17.'</td></tr>';$contadorCampos++;}
        if(strlen($ec18) <> ''){$exp18= '<tr><td>Observaciones Para Perímetros (Cm):'.$ec18.'</td></tr>';$contadorCampos++;}

        if($contadorCampos>0){$titulo2= '<br><tr><th colspan="4" class="text-center"> Perímetros (Cm) </th></tr>';}else{$titulo2="";}


        $contadorCampos=0;
        if(strlen($ec19) <> ''){$exp19= '<tr><td>Bíceps:'.$ec19.'</td>';$contadorCampos++;}
         if(strlen($ec20) <> ''){$exp20= '<td>Tríceps: '.$ec20.'</td>';$contadorCampos++;}
         if(strlen($ec21) <> ''){$exp21= '<td> Subescapular'.$ec21.'</td>';$contadorCampos++;}
        if(strlen($ec22) <> ''){$exp22= '<td>Cresta Ilíaca:'.$ec22.'</td></tr>';$contadorCampos++;}
        if(strlen($ec23) <> ''){$exp23= '<tr><td>Supraespinal:'.$ec23.'</td>';$contadorCampos++;}
         if(strlen($ec24) <> ''){$exp24= '<td> Abdominal:'.$ec24.'</td>';$contadorCampos++;}
        if(strlen($ec25) <> ''){$exp25= '<td> Muslo:'.$ec25.'</td>';$contadorCampos++;}
        if(strlen($ec26) <> ''){$exp26= '<td>Pierna Media:'.$ec26.'</td></tr>';$contadorCampos++;}
         if(strlen($ec27) <> ''){$exp27= '<tr><td>Observaciones Para Pliegues Cutáneos (mm):'.$ec27.'</td></tr>';$contadorCampos++;}

         if($contadorCampos>0){$titulo3= '<br><tr><th colspan="4" class="text-center"> Pliegues Cútaneos (mm) </th></tr>';}else{$titulo3="";}
...
//?Importante si modifican y agregan o quitan algun caracteres del modulo de antropometria en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de antropometria para evitar que se visualize la tabla si esta vacia
      ...
      //?Importante si modifican y agregan o quitan algun caracteres del modulo de antropometria en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de antropometria para evitar que se visualize la tabla si esta vacia
        ...
        if(strlen($ec28) <> ''){$exp28= '<tr><td>Dinamómetro: '.$ec28.'</td></tr>';$contadorCampos++;}

        if($contadorCampos>0){ $titulo4= '<br><tr><th colspan="4" class="text-center"> Fuerza Muscular </th></tr>';}else{$titulo4="";}
         ...
         $contadorCampos=0;
        if(strlen($ec29) <> ''){$exp29= '<tr><td> IMC:'.$ec29.'</td>';$contadorCampos++;}
        if(strlen($ec30) <> ''){$exp30= '<td>Estructura:'.$ec30.'</td>';$contadorCampos++;}
        if(strlen($ec31) <> ''){$exp31= '<td>Peso Saludable: '.$ec31.'</td>';$contadorCampos++;}
        if(strlen($ec32) <> ''){$exp32= '<td>Relación Cintura/Cadera:'.$ec32.'</tr>';$contadorCampos++;}
        if(strlen($ComposicionCorporal) <> ''){$exp37= '<td>Composición Corporal:'.$ComposicionCorporal.'</td></tr>';$contadorCampos++;}

        if($contadorCampos>0){ $titulo5= '<br><tr><th colspan="4" class="text-center"> Resultados</th></tr>';}else{$titulo5="";}
         ...
         $contadorCampos=0;
        if(strlen($ecgrasapor) <> ''){$expgrasapor= '<tr><td>Grasa: '.$ecgrasapor.'</td>';$contadorCampos++;}
        if(strlen($ec33) <> ''){$exp33= '<tr><td>Suma Pliegues: '.$ec33.'</td>';$contadorCampos++;}
        if(strlen($ec34) <> ''){$exp34= '<td>Peso Graso: '.$ec34.'</td>';$contadorCampos++;}
        if(strlen($ec35) <> ''){$exp35= '<td>Peso Magro: '.$ec35.'</td>';$contadorCampos++;}
        if(strlen($ec36) <> ''){$exp36= '<td>IAKS: '.$ec36.'</td></tr>';$contadorCampos++;}
        if(strlen($interpretacion) <> ''){$exp38= '<td>Interpretación de Grasa: '.$interpretacion.'</td></tr>';$contadorCampos++;}

        if($contadorCampos>0){ $titulo6= '<br><tr><th colspan="4" class="text-center"> % Grasas(YUHASZ)</th></tr>';}else{$titulo6="";}

 
        $contadorCampos=0;
        if(strlen($masap) <> ''){$exp39= '<tr><td>Masa Piel: '.$masap.'</td>';$contadorCampos++;}
        if(strlen($masaA) <> ''){$exp40= '<td>Masa Adiposa: '.$masaA.'</td>';$contadorCampos++;}
        if(strlen($pesoM) <> ''){$exp41= '<td>Peso Muscular: '.$pesoM.'</td>';$contadorCampos++;}

        if($contadorCampos>0){ $titulo7= '<br><tr><th colspan="4" class="text-center"> Análisis de Composición Corporal </th></tr>';}else{$titulo7="";}
      ...
      //?Importante si modifican y agregan o quitan algun caracteres del modulo de antropometria en el guardar deberan modificar en la impresion cuantos caracteres tiene al estar vacio y actualizarlo en el apartado de antropometria para evitar que se visualize la tabla si esta vacia
        ...
        $antropometria= $expTipoSeguimiento.'<table class="table table-bordered"><tr>'.$titulo1.$exp1.$exp2.$exp3.$exp4.$exp5.$exp6.$exp7. $titulo2.$exp8.$exp9.$exp10.$exp11.$exp12.$exp43.$exp44.$exp45.$exp46.$exp13.$exp14.$exp15.$exp16.$exp16a.$exp17.$exp18.$titulo3.$exp19.$exp20.$exp21.$exp22.$exp23.$exp24.$exp25.$exp26.$exp27.$titulo4.$exp28.$titulo5.$exp29.$exp30.$exp31.$exp32.$exp37.$titulo6.$expgrasapor.$exp33.$exp34.$exp35.$exp36.$exp38.$titulo7.$exp39.$exp40.$exp41.'</tr></table>';
 ...
 $diaganosticoN             = reem($_POST['diaganosticoN']);
  if ($diaganosticoN <> '') {$diaganosticoN1   = ' Diagnóstico Nutricional: '.$diaganosticoN."<br>";} 

 $objetivoP1        = reem($_POST['objetivoP1']);
  if ($objetivoP1<> '') {$objetivoP11   = 'Objetivo (Paciente + ND): '.$objetivoP1."<br>";} 

  $objetivoP        = reem($_POST['objetivoP']);
  if ($objetivoP<> '') {$objetivoP111   = ' ¿Qué Está Dispuesto a Cambiar?: '.$objetivoP."<br>";} 

$PLAN          = reem($_POST['PLAN']);
  if ($PLAN <> '') {$PLAN1   = 'Concepto y Plan: '.$PLAN."<br>";} 
 
$ConceptoyPlan = $diaganosticoN1.$objetivoP11.$objetivoP111.$PLAN1;


   {
                //echo 'cargamos el archivo';
                $nombre = $_FILES['archivo1']['name'].rand(0,10);
                ...
                if (!empty($resultado))
            {
                ...
                */
                ...
                mysqli_query($conn3, "UPDATE historiaClinica1 SET  archivo = '$nombrer'  WHERE usuario_id = '$ID'");
        ...
        ////////////////////////////////////////////////////////
        //TABLAS NUEVAS 
        $ArrayGI = $_POST['ArrayGI'];
        $tablaGastrointestinales = '
        <table class="tg" style="border-collapse: collapse;width:100%">
        <thead>
          <tr style="border: 1px solid black;">
            <th class="tg-0pky" colspan="2" style="border: 1px solid black;">Problemas Gastrointestinales</th>
          </tr>
        </thead>
        <tbody>';
        ...
        foreach (array_filter($ArrayGI) as $keyGI => $valueGI) {
          $tablaGastrointestinales.=  '<tr>
                        <td class="tg-0lax" style="border: 1px solid black;">'.$keyGI.'</td>
                        <td class="tg-0lax" style="border: 1px solid black;">'.$valueGI.'</td>
                    </tr>';
        }
        $tablaGastrointestinales .= '
        </tbody>
        ...
        ////////////////////////////////////////////////////////
        $ArrayAF = $_POST['ArrayAF'];
        $tablaAntecedentesFamiliares = '
        <table class="tg" style="border-collapse: collapse; width:100%">
        <thead>
          <tr style="border: 1px solid black;">
            <th class="tg-0pky" colspan="2" style="border: 1px solid black;">Antecedentes Familiares</th>
          </tr>
        </thead>
        <tbody>';
        
        foreach (array_filter($ArrayAF) as $keyAF => $valueAF) {
            $tablaAntecedentesFamiliares .=  '<tr style="border: 1px solid black;">
                        <td class="tg-0lax" style="border: 1px solid black;">'.$keyAF.'</td>
                        <td class="tg-0lax" style="border: 1px solid black;">'.$valueAF.'</td>
                    </tr>';
        }
        $tablaAntecedentesFamiliares .= '
        </tbody>
        ...
        $ArrayAN = $_POST['ArrayAN'];
        $tablaAntecedentesNutricionales = '
        <table class="tg" style="border-collapse: collapse;width:100%">
        <thead>
          <tr style="border: 1px solid black;">
            <th class="tg-0pky" colspan="2" style="border: 1px solid black;">Antecedentes Nutricionales</th>
          </tr>
        </thead>
        <tbody>';
        ...
        foreach (array_filter($ArrayAN) as $keyAN => $valueAN) {
            $tablaAntecedentesNutricionales .=  '<tr>
                        <td class="tg-0lax" style="border: 1px solid black;">'.$keyAN.'</td>
                        <td class="tg-0lax" style="border: 1px solid black;">'.$valueAN.'</td>
                    </tr>';
        }
        $tablaAntecedentesNutricionales .= '
        </tbody>
        ...
        // var_dump($tablaGastrointestinales);
        // echo "<br><br><br>";  
        // var_dump($tablaAntecedentesFamiliares);
        // echo "<br><br><br>";
        // var_dump($tablaAntecedentesNutricionales);
        // echo "<br><br><br>";

        
        

        $Campo1 = mysqli_query($conn3, "show COLUMNS from nutricionAdulto WHERE Field = 'ConceptoyPlan';");
        $nrowCampo1 = mysqli_num_rows($Campo1);
        if ($nrowCampo1 == "0") {
            mysqli_query($conn3, "ALTER TABLE `nutricionAdulto` ADD `ConceptoyPlan` TEXT NULL  COMMENT '*Creado desde modulo de Guardar Nutricion*'");
        }


        $queryAuditor = "INSERT INTO nutricionAdulto (cliente_id, usuario_id, Fecha, Hora, remite, motivoConsulta, diagnostico, tratamiento, notas, recipe, incapacidades, cie10, rSistema,  enfermedadActual, acompananteFamiliar, telefono_acompanante, paraClinicos, remision, diagnosticoMsalud, comoTomarlo, antecedentesPers,antecedentesFami,antenutricion, parametros, estiloVida, funcionalidadMuscular,anamnesis,consumoH,antropometria,tipo, archivo, examenPartesdCuerpo, laboratorio, ecografia, otros, Antecedentes_personales, Antecedentes_Familiares, Antecedentes_Ginecologicos, notasadicionales, enfermedad,ConceptoyPlan) VALUES  ('$clienteId', '$ID', '$fechar', '$hora', '$remite','$motivoConsulta', '$diagnostico', '$tratamiento', '$notas',  '$recipe', '$incapacidades', '$cie10', '$rSistema',  '$acompananteFamiliar', '$telefono_acompanante', '$paraClinicos', '$remision', '$diagnosticoMsalud', '$comoTomarlo','$enfermedadActual','$antecedentesPers','$antecedentesFami','$antenutricion','$parametros','$estiloVida','$funcionalidadMuscular','$anamnesis','$consumoH','$antropometria', '$TIPOA' ,'$nombrer', '$examenPartesdCuerpo', '$laboratorio', '$ecografia', '$otros_examenesRealizar', '$Antecedentes_personales', '$Antecedentes_Familiares', '$Antecedentes_Ginecologicos', '$notasadicionales', '$enfermedad','$ConceptoyPlan'";
...
auditorMaster($idusuario, '1', $enlace_actual, $queryAuditor);
        ...
        mysqli_query($conn3,"INSERT INTO nutricionAdulto (cliente_id, usuario_id, Fecha, Hora, remite, motivoConsulta, diagnostico, tratamiento, notas, recipe, incapacidades, cie10, rSistema,  enfermedadActual, acompananteFamiliar, telefono_acompanante, paraClinicos, remision, diagnosticoMsalud, comoTomarlo, antecedentesPers,antecedentesFami,antenutricion, parametros, estiloVida, funcionalidadMuscular,anamnesis,consumoH,antropometria,tipo, archivo, examenPartesdCuerpo, laboratorio, ecografia, otros, Antecedentes_personales, Antecedentes_Familiares, Antecedentes_Ginecologicos, notasadicionales, enfermedad,ConceptoyPlan,problemasGastrointestinales,antecedentesNutricionales,finalidadConsulta) VALUES  ('$clienteId', '$ID', '$fechar', '$hora', '$remite','$motivoConsulta', '$diagnostico', '$tratamiento', '$notas',  '$recipe', '$incapacidades', '$cie10', '$rSistema',  '$acompananteFamiliar', '$telefono_acompanante', '$paraClinicos', '$remision', '$diagnosticoMsalud', '$comoTomarlo','$enfermedadActual','$antecedentesPers','$tablaAntecedentesFamiliares','$antenutricion','$parametros','$estiloVida','$funcionalidadMuscular','$anamnesis','$consumoH','$antropometria', '$TIPOA' ,'$nombrer', '$examenPartesdCuerpo', '$laboratorio', '$ecografia', '$otros_examenesRealizar', '$Antecedentes_personales', '$Antecedentes_Familiares', '$Antecedentes_Ginecologicos', '$notasadicionales', '$enfermedad','$ConceptoyPlan','$tablaGastrointestinales','$tablaAntecedentesNutricionales','$finalidadConsulta');");
...
foreach ($codigo1 as $e) {
  ...
  */      //
  ...
  mysqli_query($conn3, "INSERT INTO historiaClinica9_Quirurgico_Cie10 (cliente_id,usuario_id,historiaClinica9_id,Fecha,Hora,codigo, Tipo) VALUES ('$clienteId', '$ID', '$historiaClinica1','$fechar','$hora','$codigocie10', '2')");
}
