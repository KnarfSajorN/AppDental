...
function ActualizarDetallesOdontograma($id_detalle)
{
    ...
    include 'funciones/conn3.php';
    $id = $id_detalle;
    //consultar el campo Numero_Diente y Nombre_Cara y cliente_id de la tabla ODP_OdontogramaMasterDetalle
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE id = '$id' limit 1");
    ...
    //consultar el campo d$Numero_diente y r$Numero_diente de la tabla ODP_OdontogramaMaster
    $queryOdontogramaDetalle = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMaster WHERE cliente_id = '$cliente_id' AND usuario_id='$usuario_id' limit 1");
    ...
    $QueryDetalleOdonto = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE  Numero_Diente = '$Numero_Diente' AND Posicion = 'Toda La Pieza' AND cliente_id = '$cliente_id' AND usuario_id='$usuario_id' AND Activo = 1 ORDER BY id DESC limit 1");
    ...
    //Aqui se busca si existe el procedimiento en la tabla ODP_OdontogramaMasterDetalle por cada cara y que sea el ultimo procedimiento activo   
    $ArregloPosiciones = array("RecuadroSuperior", "RecuadroIzquierdo", "RecuadroInferior", "RecuadroDerecha", "RecuadroCentro", "RecuadroSeparadoSuperior", "RecuadroSeparadoInferior");
    foreach ($ArregloPosiciones as $key_Posiciones => $value_Posiciones) {

        $QueryDetalleOdonto = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE  Numero_Diente = '$Numero_Diente' AND Posicion = '$value_Posiciones' AND cliente_id = '$cliente_id' AND usuario_id='$usuario_id' AND Activo = 1 AND id>'$id_TodaLaPieza' ORDER BY id DESC limit 1");
        $rowDetalleOdonto = mysqli_num_rows($QueryDetalleOdonto);
        if ($rowDetalleOdonto > 0) {
    ...
    //actualizar el campo d$Numero_diente y r$Numero_diente de la tabla ODP_OdontogramaMaster
    $queryOdontogramaDetalle = mysqli_query($conn3, "UPDATE ODP_OdontogramaMaster SET d$Numero_Diente = '" . json_encode($ArregloD_Array) . "' WHERE id = '$idOdontogramaMaster' limit 1");
    $Afectadas[1] = mysqli_affected_rows($conn3);
    //echo de la consulta
    //echo "UPDATE ODP_OdontogramaMaster SET d$Numero_Diente = '".json_encode($ArregloD_Array)."' WHERE id = '$idOdontogramaMaster' limit 1";
    $queryOdontogramaMaster = mysqli_query($conn3, "UPDATE ODP_OdontogramaMaster SET r$Numero_Diente = '" . json_encode($ArregloR_Array) . "' WHERE id = '$idOdontogramaMaster' limit 1");
    $Afectadas[2] = mysqli_affected_rows($conn3);
    //echo de la consulta
    //echo "UPDATE ODP_OdontogramaMaster SET r$Numero_Diente = '".json_encode($ArregloR_Array)."' WHERE id = '$idOdontogramaMaster' limit 1";
...
function PresupuestarDetalle($detalle_id_odontograma, $inventario)
{
    ...
    if ($nrowtabla == 0) {
        ...
        `inventario_id` TEXT NULL DEFAULT '0' COMMENT 'id de la tabla ODP_Procedimiento / seria el tratamiento que seleccionan al guardar el detalle dental',
        `Estado` TEXT NULL DEFAULT '0' COMMENT '0-> No Facturado/Presupuestado | 1-> Facturado/Presupuestado',
        `detalle_odontograma_id` text NULL DEFAULT '' COMMENT 'id de la tabla ODP_OdontogramaMasterDetalle',
...
if ($_POST["Tipo_Consulta"] == "Agregar Dato Odontograma" or $_POST["Tipo_Consulta"] == "Agregar Dato Odontograma Inicial") {

    $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'ODP_OdontogramaMaster'");
    $nrowtabla = mysqli_num_rows($tabla);
    if ($nrowtabla == 0) {

        $query = "CREATE TABLE `ODP_OdontogramaMaster` (
        `id` int(11) NOT NULL,
        `Fecha` date DEFAULT NULL,
        `Hora` time NOT NULL,
        `cliente_id` int(11) DEFAULT NULL,
        ...
        if (!$creaciontabla) {
            ...
            mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMaster` ADD PRIMARY KEY (`id`);");
            mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMaster` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;");
        }
    }
    ...
    $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'ODP_OdontogramaMasterDetalle'");
    $nrowtabla = mysqli_num_rows($tabla);
    if ($nrowtabla == 0) {

        $query = "CREATE TABLE `ODP_OdontogramaMasterDetalle` (
        `id` int(11) NOT NULL,
        `Fecha` date DEFAULT NULL,
        `Hora` time NOT NULL,
        `cliente_id` int(11) DEFAULT NULL,
        ...
        `inventario_id` text NULL DEFAULT '' COMMENT 'si es diferente de 0 es que adjuntaron un tratamiento con un inventario *Creado desde modulo de ODP_Ajax*',
        ...
        if (!$creaciontabla) {
            ...
            mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` ADD PRIMARY KEY (`id`);");
            mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;");
        }
    }

    //Tema de facturacion
    $Campo1 = mysqli_query($conn3, "show COLUMNS from ODP_OdontogramaMasterDetalle WHERE Field = 'detalle_id_factura';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` ADD `detalle_id_factura` TEXT NULL DEFAULT '0' COMMENT 'si es 0 es que no esta facturado  el detalle, si es otro numero sera el id de sdetalleoper *Creado desde modulo de OD_Ajax*'");
    }
    //NuevosCampos

    $Campo1 = mysqli_query($conn3, "show COLUMNS from ODP_OdontogramaMasterDetalle WHERE Field = 'inventario_id';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` ADD `inventario_id` TEXT NULL DEFAULT '0' COMMENT 'si es diferente de 0 es que adjuntaron un tratamiento con un inventario *Creado desde modulo de OD_Ajax*'");
    }
    $Campo1 = mysqli_query($conn3, "show COLUMNS from ODP_OdontogramaMasterDetalle WHERE Field = 'cie_10';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` ADD `cie_10` TEXT NULL DEFAULT '0' COMMENT 'cie-10 de la tabla OD_Cie10 *Creado desde modulo de OD_Ajax*'");
    }
    //////////////////////

    $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'ODP_FirmaDetalle'");
    $nrowtabla = mysqli_num_rows($tabla);
    if ($nrowtabla == 0) {

        $query = "CREATE TABLE `ODP_FirmaDetalle` (
        `id` int(11) NOT NULL,
        `Fecha` date DEFAULT NULL,
        `Hora` time NOT NULL,
        `usuario_id` int(11) DEFAULT NULL COMMENT 'usuario que envia el mensaje ya sea whatsapp/correo',
        ...
        if (!$creaciontabla) {
            ...
            mysqli_query($conn3, "ALTER TABLE `ODP_FirmaDetalle` ADD PRIMARY KEY (`id`);");
            mysqli_query($conn3, "ALTER TABLE `ODP_FirmaDetalle` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;");
        }
    }
}
...
if ($_POST["Tipo_Consulta"] == "Agregar Dato Odontograma") {
    ...
    $QueryOdontograma = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMaster WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' ");
    $NumRowOdontograma = mysqli_num_rows($QueryOdontograma);

    if ($NumRowOdontograma == 0) {
        mysqli_query($conn3, "INSERT INTO ODP_OdontogramaMaster(usuario_id, cliente_id, Fecha, Hora ) 
    ...
    $inventario_id = funcionMaster($Procedimiento, 'id', 'inventario_id', 'ODP_Procedimiento') ?: 0;
    $cie_10 = $_POST["cie_10"];

    //Insertar
    mysqli_query($conn3, "INSERT INTO ODP_OdontogramaMasterDetalle (usuario_id, cliente_id,     Fecha, Hora,      Numero_Diente, Detalle,   Procedimiento,    Posicion, NombreCara, inventario_id, cie_10) 
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMaster WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' LIMIT 1");
    ...
    $queryList = mysqli_query($conn3, "UPDATE ODP_OdontogramaMaster SET d{$Numero_Diente}='$ADientes', r{$Numero_Diente}='$RDientes' WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' limit 1;");

    //echo "UPDATE ODP_OdontogramaMaster SET d{$Numero_Diente}='$ADientes', r{$Numero_Diente}='$RDientes' WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' limit 1;";
    $SVG = funcionMaster(funcionMaster($Procedimiento, 'id', 'Icono', 'ODP_Procedimiento'), 'id', 'SVG', 'ODP_Iconos_SVG');
    $Color = funcionMaster($Procedimiento, 'id', 'Color', 'ODP_Procedimiento');
...
elseif ($_POST["Tipo_Consulta"] == "Cargar Imagen Pieza") {
    ...
    while ($RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster)) {
        ...
        foreach (json_decode($ArregloD) as $key => $value) {
            ...
            $Activo = funcionMaster($ArregloR[$key], 'id', 'Activo', 'ODP_OdontogramaMasterDetalle');

            $SVG = funcionMaster(funcionMaster($value, 'id', 'Icono', 'ODP_Procedimiento'), 'id', 'SVG', 'ODP_Iconos_SVG');
            $Color = funcionMaster($value, 'id', 'Color', 'ODP_Procedimiento');
            $SVG = str_replace('fill="currentColor"', "fill='{$Color}'", $SVG);
            $Arreglo[] = $SVG;
        }
    }
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Numero_Diente='$diente_id' AND Activo = 1 ORDER BY id DESC LIMIT 1 ");
    while ($RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster)) {
        $Procedimiento = $RowOdontogramaMaster["Procedimiento"];
    }
    ...
    echo $SVG . "|" . funcionMaster($Procedimiento, 'id', 'Nombre', 'ODP_Procedimiento');
    ...
    echo "<table class='table table-bordered table-striped' style='text-align-last: center;'>
        ...
        <tbody>";
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Numero_Diente='$diente_id' AND Activo = 1 ORDER BY id DESC");
    while ($RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster)) {

        $id_detalle = $RowOdontogramaMaster["id"];
        $Nombre_Procedimiento = funcionMaster($RowOdontogramaMaster["Procedimiento"], 'id', 'Nombre', 'ODP_Procedimiento');
        ...
        */
        ...
        $EstadoFirma = funcionMaster($RowOdontogramaMaster["id"], 'detalle_id', 'id', 'ODP_FirmaDetalle');
        $BotonAdicional = "";
        $BotonesAcciones = "";
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Activo=1 ORDER BY id DESC");
    while ($RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster)) {
...
elseif ($_POST["Tipo_Consulta"] == "Eliminar Detalle Odontograma") {
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "UPDATE ODP_OdontogramaMasterDetalle SET Activo = 0 WHERE id = '$id' limit 1");
    ...
    */
    ...
    $queryOdontogramaMaster = mysqli_query($conn3, "UPDATE ODP_OdontogramaMasterDetalle SET Detalle = '{$Detalle}',Procedimiento = '{$Procedimiento}',Posicion = '{$Posicion}',NombreCara = '{$NombreCara}',inventario_id='{$inventario_id}', cie_10='{$cie_10}' WHERE id = '$id_detalle' limit 1") or die(mysqli_error($conn3));
    //FacturarDetalle($id_detalle);
    $AfectadasUp = mysqli_affected_rows($conn3);

    if ($AfectadasUp > 0) {
        $queryOdontogramaMaster = mysqli_query($conn3, "UPDATE ODP_OdontogramaMasterDetalle SET Fecha='{$Fecha}', Hora='{$Hora}' WHERE id = '$id_detalle' limit 1");
        $Afectadas = ActualizarDetallesOdontograma($id_detalle);
        $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE id = '$id_detalle' limit 1");
        $RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster);
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE id = '$detalle_odontograma_id' limit 1");
    $RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster);

    $usuario_id = $RowOdontogramaMaster["usuario_id"];
    $Numero_Diente = $RowOdontogramaMaster["Numero_Diente"];
    $NombreCara = $RowOdontogramaMaster["NombreCara"];
    $Procedimiento = funcionMaster($RowOdontogramaMaster["Procedimiento"], 'id', 'Nombre', 'ODP_Procedimiento');

    //$tabla_encriptado = Encriptar("ODP_FirmaDetalle"); //nombre de la tabla
    ...
    $mensajeW = " Sr(a) *" . trim(funcionMaster($cliente_id, 'cliente_id', 'nombre_cliente', 'cliente'), " ") . "* Se le ha registrado un detalle en la pieza dental #{$Numero_Diente} en {$NombreCara} el procedimiento {$Procedimiento} por el Doctor " . funcionMaster($usuario_id, 'ID', 'NOMBRE_USUARIO', 'usuarios') . ", para firmar el detalle utilizar el siguiente link, Link: {$Base}ODP_ModuloFirma.php?id={$detalle_id_encriptado}&ui={$usuario_encriptado}";
    $action = 0;
    ...
    $titulo = "Medicalsoft - Detalle Odontograma  #" . $CodigoUnico;
    ...
    $url = ["{$Base}ODP_ModuloFirma.php?id={$detalle_id_encriptado}&ui={$usuario_encriptado}"];
    $botonurl = ["Firmar"];
    ...
    $cabeceras .= 'To: ' . funcionMaster($usuario_id, 'ID', 'NOMBRE_USUARIO', 'usuarios') . '_' . $CodigoUnico . ' <noreply@medicalsoftplus.com>' . "\r\n";
    ...
    $queryList = mysqli_query($conn3, "SELECT * FROM ODP_FirmaDetalle where detalle_id = '$Detalle_id' ORDER BY id DESC LIMIT 1 ");
    ...
    $Campo1 = mysqli_query($conn3, "show COLUMNS from ODP_OdontogramaMasterDetalle WHERE Field = 'Estado_Procedimiento';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` ADD `Estado_Procedimiento` TEXT NULL DEFAULT 'Registrado' COMMENT 'Estado del Odontograma*Creado desde modulo de OD_Ajax*'");
    }
    $Campo1 = mysqli_query($conn3, "show COLUMNS from ODP_OdontogramaMasterDetalle WHERE Field = 'Estado_Procedimiento_Detalle';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `ODP_OdontogramaMasterDetalle` ADD `Estado_Procedimiento_Detalle` TEXT NULL DEFAULT '' COMMENT 'Detalle del estado del odontograma *Creado desde modulo de OD_Ajax*'");
    }
    ...
    $queryPresupuestoTratamiento = mysqli_query($conn3, "UPDATE ODP_OdontogramaMasterDetalle SET Estado_Procedimiento='{$Estado_Procedimiento}', Estado_Procedimiento_Detalle='{$Detalle_Procedimiento}' WHERE id = '$detalle_odontograma_id' limit 1");
    $AfectadasUp = mysqli_affected_rows($conn3);
    if ($AfectadasUp > 0) {
        $Arreglo["Estado"] = "Actualizado";
    } else {
    ...
    $Tabla = "ODP_OdontogramaEvolucion";
    $Tabla2 = "ODP_OdontogramaEvolucionImagenes";
    ...
    foreach ($_FILES["files_evolucion"]["tmp_name"] as $index => $file) {
        ...
        $dirBase = "ODP_ImagenesEvoluciones/";
