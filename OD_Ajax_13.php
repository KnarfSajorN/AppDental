...
function ActualizarDetallesOdontograma($id_detalle){
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE id = '$id' limit 1");
    ...
    //consultar el campo d$Numero_diente y r$Numero_diente de la tabla OD_OdontogramaMaster
    $queryOdontogramaDetalle = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMaster WHERE cliente_id = '$cliente_id' limit 1");
    ...
    $QueryDetalleOdonto = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE  Numero_Diente = '$Numero_Diente' AND Posicion = 'Toda La Pieza' AND cliente_id = '$cliente_id' AND Activo = 1 ORDER BY id DESC limit 1");
    ...
    //Aqui se busca si existe el procedimiento en la tabla OD_OdontogramaMasterDetalle por cada cara y que sea el ultimo procedimiento activo   
    $ArregloPosiciones = array("RecuadroSuperior", "RecuadroIzquierdo", "RecuadroInferior", "RecuadroDerecha", "RecuadroCentro", "RecuadroSeparadoSuperior","RecuadroSeparadoInferior");
    foreach ($ArregloPosiciones as $key_Posiciones => $value_Posiciones) {

        $QueryDetalleOdonto = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE  Numero_Diente = '$Numero_Diente' AND Posicion = '$value_Posiciones' AND cliente_id = '$cliente_id' AND Activo = 1 AND id>'$id_TodaLaPieza' ORDER BY id DESC limit 1");
        $rowDetalleOdonto = mysqli_num_rows($QueryDetalleOdonto);
        if ($rowDetalleOdonto > 0) {
    ...
    //actualizar el campo d$Numero_diente y r$Numero_diente de la tabla OD_OdontogramaMaster
    $queryOdontogramaDetalle = mysqli_query($conn3, "UPDATE OD_OdontogramaMaster SET d$Numero_Diente = '".json_encode($ArregloD_Array)."' WHERE id = '$idOdontogramaMaster' limit 1");
    $Afectadas[1] = mysqli_affected_rows($conn3);
    //echo de la consulta
    //echo "UPDATE OD_OdontogramaMaster SET d$Numero_Diente = '".json_encode($ArregloD_Array)."' WHERE id = '$idOdontogramaMaster' limit 1";
    $queryOdontogramaMaster = mysqli_query($conn3, "UPDATE OD_OdontogramaMaster SET r$Numero_Diente = '".json_encode($ArregloR_Array)."' WHERE id = '$idOdontogramaMaster' limit 1");
    $Afectadas[2] = mysqli_affected_rows($conn3);
    //echo de la consulta
    //echo "UPDATE OD_OdontogramaMaster SET r$Numero_Diente = '".json_encode($ArregloR_Array)."' WHERE id = '$idOdontogramaMaster' limit 1";
...
function PresupuestarDetalle($detalle_id_odontograma,$inventario){
    ...
    if ($nrowtabla == 0) {
        ...
        `detalle_odontograma_id` text NULL DEFAULT '' COMMENT 'id de la tabla OD_OdontogramaMasterDetalle',
...
if ($_POST["Tipo_Consulta"] == "Agregar Dato Odontograma" OR $_POST["Tipo_Consulta"] == "Agregar Dato Odontograma Inicial") {
    ...
    $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'OD_OdontogramaMaster'");
    $nrowtabla = mysqli_num_rows($tabla);
    if ($nrowtabla == 0) {

        $query = "CREATE TABLE `OD_OdontogramaMaster` (
        `id` int(11) NOT NULL,
        `Fecha` date DEFAULT NULL,
        `Hora` time NOT NULL,
        `cliente_id` int(11) DEFAULT NULL,
        ...
        if (!$creaciontabla) {
            ...
            mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMaster` ADD PRIMARY KEY (`id`);");
            mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMaster` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;");
        }
    }
    ...
    $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'OD_OdontogramaMasterDetalle'");
    $nrowtabla = mysqli_num_rows($tabla);
    if ($nrowtabla == 0) {

        $query = "CREATE TABLE `OD_OdontogramaMasterDetalle` (
        `id` int(11) NOT NULL,
        `Fecha` date DEFAULT NULL,
        `Hora` time NOT NULL,
        `cliente_id` int(11) DEFAULT NULL,
        ...
        if (!$creaciontabla) {
            ...
            mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` ADD PRIMARY KEY (`id`);");
            mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;");
        }
    }

    //Tema de facturacion
    $Campo1 = mysqli_query($conn3, "show COLUMNS from OD_OdontogramaMasterDetalle WHERE Field = 'detalle_id_factura';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` ADD `detalle_id_factura` TEXT NULL DEFAULT '0' COMMENT 'si es 0 es que no esta facturado  el detalle, si es otro numero sera el id de sdetalleoper *Creado desde modulo de OD_Ajax*'");
    }
    //NuevosCampos

    $Campo1 = mysqli_query($conn3, "show COLUMNS from OD_OdontogramaMasterDetalle WHERE Field = 'inventario_id';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` ADD `inventario_id` TEXT NULL DEFAULT '0' COMMENT 'si es diferente de 0 es que adjuntaron un tratamiento con un inventario *Creado desde modulo de OD_Ajax*'");
    }
    $Campo1 = mysqli_query($conn3, "show COLUMNS from OD_OdontogramaMasterDetalle WHERE Field = 'cie_10';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` ADD `cie_10` TEXT NULL DEFAULT '0' COMMENT 'cie-10 de la tabla OD_Cie10 *Creado desde modulo de OD_Ajax*'");
    }
    //////////////////////
...
if ($_POST["Tipo_Consulta"] == "Agregar Dato Odontograma") {
    ...
    $QueryOdontograma = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMaster WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' ");
    $NumRowOdontograma = mysqli_num_rows($QueryOdontograma);

    if($NumRowOdontograma==0){
        mysqli_query($conn3, "INSERT INTO OD_OdontogramaMaster(usuario_id, cliente_id, Fecha, Hora ) 
    ...
    //Insertar
    mysqli_query($conn3, "INSERT INTO OD_OdontogramaMasterDetalle (usuario_id, cliente_id,     Fecha, Hora,      Numero_Diente, Detalle,   Procedimiento,    Posicion, NombreCara, inventario_id, cie_10) 
    ...
    $queryList = mysqli_query($conn3, "UPDATE OD_OdontogramaMaster SET d{$Numero_Diente}='$ADientes', r{$Numero_Diente}='$RDientes' WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' limit 1;");

    //echo "UPDATE OD_OdontogramaMaster SET d{$Numero_Diente}='$ADientes', r{$Numero_Diente}='$RDientes' WHERE cliente_id = '{$cliente_id}' AND usuario_id = '$usuario_id' limit 1;";
    ...
    $diente_id = $_POST["diente_id"];
        ...
        while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
            ...
            foreach (json_decode($ArregloD) as $key => $value) {
                ...
                $Activo = funcionMaster($ArregloR[$key],'id','Activo','OD_OdontogramaMasterDetalle');
...
elseif($_POST["Tipo_Consulta"] == "Cargar Imagen Diente")
{
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Numero_Diente='$diente_id' AND Activo = 1 ORDER BY id DESC LIMIT 1 ");
    while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
        $Procedimiento = $RowOdontogramaMaster["Procedimiento"];
    }
    ...
    if($Procedimiento=="17"){
        switch ($diente_id) {
            case '55':case '54':case '53':case '52':case '51':case '61':case '62':case '63':case '64':case '65':
            case '85':case '84':case '83':case '82':case '81':case '71':case '72':case '73':case '74':case '75':
...
elseif($_POST["Tipo_Consulta"] == "Cargar Historial Diente")
{
    ...
    echo "<table class='table table-bordered table-striped' style='text-align-last: center;'>
        ...
        <tbody>";
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Numero_Diente='$diente_id' AND Activo = 1 ORDER BY id DESC");
    while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
...
elseif($_POST["Tipo_Consulta"] == "Cargar Historial Paciente")
{
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Activo=1 ORDER BY id DESC");
    while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
...
elseif($_POST["Tipo_Consulta"] == "Eliminar Detalle Odontograma")
{
    ...
    $QueryOdontogramaMaster = mysqli_query($conn3, "UPDATE OD_OdontogramaMasterDetalle SET Activo = 0 WHERE id = '$id' limit 1");
...
elseif($_POST["Tipo_Consulta"] == "Guardar Edicion Informacion Detalle Odontograma"){
    ...
    $queryOdontogramaMaster = mysqli_query($conn3, "UPDATE OD_OdontogramaMasterDetalle SET Detalle = '{$Detalle}',Procedimiento = '{$Procedimiento}',Posicion = '{$Posicion}',NombreCara = '{$NombreCara}',inventario_id='{$inventario_id}', cie_10='{$cie_10}' WHERE id = '$id_detalle' limit 1") or die (mysqli_error($conn3));
    //FacturarDetalle($id_detalle);
    $AfectadasUp = mysqli_affected_rows($conn3);

    if($AfectadasUp > 0){
        $queryOdontogramaMaster = mysqli_query($conn3, "UPDATE OD_OdontogramaMasterDetalle SET Fecha='{$Fecha}', Hora='{$Hora}' WHERE id = '$id_detalle' limit 1");
        $Afectadas = ActualizarDetallesOdontograma($id_detalle);
        $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE id = '$id_detalle' limit 1");
        $RowOdontogramaMaster = mysqli_fetch_array($QueryOdontogramaMaster);
...
elseif($_POST["Tipo_Consulta"] == "Actualizar Estado Procedimiento"){
    ...
    $Campo1 = mysqli_query($conn3, "show COLUMNS from OD_OdontogramaMasterDetalle WHERE Field = 'Estado_Procedimiento';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` ADD `Estado_Procedimiento` TEXT NULL DEFAULT 'Registrado' COMMENT 'Estado del Odontograma*Creado desde modulo de OD_Ajax*'");
    }
    $Campo1 = mysqli_query($conn3, "show COLUMNS from OD_OdontogramaMasterDetalle WHERE Field = 'Estado_Procedimiento_Detalle';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `OD_OdontogramaMasterDetalle` ADD `Estado_Procedimiento_Detalle` TEXT NULL DEFAULT '' COMMENT 'Detalle del estado del odontograma *Creado desde modulo de OD_Ajax*'");
    }
    ...
    $queryPresupuestoTratamiento = mysqli_query($conn3, "UPDATE OD_OdontogramaMasterDetalle SET Estado_Procedimiento='{$Estado_Procedimiento}', Estado_Procedimiento_Detalle='{$Detalle_Procedimiento}' WHERE id = '$detalle_odontograma_id' limit 1");
    $AfectadasUp = mysqli_affected_rows($conn3);
    if($AfectadasUp > 0){
        $Arreglo["Estado"]="Actualizado";
