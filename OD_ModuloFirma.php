...
if($_POST["Tipo_Consulta"] == "Agregar Firma"){
  ...
  $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE id = '$Tabla_id'");
  while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
    $cliente_id = $RowOdontogramaMaster["cliente_id"];
  }

   
  $QueryRealizado = mysqli_query($conn3,"INSERT INTO OD_FirmaDetalle (Fecha,Hora,usuario_id,cliente_id,Firma,Firma_Nombre,Firma_Documento,detalle_id)
...
$QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM OD_OdontogramaMasterDetalle WHERE id = '$detalle_id'");
while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
...
$queryList=mysqli_query($conn3,"SELECT Firma FROM OD_FirmaDetalle where Firma != '' AND detalle_id = '$detalle_id'");
