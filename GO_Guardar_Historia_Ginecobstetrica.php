...
$historia_vacunacion = '<table class="table table-bordered" width="100%"><tr th aling="center"> <th colspan="6"><B> <H4 aling="center">Historial de Vacunas</H4></B></th></tr>';
...
if ($_POST['niño2']) {
	...
	if ($_POST['niño2'] == 1) {
		$textEscalaTanner = '<b>Estado 1</b>. Sin vello púbico, Testículos y Pene infantil.';
	} else if ($_POST['niño2'] == 2) {
		$textEscalaTanner = '<b>Estado 2</b>. Aumento del escroto y testículos, piel del escroto enrojecida y arrugada, pene infantil vello púbico escaso en la base del pene.';
...
if ($_POST['niña2']) {
	...
	if ($_POST['niña2'] == 1) {
		$textEscalaTanner = '<b>Estado 1</b>. Pecho infantil, no vello púbico.';
	} else if ($_POST['niña2'] == 2) {
		$textEscalaTanner = '<b>Estado 2</b>. Botón mamario, vello púbico no rizado escaso, en labios mayores.';
	} else if ($_POST['niña2'] == 3) {
		$textEscalaTanner = '<b>Estado 3</b>. Aumento y elevación de pecho y areola. Vello rizado, basto y oscuro sobre pubis.';
	} else if ($_POST['niña2'] == 4) {
		$textEscalaTanner = '<b>Estado 4</b>. Areola y pezón sobreelevado sobre mama. Vello púbico tipo adulto no sobre muslos.';
	} else if ($_POST['niña2'] == 5) {
		$textEscalaTanner = '<b>Estado 5</b>. Pecho Adulto, areola no sobreelevada. Vello adulto zona medial muslo.';
	} else {
	}
}
...
mysqli_query($conn3, "INSERT INTO  Historia_Clinica_Ginecobstetrica (fecha, cliente_id, usuario_id, InformacionAcudiente,    EnfermedadActual, Checks_Antecedentes, AntecentesGinecobstetricos , AntecedentesFamiliares, Checks_Revision, SignosVitales, Paraclinicos, Imagenologia_Examen, Laboratorio_Examenes, ExamenFisico, OrganoSentidos, SintomasGenerales, DiagnosticoAcupuntura, DiagnosticoConsulta, Impresion, PlanManejo, Incapacidades, Insumos, RecetaId,imgEscalaTanner,textEscalaTanner, historialvacu) VALUES  
('$fecha', '$clienteId', '$idusuario', '$InformacionAcudiente',  '$EnfermedadActual', '$Checks_Antecedentes', '$AntecentesGinecobstetricos', '$AntecedentesFamiliares',  '$Checks_Revision', '$SignosVitales', '$Paraclinicos','$Imagenologia_Examen', '$Laboratorio_Examenes','$ExamenFisico','$OrganoSentidos', '$SintomasGenerales', '$DiagnosticoAcupuntura','$DiagnosticoConsulta', '$Impresion', '$PlanManejo', '$Incapacidades','$Insumos','$RecetaId', '$imgEscalaTanner', '$textEscalaTanner', '$historialvacu');");
...
if ($_POST['SignosVitales']["Peso Corporal"] <> "" or $_POST['SignosVitales']["Altura"] <> "" or $_POST['SignosVitales']["IMC"] <> "" or $_POST['SignosVitales']["Perimetro_Cefalico"] <> "") {
	...
	mysqli_query($conn3, "INSERT INTO  Grafica_Crecimiento (usuario_id,cliente_id,Peso, Altura, IMC, Perimetro_Cefalico, Edad, id_historia, Tipo_Historia) VALUES  ('$idusuario','$clienteId',{$informacion_grafica},'Historia Clinica Ginecoobstetrica');");
...
// Para los RIPS
$RIP_Nombre_Historia = "Historia_Clinica_Ginecobstetrica";
$RIP_historia_id = $id_historia;
include 'IR_GuardarRips.php';

// Para el Recetario
$Recetario_Nombre_Historia = "Historia_Clinica_Ginecobstetrica";
...
$Campo1 = mysqli_query($conn3, "show COLUMNS from historiaClinica9_Quirurgico_Cie10 WHERE Field = 'Tipo_Historia';");
$nrowCampo1 = mysqli_num_rows($Campo1);
if ($nrowCampo1 == "0") {
    mysqli_query($conn3, "ALTER TABLE `historiaClinica9_Quirurgico_Cie10` ADD `Tipo_Historia` TEXT NULL  COMMENT 'de que historia se realizo *Creado desde modulo de Guardar Historia Ginecobstetrica*'");
}

//registro de cie10
foreach ($_POST['DiagnosticoConsulta']['CIE10'] as $value) {
	$codigocie10 = $value;
	mysqli_query($conn3, "INSERT INTO historiaClinica9_Quirurgico_Cie10 (cliente_id,usuario_id,historiaClinica9_id,Fecha,Hora,codigo,Tipo_Historia) VALUES ('$clienteId', '$idusuario', '$id_historia','$fecha_dias','$fecha_horas','$codigocie10','Historia_Clinica_Ginecobstetrica')");
}

$Campo1 = mysqli_query($conn3, "show COLUMNS from examenesaRealizar WHERE Field = 'Tipo_Historia';");
$nrowCampo1 = mysqli_num_rows($Campo1);
if ($nrowCampo1 == "0") {
    mysqli_query($conn3, "ALTER TABLE `examenesaRealizar` ADD `Tipo_Historia` TEXT NULL  COMMENT 'de que historia se realizo *Creado desde modulo de Guardar Historia Ginecobstetrica*'");
}
...
mysqli_query($conn3, "INSERT INTO examenesaRealizar (usuario_id,cliente_id,historia_id,laboratorio,ecografia,otros,fechaHora,Tipo_Historia) 
    VALUES 
    ('$idusuario','$clienteId','$id_historia', '$Laboratorio_Examenes', '$Imagenologia_Examen', '$otros', '$fecha','Historia_Clinica_Ginecobstetrica');");


echo "<script language='Javascript'> window.location='GO_Finalizado_Historia_Ginecobstetrica?historiaClinica1=$id_historia';</script>";
