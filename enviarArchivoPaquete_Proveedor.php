...
$mensaje = '
                <html>
                <head>
                  <title>Documentos del Proveedor </title>
                    <body> 
                    
                               Hola buen dia ' . $nombreproveedor . ', enviamos los siguientes archivos :<br><br>';


$queryImg = mysqli_query($conn3, "SELECT * FROM archivos_proveedor  where proveedor_id = '$proveedor_id' and  descripcion='$archivo' AND estado = 1");
...
while ($resulImg = mysqli_fetch_array($queryImg)) {
  ...
  $mensaje .=
    'Ver archivo:  '.$Base.'/archivos_proveedor/' . $resulImg['codigo'] . ' .<br>
...
$queryImg = mysqli_query($conn3, "SELECT * FROM archivos_proveedor  where proveedor_id = '$proveedor_id' and  descripcion='$archivo' AND estado = 1");
//echo "SELECT * FROM archivos  where cliente_id = '$clienteId' and id='$id'";
$nrowlER = mysqli_num_rows($queryImg);
while ($resulImg = mysqli_fetch_array($queryImg)) {
  $Producto              = $resulImg['codigo'];
  ...
  $mensajeW .=
    'Ver archivo:  '.$Base.'/archivos_proveedor/' . $Producto . ' 
