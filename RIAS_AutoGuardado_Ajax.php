...
if (isset($_POST["Tipo_Consulta"]) && $_POST["Tipo_Consulta"]=="Guardar Datos Historia Clinica") {
    ...
    //Aquí creamos la tabla del RIAS_AutoGuardado 
    $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'RIAS_AutoGuardado'");
    $nrowtabla = mysqli_num_rows($tabla);
    if ($nrowtabla == 0) {
        $query = "CREATE TABLE `RIAS_AutoGuardado` ( 
            ...
            `JsonDataEscala` TEXT NULL DEFAULT '' ,
    ...
    */
    $valorCampo_jsonDataEscala = mysqli_real_escape_string($conn3,$_POST['valorCampo_jsonDataEscala']);
    ...
    //$Arreglo_Familiares = json_encode($Arreglo_Familiares);
     ...
     //Aquí se valida si el RIAS_AutoGuardado existe
    $QueryAutoGuardado = mysqli_query($conn3, "SELECT * FROM  RIAS_AutoGuardado where cliente_id = '$cliente_id' and usuario_id = '$usuario_id' and Ruta = '$Ruta' AND Estado = 1");
    $NrowAutoGuardado = mysqli_num_rows($QueryAutoGuardado);

    $Fecha = date("Y-m-d H:i:s");
    if($NrowAutoGuardado>0){
        //$query = "INSERT INTO RIAS_AutoGuardado (Campos,Ruta,usuario_id,cliente_id) VALUES ('$Campos','$Ruta','$usuario_id','$cliente_id')";
        //hacer un update con la consulta anterior

        //si se existe se actualizan los campos
        $query = "UPDATE RIAS_AutoGuardado SET Campos='$Campos', JsonDataEscala='$valorCampo_jsonDataEscala' ,Arreglo_Paraclinicos='$Arreglo_Paraclinicos', Arreglo_Incapacidades='$Arreglo_Incapacidades', Arreglo_Insumos='$Arreglo_Insumos', Arreglo_Familiares='$Arreglo_Familiares', Fecha_Actualizacion='$Fecha' WHERE cliente_id = '$cliente_id' and usuario_id = '$usuario_id' and Ruta = '$Ruta' and Estado = 1 LIMIT 1";
        $Arreglo["Tipo"]="Update";
    }else{

        //Si no existes se crean los campos
        $query = "INSERT INTO RIAS_AutoGuardado (Campos,JsonDataEscala,Arreglo_Paraclinicos,Arreglo_Incapacidades,Arreglo_Insumos,Arreglo_Familiares,Ruta,Nombre_Tabla,usuario_id,cliente_id,Fecha,Fecha_Actualizacion) 
        VALUES ('$Campos','$Arreglo_Paraclinicos','$valorCampo_jsonDataEscala','$Arreglo_Incapacidades','$Arreglo_Insumos','$Arreglo_Familiares','$Ruta','$nombre_tabla','$usuario_id','$cliente_id','$Fecha','$Fecha')";
        $Arreglo["Tipo"]="Insert";
    }
    
    $resultado = mysqli_query($conn3, $query);

    //Aquí se enviar el arreglo con el estado si fue correcto o incorrecto el RIAS_AutoGuardado
    if (!$resultado) {
        $Arreglo["Estado"]="Fail";
...
elseif (isset($_POST["Tipo_Consulta"]) && $_POST["Tipo_Consulta"]=="Consultar Estado AutoGuardado") {
    ...
    $cliente_id = $_POST['cliente_id'];
     ...
     //Consulta si existe el RIAS_AutoGuardado
    $QueryAutoGuardado = mysqli_query($conn3, "SELECT * FROM  RIAS_AutoGuardado where cliente_id = '$cliente_id' and usuario_id = '$usuario_id' and Ruta = '$Ruta' AND Estado = 1");
    $NrowAutoGuardado = mysqli_num_rows($QueryAutoGuardado);
...
elseif (isset($_POST["Tipo_Consulta"]) && $_POST["Tipo_Consulta"]=="Cargar AutoGuardado") {
    ...
    while ($row_recordset32 = mysqli_fetch_array($QueryAutoGuardado)) {
        ...
        $valorCampo_jsonDataEscala = $row_recordset32['JsonDataEscala'];
    }
    ...
    if($NrowAutoGuardado>0){
        ...
        $Arreglo["valorCampo_jsonDataEscala"] = ($valorCampo_jsonDataEscala);
    }else{
        $Arreglo["Estado"]="No Existe";
    }
...
elseif (isset($_POST["Tipo_Consulta"]) && $_POST["Tipo_Consulta"]=="Cargar Estado Error Autoguardado") {
    ...
    //aqui se actualiza el RIAS_AutoGuardado ya que dio error y para generar un nuevo RIAS_AutoGuardado este debera pasarse a estado 2
    
    $query = "UPDATE RIAS_AutoGuardado SET Estado='2' WHERE cliente_id = '$cliente_id' and usuario_id = '$usuario_id' and Ruta = '$Ruta'";
    $resultado = mysqli_query($conn3, $query);
