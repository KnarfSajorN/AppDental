...
if (isset($_POST['GuardarDetalleEntradaActivos'])) {
  ...
  /////////////////////////////////////////////////////////////////// creacion de la tabla para los detalles Activos //////////////////////////////////////////////////////////////////////////
  $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'Activos_Detalles_Temp'");
  $nrowtabla = mysqli_num_rows($tabla);
  if ($nrowtabla == 0) {
      $query = "CREATE TABLE `Activos_Detalles_Temp` ( 
          ...
          `Tipo` INT(11) NULL DEFAULT '0' COMMENT '1 Carga , 2 Descarga, 3 Asignacion',
          `Activo` INT(11) NULL DEFAULT '1' COMMENT '1 Activo, 0 Inactivo, 2 Procesado',
          PRIMARY KEY (`id`)) ENGINE = MyISAM;";
          $creaciontabla = mysqli_query($conn3, $query);
  }

  $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'Activos_Detalles'");
  $nrowtabla = mysqli_num_rows($tabla);
  if ($nrowtabla == 0) {
      $query = "CREATE TABLE `Activos_Detalles` ( 
          `id` INT(11) NOT NULL AUTO_INCREMENT , 
          `Fecha` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ,
          `idOperacion` INT(11) NULL DEFAULT '0' , 
          `usuario_id` INT(11) NULL DEFAULT '0' ,
  ...
  $TipoOperacion = "1"; //Operacion Carga
...
$queryList = mysqli_query($conn3, "INSERT INTO Activos_Detalles_Temp (usuario_id, idProducto, Descripcion, Cantidad, Costo, Precio, Total, SinvDep_id, Deposito_id, Tipo) 
  ...
  if ($queryList != true) {
    echo "<script language='Javascript'> window.location='IN_ActivosCargar.php'</script>";
  } else {
    echo "<script language='Javascript'> window.location='IN_ActivosCargar.php'</script>";
  }
}
...
if (isset($_POST['GuardarOperacionCargarActivo'])) {
  

  /////////////////////////////////////////////////////////////////// creacion de la tabla para los detalles Activos //////////////////////////////////////////////////////////////////////////
  $tabla = mysqli_query($conn3, "SHOW TABLES LIKE 'Activos_Operaciones'");
  $nrowtabla = mysqli_num_rows($tabla);
  if ($nrowtabla == 0) {
      $query = "CREATE TABLE `Activos_Operaciones` ( 
          `idOperacion` INT(11) NOT NULL AUTO_INCREMENT , 
          `Fecha` DATE NULL DEFAULT CURRENT_TIMESTAMP , 
          `usuario_id` INT(11)  NOT NULL,
          `proveedor_id` INT(11) NULL DEFAULT '0' ,
          `NumeroOperacion` TEXT NULL , 
          
          `Total_Cantidad` INT(11) NULL DEFAULT '0' , 
  ...
  $Campo1 = mysqli_query($conn3, "show COLUMNS from usuarios WHERE Field = 'NumeroCargaActivos';");
  $nrowCampo1 = mysqli_num_rows($Campo1);if ($nrowCampo1 == "0") {mysqli_query($conn3, "ALTER TABLE `usuarios` ADD `NumeroCargaActivos` INT(11) NULL DEFAULT '0' COMMENT 'Numeracion Carga Activos *Creado desde modulo de Totalizar Activos IN_ActivosCargar*'");}
  ...
  while ($RowUsuario = mysqli_fetch_array($QueryUsuario)) {
      $NumeroCargaActivos      = $RowUsuario['NumeroCargaActivos'];
  }

  $TipoOperacion = "1"; //1-> Carga de Activos 2-> Descarga de Activos
  ...
  mysqli_query($conn3, "INSERT INTO Activos_Operaciones (usuario_id, proveedor_id, NumeroOperacion, Total_Cantidad, Total_Costo, Total_Precio, Motivo, Tipo) 
                                       VALUES ('$usuario_id', '$proveedor_id', '$NumeroCargaActivos', '$Total_Cantidad', '0', '0', '$Motivo', '$TipoOperacion')");

  $idOperacion = mysqli_insert_id($conn3);

  mysqli_query($conn3, "UPDATE usuarios SET NumeroCargaActivos = $NumeroCargaActivos WHERE ID = '$usuario_id'");
  ...
  while ($RowDetallesTemp = mysqli_fetch_array($QueryDetallesTemp)) {
    ...
    $Detalle_id = mysqli_insert_id($conn3);
      ...
      $idProducto = $RowDetallesTemp['idProducto'];
      $Seriales = $RowDetallesTemp['Seriales'];
      $fechaRegistro = date("Y-m-d H:i:s");
      $Usuario_id1 = $RowDetallesTemp['usuario_id'];
      ...
      $Tipo_Serial = "2"; //1-> orden de compras | 2-> Cargar | 3-> Descargar 
      foreach (json_decode($Seriales,true) as $key => $value) {
        $Serial = $value;

        mysqli_query($conn3, "INSERT INTO SinvSerial (Fecha, usuario_id, Serial, Caracter, idSinvetrios, idSinvDep,Detalle_id,Tipo_Detalle) VALUES ('$fechaRegistro', '$Usuario_id1', '$Serial', '$Caracter', '$idProducto', '$SinvDep_id','$Detalle_id','$Tipo_Serial')");
...
echo "<script language='Javascript'> window.location='IN_ImprimirActivosCargar.php?id={$idOperacion}';</script>";
...
//1 Carga , 2 Descarga
$resultado = mysqli_query($conn3, "SELECT * FROM  Activos_Detalles_Temp where Activo = 1 and  usuario_id = $ID_Usuario AND Tipo = '1' order by id");
                    ...
                    <div class="card-body">
                      ...
                      <div class="tab-content">
                        ...
                        <!-- nuevo -->
                          ...
                          <div class="col-md-12">
                            <table class="table table-striped table-bordered">
                              ...
                              <tbody>
                                ...
                                //1 Carga , 2 Descarga
                                $Contador=0;
                                $queryOpera = "SELECT * FROM  Activos_Detalles_Temp where Activo = 1 and  usuario_id = $ID_Usuario AND Tipo = '1' order by id";
                          ...
                          <form id="TotalizarCargarActivo" method="post" action="IN_ActivosCargar.php"  class="form-group row" onsubmit="GuardarCargarActivo();">


                            <input type="hidden" name="datos[usuario_id]" value="<?= $_SESSION['ID'] ?>">
                            <input type="hidden" name="GuardarOperacionCargarActivo" value="1">
                            <div class="col-md-12">
                          ...
                          <div class="row">
                            <div class="col-md-12">
                              <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                  <?php
                                  $queryOpera = "SELECT * from Activos_Operaciones where Tipo = '1'and usuario_id = '{$_SESSION['ID']}' order by idOperacion DESC";
                                  $resultOpera = mysqli_query($conn3, $queryOpera);
                                  while ($rowOpera = mysqli_fetch_array($resultOpera)) {
                                  ...
                                  ?>
                                    <tr>
                                      ...
                                      <td>
                                        <a href='IN_ImprimirActivosCargar.php?id=<?=$rowOpera['idOperacion'];?>' class='btn btn-block btn-outline-info btn-lg rounded-pill shadow' style='width: 100%;'><i class='fa fa-print' title='Imprimir'> Imprimir</i></a>
                                      </td>
                                    </tr>
                                  <?php
                                  }
...
include 'IncludeModalSerialesActivosCargar.php';
?>
...
<script>
function GuardarCargarActivo(){
  event.preventDefault(); 

  tokenMaster('<?= $ttext ?>', '<?= $tuser ?>', '<?= $tnumeros ?>').then((isTokenConfirmed) => {
                    if (isTokenConfirmed) {
                        //console.log('paso');
                        document.getElementById('TotalizarCargarActivo').submit();
                    } else {
                        //console.log('no paso');
                    }
                });
