    ...
    <div class="row">
        <div class="col-xs-12">
            <h2 class="page-header">
                ...
                <small class="pull-right"> Fechar: <?php echo date("d-m-y") ?></small>
            </h2>
        </div>
        ...
        <div class="col-xs-12 table-responsive">
            <table class="table table-striped" border="1">
                ...
                <tbody>
                    ...
                    // $queryListC = mysqli_query($conn3, "SELECT * from cliente where Fechar BETWEEN '$desde' and '$hasta' $andtipo  ");
                    $queryListC = mysqli_query($conn3, "SELECT * FROM cliente WHERE  DATE(Fechar) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) $queryEPS GROUP BY cliente_id ");
                    ...
                    while ($rowC = mysqli_fetch_array($queryListC)) {
                        ...
                        $FecharNacimiento = funcionMaster($cliente_id, 'cliente_id', 'FecharNacimiento', 'cliente');
                        $Eps = funcionMaster($cliente_id, 'cliente_id', 'entidad_id', 'cliente');
                        if ($genero == 'M') {
                            $genero = "Masculino";
                        }
                        if ($genero == 'F') {
                        ...
                        <td width="25%">' . funcionMaster1($Eps,'id','Nombre', 'Rips_Entidades') . ' </td>
...
$queryList1 = mysqli_query($conn3, "SELECT COUNT(DISTINCT(cliente_id)) AS total,entidad_id AS Eps FROM cliente  WHERE DATE(Fechar) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) group by entidad_id");
...
// $queryList2 = mysqli_query($conn3, "SELECT FROM cliente WHERE  DATE(Fecharr) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE)");

// $queryList3 = mysqli_query($conn3, "SELECT FROM cliente WHERE  DATE(Fecharr) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE)");

// $queryList4 = mysqli_query($conn3, "SELECT FROM cliente WHERE  DATE(Fecharr) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE)");

// $queryList5 = mysqli_query($conn3, "SELECT FROM cliente WHERE DATE(Fecharr) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE)");
?>
    ...
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ...
            while ($row1 = mysqli_fetch_array($queryList1)) {
                // $count1 += $row1["total"];
                // echo "['" . $genero . "', " . $row["total"] . "],";
                if ($row1['Eps'] != '' && $row1['Eps'] > 0) {
                    echo "[' " . funcionMaster1($row1["Eps"], 'id', 'Nombre', 'Rips_Entidades') . "', " . $row1["total"] . "],";
                }
            }
