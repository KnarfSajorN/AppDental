...
//SELECT SUM(totalBruto) as totalBruto, sum(montoPagado) as montoPagado, idCliente FROM `sOperacionInv`GROUP BY (idCliente)
                //    $resultado=mysql_query("SELECT * FROM  sOperacionInv where idEmpresa =$ID order by numeroDoc");
                    ...
                    $resultado=mysql_query("SELECT SUM(totalBruto) as totalBruto, sum(montoPagado) as montoPagado, idCliente FROM `sOperacionInv` where tipo = 1 GROUP BY (idCliente)");
...
if ($saldo > 0) {
                  ...
                  <td>    
                      ...
                      <a href="SclienteAdministracion_cuentasAcobrarCliente.php?idcliente='.$fila[2].'"><i class="fa fa-eye"></i> Ver</a>     
...
$queryList = mysqli_query($conn3, "SELECT * FROM sCuentasCobrar where montoPendiente <> 0 and idUsuario = $ID");
while ($rowMotorizado = mysqli_fetch_array($queryList)) {
    $numeroDoc = $rowMotorizado['numeroDoc'];
    $cliente_id = $rowMotorizado['idEmpresa']; // este campo se usa para el cliente id
    $idOperacion = $rowMotorizado['idDocumento']; 
...
$montoPendiente = $montoBase - $montoPagado;
    $nombre = funcionMaster($cliente_id,'cliente_id','nombre_cliente','cliente');
    $telefono = funcionMaster($cliente_id,'cliente_id','whatsapp','cliente');

    
    echo "<tr>
            <td width='20%' align='left'>{$nombre}</td>
            <td width='20%' align='Right'>{$telefono}</td>
            <td width='10%' align='Right'>{$idOperacion}</td>
            <td width='10%' align='Right'>{$montoPagado}</td>
            <td width='10%' align='Right'>{$montoPendiente}</td>
            <td width='20%'> <a href='ActualizarPagoFactura.php?idOperacion={$idOperacion}' class='btn btn-primary' style='width:100%'><i class='fa fa-bill'></i> Registrar Pago Completo </a>  </td>
          </tr>";
}
?>
