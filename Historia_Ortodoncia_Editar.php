...
$id_historia = decrypt($_GET['id']);
$tabla = 'Historia_Ortodoncia';
$ID = $_SESSION['ID'];


$QueryDatos = "SELECT * FROM $tabla tp INNER JOIN tablaOrtodoncia tOr ON tOr.id = tp.id_tablaOrtodoncia where tp.id = $id_historia ";
$ResultDatos = mysqli_query($conn3, $QueryDatos) or die(mysqli_error($conn3));
$DatosHistoria = mysqli_fetch_assoc($ResultDatos); 
$clienteId = $DatosHistoria['cliente_id'];
$DatosHistoria = json_encode($DatosHistoria);
                    ...
                    <div class="box-body">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="box box-solid">
                                <!-- /.box-header -->
                                <form action="Guardar_Historia_Ortodoncia_Editada.php" method="POST"
...
<script>
    const datosHistoria = <?=$DatosHistoria?>;
     const reformatData = (data = datosHistoria) => {

        // COMO VIENEN
        let { InformacionAcudiente, EnfermedadActual, Checks_Antecedentes } = data;
        let { DiagnosticoConsulta, Impresion, PlanManejo, Checks_Revision } = data;
        let { DiagnosticoAcupuntura, ExamenFisico, OrganoSentidos } = data;
        
        // SEPARANDO COMO ARRAY
        InformacionAcudiente = InformacionAcudiente.split("<br>"); 
        EnfermedadActual     = EnfermedadActual.split("<br>"); 
        Checks_Antecedentes  = Checks_Antecedentes.split("<br>"); 
        DiagnosticoConsulta  = DiagnosticoConsulta.split("<br>"); 
        Impresion            = Impresion.split("<br>"); 
        PlanManejo           = PlanManejo.split("<br>"); 
        Checks_Revision      = Checks_Revision.split("<br>"); 
        OrganoSentidos      = OrganoSentidos.split("<br>"); 
        
        // AGRUPANDO EN KEY => VALUE
        const InfoAcudiente = InformacionAcudiente.reduce((acc, item) => {
        ...
        const EnfActual = EnfermedadActual.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        const ChecksAnt = Checks_Antecedentes.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        const DgConsulta = DiagnosticoConsulta.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        const Impr = Impresion.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        const PlanM = PlanManejo.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        const ChecksRev = Checks_Revision.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        const OrgSentidos = OrganoSentidos.reduce((acc, item) => {
            let [key, value] = item.trim().split(": ");
            key = key.trim();
            acc[key] = value;
            return acc;
        ...
        return {
            ...data,
            InformacionAcudiente: InfoAcudiente,
            EnfermedadActual: EnfActual,
            Checks_Antecedentes: ChecksAnt,
            DiagnosticoConsulta: DgConsulta,
            Impresion: Impr,
            PlanManejo: PlanM,
            Checks_Revision: ChecksRev,
            OrganoSentidos: OrgSentidos,
            DiagnosticoAcupuntura: {'Tiempo de Tratamiento' : DiagnosticoAcupuntura.replace("<br>", "")}, 
            ExamenFisico: {'Compromisos' : ExamenFisico.replace("Compromisos: ", "").replace("<br>", "")} 
        }
     }
     
     const dataClean = reformatData();
     ...
     $("#FormularioHistoriaClinica input, #FormularioHistoriaClinica textarea, #FormularioHistoriaClinica select").each(function() {
        ...
        if (name) {
            ...
            let nombreSinCierreCorchetes = name.replace("]", "");
            let nombreArray = nombreSinCierreCorchetes.split("[");
            
            let valorGuardado = "";

            if (nombreArray.length == 2) {
                valorGuardado = dataClean[nombreArray[0]][nombreArray[1]];
            }else if (nombreArray.length == 1) {
                valorGuardado = dataClean[nombreArray[0]];
            }
            ...
            //     console.log("value", valorGuardado);
            // }
