...
while ($rowMotorizado = mysqli_fetch_array($queryList)) {
  ...
  $tel              = $rowMotorizado['celular_cliente'];
              ...
              <table class="table table-bordered" style="font-size: 10px;">
                ...
                <tbody>
                  ...
                  while ($Lista = mysqli_fetch_array($queryList)) {
                    ...
                    $queryList1 = mysqli_query($conn3, "SELECT l_v.id as ListaVacuna,v_a.Fecha_Administracion as Fecha_A,v_a.Fecha_Proxima_Aplicacion as Fecha_P,v_a.Sitio_Anatomico as Sitio_A,v_a.Observacion as Observacion, v_a.Dosis_Aplicada as Dosis FROM vacunas_aplicadas as v_a , listado_vacunas as l_v  WHERE v_a.Id_Lista_Vacuna = l_v.id AND l_v.Id_Grupo='$numero_grupo' AND v_a.Id_Cliente = '$idcliente' ORDER BY v_a.id , v_a.Id_Lista_Vacuna, v_a.Dosis_Aplicada");
                    $nrowl = mysqli_num_rows($queryList1);
                    while ($Lista1 = mysqli_fetch_array($queryList1)) {
                      if ($contador == "0") {
                        echo '<tr><td colspan="6" style="background-color: beige;"><b>' . $nombre_grupo . '</b></td></tr>';
                      }
