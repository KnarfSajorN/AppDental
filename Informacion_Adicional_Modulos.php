<?php 
   ...
   if(isset($_POST['Actualizar_Informacion_Pagina']))
   {
      ...
      $ruta = htmlentities($_SERVER['PHP_SELF']);
      if($_GET["Usuario"]<>""){$Tabla.="&Usuario=".$Usuario;}
   ...
   if($_GET["Usuario"]<>"")
   {
     ...
     $rutaEnviar="?Tabla={$Tabla}&Usuario={$Usuario}";
   }
   else {
     $usuario_id = $_SESSION['ID'];
     $rutaEnviar="?Tabla={$Tabla}";
   }
    ...
    <section class="content"> 
      <div class="row">      
        ...
        <div class="content">
          <div class="box">
            <div class="box-body">
              <form action="<?php echo htmlentities($_SERVER['PHP_SELF'].$rutaEnviar); ?>" method="POST">
              ...
              <?php
                ...
                foreach ($Tablas_Base_Datos[$Tabla] as $key => $value) {
                    ...
                    switch ($Tipo_Campo) 
                    {
                        ...
                        case "date":
                        case "email":
                          echo "<input name=\"Arreglo[{$Nombre_Campo}]\" type=\"{$Tipo_Campo}\" class=\"form-control input-lg\" placeholder=\"{$key}\" value=\"{$Valor_Campo}\" {$Filtro}>";
                          break;
                        case "select":
                          echo "<select name=\"Arreglo[{$Nombre_Campo}]\" id=\"{$Nombre_Campo}\"  class=\"form-control input-lg\" style=\"width: 100%;\" {$Filtro} required>
                             ...
                             <thead>
                              ...
                              <tbody>
                                ...
                                if($_GET["Usuario"]<>"")
                                {
                                  ...
                                  $queryList=mysqli_query($conn3,"SELECT * FROM  {$Tabla} where Activo=1 ");
                                }
                                
                                while($rowMotorizado=mysqli_fetch_array($queryList))
                                {
                                  $id = $rowMotorizado[$Tablas_Base_Datos[$Tabla."_id"]];$datos="";
                                  foreach ($Tablas_Base_Datos[$Tabla] as $key => $value) {
                                  ...
                                  echo "<tr><th width='2%' scope='row'>{$id}</th>
                                         {$datos}
                                         <td width='20%' align='center'><font color='#04CC05'> <a href='{$ruta}?Tabla={$Tabla}&Editar={$id}' class='btn btn-primary' style='width: 200px;'><i class='fa fa-pencil' title='Editar'></i>Editar</a></font><br>
                                            <font> <a href='{$ruta}?Tabla={$Tabla}&Eliminar={$id}' class='btn btn-primary' style='width: 200px;background-color:#ef4259;margin-top:5px;margin-bottom:5px'> <i class='fa fa-close' title='Eliminar'> Eliminar</i></a></font>
...
<script type="text/javascript">
  function Select(idselect,nombretabla,idtabla,textotabla)
   {
       $(idselect).select2({
        ajax: { 
         url: "Informacion_Adicional_Modulos_Ajax.php",
         dataType: 'json',
         type: "POST",
         delay: 250,
         data: function (params) {
          return {
            ...
            NombreTablas:nombretabla,
            idTabla:idtabla,
            Texto:textotabla // search term
          };
         },
...
window.onload = function() {
  ...
  foreach ($Tablas_Base_Datos[$Tabla] as $key => $value) 
  {
    ...
    {
      $Funcion = str_replace("onchange=", "", $value["Filtro"]);
