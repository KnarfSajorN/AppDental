...
var Artyom = (function () {
    ...
    Artyom.prototype.debug = function (message, type) {
        // si message termina en . se quita
    ...
    Artyom.prototype.execute = function (voz) {
        ...
        // todo a minúscula
        voz = voz.toLowerCase();
        // si termina en . se quita
        if (voz.substr(voz.length - 1) === ".") {
            voz = voz.substr(0, voz.length - 1);
        }
        
        _this.ArtyomProperties.name = _this.ArtyomProperties.name ? _this.ArtyomProperties.name.toLowerCase() : null;
