...
//echo "SELECT * FROM  e_tratamiento1 where ID = $historiaClinica1";
...
if (isset($_GET['tipo'])) {
  ...
  $mensajeW = 'Sr(a) *' . $nombre_cliente . '* Se le envía el siguiente documento registrado, para visualizarlo ingresar en el siguiente link: ' . $Base . 'HC_ImprimirFichaClinica?iHC=' . encrypt($historiaClinica1) . '';
  $accion = 0;
...
if (isset($_POST['Enviar_Firma'])) {
  ...
  $mensajeW = 'Sr(a) *' . $nombre_cliente . '* Se le ha sugerido firmar esta consulta, para proceder abrir el siguiente link ' . $Base . 'firma/firmardocumento/' . $historiaClinica1 .'/e_tratamiento1/'. $cliente_id;
  $accion = 0;
  Whatsapp_sent_cliente($linkkey, $whatsapp, $mensajeW, $cliente_id, $usuario_id, $whatsapp, $accion);
...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <div align="center">
      ...
      <a class="btn btn-outline-info btn-lg rounded-pill shadow" href="<?php echo $Base; ?>HC_HistoriaFichaClinica?cI=<?php echo encrypt($usuario_id) ?> ">
        <i class="fa fa-heartbeat"></i> Nuevo Consulta
      </a>

      <a class="btn btn-outline-info btn-lg rounded-pill shadow" href="<?php echo $Base; ?>agregarCitas?cI=<?php echo encrypt($usuario_id) ?> ">
    ...
    <div align="center">
      ...
      <a class="btn btn-outline-info btn-lg rounded-pill shadow" target="_blank" href="<?php echo $Base; ?>HC_ImprimirFichaClinica?iHC=<?php echo encrypt($historiaClinica1); ?>">
        <i class="fa fa-print"></i> Imprimir Consulta
      </a>

      <?php 
        $botonesImprimir = [
          ['Consulta', base64_encode($Base.'HC_ImprimirFichaClinica_plantilla.php?iHC='.encrypt($historiaClinica1).'')],         
        ];
...
<a class="btn btn-outline-info btn-lg rounded-pill shadow" href="<?php echo $Base; ?>HC_FinalizadoFichaClinica?iHC=<?php echo encrypt($historiaClinica1); ?>&tipo=consulta">
...
<hr>
  ...
  <br>
      ...
      ?>
        <form action="HC_FinalizadoFichaClinica?iHC=<?php echo encrypt($historiaClinica1) ?>" method="POST" name="formularioEnvioExamen">
