...
<script>
    ...
    },
    {
        "data": function (row, type, set) {
            botones = "";
            botones += "<a href='cHistoria?cI=<?= salt() ?>" + btoa(row.cliente_id) + "&iCr=<?= encrypt($idHistoria) ?>&tipo=0' title='Agregar <?php echo $nombreH; ?>'><i class='fas fa-file-medical'></i> </a>";
            botones += "<a href='cVerPaciente?cI=<?= salt() ?>" + btoa(row.cliente_id) + "&iCr=<?= encrypt($idHistoria) ?>&tipo=0' title='Ver <?php echo $nombreH; ?>'><i class='fas fa-book-medical'></i> </a>";
            ...
            if ('<?= $name ?>' == 'historia_1_cirugiaplastica54165654312' || '<?= $name ?>' == 'historiaclinicadermatologia') {
                botones += "<a href='fotosPaciente?cI=<?= salt() ?>" + btoa(row.cliente_id) + "' title='Antes y después'><i class='far fa-images text-info'></i> </a>";
            }
            return botones;
        }
    }
