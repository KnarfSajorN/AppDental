...
$id_historia_registro = decrypt($_GET['iC']);
...
$Nombre_Tabla = $name;
$QueryDatosRegistro = "SELECT * FROM {$Nombre_Tabla} WHERE id = '$id_historia_registro'";
$ResultDatosRegistro = mysqli_query($conn3, $QueryDatosRegistro);
$rowDatosRegistro = mysqli_fetch_assoc($ResultDatosRegistro);
$rowDatosRegistro = json_encode($rowDatosRegistro);

$QueryDatosRips = "SELECT CIE10_1,CIE10_2,CIE10_3,CIE10_4 FROM Rips_Informacion WHERE Nombre_Historia = '{$Nombre_Tabla}' AND historia_id = '$id_historia_registro'";
$ResultDatosRips = mysqli_query($conn3, $QueryDatosRips);
$rowDatosRips = mysqli_fetch_assoc($ResultDatosRips);
if (!is_array($rowDatosRips)) {
    $rowDatosRips = [];
}
$rowDatosRips = json_encode($rowDatosRips);
                            ...
                            <div class=" box-solid">
                                ...
                                <div class="box-body">
                                    <div class="box-group" id="accordion1">
                                        ...
                                        <form action="configEditarFormulario.php" method="POST" name="<?php echo $name ?>"
                                        ...
                                        <div class="box-group" id="accordion1">
                                            ...
                                            $QDetalle = "SELECT * FROM  configTablaDetalle
                                                            where 1=1
                                                            and idTabla = $idHistoria
                                                            and estado = 1
                                                            order by convert(orden, signed) asc";

                                            $queryDetalle = mysqli_query($conn3, $QDetalle);
                        ...
                        <div class="col-sm-12">
                          ...
                          <center><button type="submit" class="btn btn-block btn-primary btn-sm">
                              <h2> <strong> <?php echo $boton ?> </strong> </h2>
                                            ...
                                            <input type="hidden" name="id_historia_registro" value="<?=$id_historia_registro?>">
                                    </div>
...
//     include 'AutoGuardados/CreadorHistorias/AutoGuardado_Historia_Generico.php';
//     //usar esta si es necesario modificar el codigo del autoguardado
...
<script>
        $(document).ready(function() {
            const datosHistoria = <?=$rowDatosRegistro?>;
            
            const keysDatos = Object.keys(datosHistoria);
            keysDatos.forEach(key => {
                const firstCharacter = key[0];
                const value = datosHistoria[key];
                // console.log("firstCharacter", firstCharacter);
                // console.log("value", value);
                
                //PARA QUE SOLO TOME LOS CAMPOS DE DILIGENCIAMIENTO DE HISTORIA
                if (firstCharacter == "x" && value) {
                    if (key == 'x29966887836' || key == 'x26792106189') {
                        console.log(`$(#${key})`, $("#"+key));
                        console.log("value", value);
                    }
            ...
            const datosRips = <?=$rowDatosRips?>;
            if (datosRips.length > 0) {
                const keysRips = Object.keys(datosRips);
                keysRips.forEach(key => {
                    $(`select[name='DiagnosticoConsultaMedica[${key}]']`).val(value).change();
                })
            }
