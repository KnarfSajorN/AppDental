...
include("funciones/funciones.php");
    ...
    $queryList = mysqli_query($conn3, "SELECT * FROM  abonoC where  id = '$idAbono' ");
    while ($rowMotorizado = mysqli_fetch_array($queryList)) {
        ...
        $idCliente = $rowMotorizado['cliente_id'];

        $idCuentaContableCXC = $rowMotorizado['idCuentaContableCXC'];
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    $queryList = mysqli_query($conn3, "SELECT max(id) as ultimo FROM CCompDiario where tipo_comprobante = 5");
    ...
    $numerocomprobante = "CxC-" . str_pad($actual, 6, "0", STR_PAD_LEFT);
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	values 
	('$numerocomprobante','$fechaComprobante',0,0,'Pago Cuenta x Cobrar Factura #{$numeroDoc}','$MontoPagado','$MontoPagado','2','','0','$idCentroCosto','5','$usuario_id', '$idCliente', '1')") or die(mysqli_error($conn3));
	...
	values  
	('$numerocomprobante','$fechaComprobante',0,0,0,'$idCuentaContable_debe',0,'$NombreCuenta','$MontoPagado','0','$Nombre_Pago | Pago Cuentas x Cobrar Factura #{$numeroDoc}','$ultimoCompDiarios')") or die(mysqli_error($conn3));




    $idCuentaContable_haber = $idCuentaContableCXC;
    $NombreCuenta = funcionMaster($idCuentaContable_haber, 'id', 'descripcion', 'CCuentas');
    mysqli_query($conn3, "INSERT INTO CCompDiarioMov 
	(numero, fecha, tipo, estado, asiento, cuenta, id_ccosto, descripcion, monto_debe, monto_haber, referencia, idComprobante)
	values  
	('$numerocomprobante','$fechaComprobante',0,0,0,'$idCuentaContable_haber',0,'$NombreCuenta','0','$MontoPagado','Cuentas x Cobrar Factura #{$numeroDoc}','$ultimoCompDiarios')") or die(mysqli_error($conn3));


$Campo1 = mysqli_query($conn3, "show COLUMNS from abonoC WHERE Field = 'Comprobante_id';");
$nrowCampo1 = mysqli_num_rows($Campo1);
if ($nrowCampo1 == "0") {
	mysqli_query($conn3, "ALTER TABLE `abonoC` ADD `Comprobante_id` TEXT NULL DEFAULT '0' COMMENT 'id de la tabla CCompDiario *Creado desde CTB_RegistroMovimientosCXC*'");
}

mysqli_query($conn3,"UPDATE abonoC set  Comprobante_id='$ultimoCompDiarios' where id = '$idOperacion' limit 1");
