<?php
// no poner ningun include arriba pra evitar error aqui en la respuesta que da
if($_POST['Tipo_Consulta']=="Buscar Precio"){
...
$Nombre_Tabla = "empresasAfiliadas_Servicios";
#Inicio
if (isset($_POST['Guardar_Informacion_Pagina'])) {
    ...
    if ($nrowtabla == 0) {
        ...
        {$Campos},
        `empresaAfiliada_id` int(11) NOT NULL,
    ...
    $empresaAfiliada_id = $_POST['empresaAfiliada_id'];
    ...
    $queryList = mysqli_query($conn3, "INSERT INTO {$Nombre_Tabla} (usuario_id,empresaAfiliada_id,{$Campos}) VALUES ('$usuario_id','$empresaAfiliada_id',{$Valores});");

    $ruta = htmlentities($_SERVER['PHP_SELF']);
    $ruta = str_replace('.php', '', $ruta);
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}?empresaAfiliada_id={$empresaAfiliada_id}&error=Hubo Un Error Al Guardar La Tarifa {$MensajeError}'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}?empresaAfiliada_id={$empresaAfiliada_id}&msg=Se Guardo La Tarifa Correctamente {$MensajeError}'</script>";
    }
}
...
if ($_GET['Eliminar'] <> "") {
    $id = $_GET['Eliminar'];
    $empresaAfiliada_id = $_GET['empresaAfiliada_id'];
    $queryList = mysqli_query($conn3, "UPDATE {$Nombre_Tabla} SET Activo='0' WHERE id ='{$id}' limit 1");

    $ruta = htmlentities($_SERVER['PHP_SELF']);
    $ruta = str_replace('.php', '', $ruta);
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}?empresaAfiliada_id={$empresaAfiliada_id}&error=Hubo Un Error Al Eliminar El Convenio'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}?empresaAfiliada_id={$empresaAfiliada_id}&msg=Se Elimino el Convenio Correctamente'</script>";
    }
}
///////////////////////////////////////////////////////////////////////////
...
<div class="content-wrapper p-3">
    ...
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href="#"> Registro de Servicios a Afiliar </a></li>
        </ol>
    </section>
    ...
    <section class="content">
        <div class="">
            <div class="content">
                <h4 class="Titulo_Pagina"> <a href='<?php echo "{$Base}SO_EmpresasAfiliadas"; ?>'> <i class="fa-solid fa-right-from-bracket fa-rotate-180"></i></a>&nbsp;&nbsp;Registro de Servicios a Afiliar</h4>
                <div class="box">
                    <div class="box-body">
                        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <label>Servicio/Producto del inventario</label>
                                    <select name="Arreglo[inventario_id]" class="form-control select2" style="width: 100%;" onchange="PrecioProducto(this.value)">
                                    ...
                                    <label>*Solo Aparecerán los Inventario que son Simples y Servicios*</label>
                                </div>

                                <div class="form-group col-md-6">
                                    <label>Valor</label>
                                    <input type="number" class="form-control input-lg" name="Arreglo[Precio]" placeholder="Precio" step="0.01" required>
                                </div>
                                


                                <input type="hidden" name="usuario_id" value="<?php echo $usuario_id; ?>">
                                <input type="hidden" name="empresaAfiliada_id" value="<?php echo $empresaAfiliada_id; ?>">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <table id="example1" class="table table-bordered table-striped">
                                            <thead>
                                                <tr>
                                                    <th scope="col">#</th>
                                                    <th scope="col">Nombre del Servicio Afiliado</th>
                                            ...
                                            <tbody>
                                                ...
                                                $queryList = mysqli_query($conn3, "SELECT * FROM  {$Nombre_Tabla} WHERE Activo='1' AND empresaAfiliada_id='{$empresaAfiliada_id}' ");
                                                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
                                                    //$contador++;
                                                    $id = $rowMotorizado['id'];
                                                    $inventario_id = $rowMotorizado['inventario_id'];
                                                    $NombreInventario = funcionMaster($inventario_id, 'ID', 'descripcion', 'sinvetrios');
                                                    $Precio = $rowMotorizado['Precio'];

                                                    $ruta = htmlentities($_SERVER['PHP_SELF']);
                                                    $ruta = str_replace('.php', '', $ruta);
                                                    echo "<tr ><th scope='row' width='2%'>{$id}</th>
                                                    <td width='20%' align='center'>{$NombreInventario}</td>
                                                    <td width='20%' align='center'>{$Precio}</td>";

                                                    echo "<td width='20%' align='center'><font color='#04CC05'> <a href='{$ruta}?Editar={$id}&empresaAfiliada_id={$empresaAfiliada_id}' class='btn btn-block btn-outline-info rounded-pill shadow' ><i class='fa fa-pencil' title='Editar'> Editar</i></a></font><br>
                                                    <font> <a href='{$ruta}?Eliminar={$id}&empresaAfiliada_id={$empresaAfiliada_id}' class='btn btn-block btn-outline-danger rounded-pill shadow'> <i class='fa fa-close' title='Eliminar'> Eliminar</i></a></font><br>
                                                    </td></tr>";
                                                }
...
<script type="text/javascript">
    ...
    function PrecioProducto(valor){
        ...
        $.ajax({
            type: "POST",
            url: "SO_EmpresaAfiliarServicios.php",
            data: {
                inventario_id :valor,
                Tipo_Consulta:"Buscar Precio"
            },
