...
if (isset($_GET['send'])) {
    ...
    ECHO "SELECT idCliente FROM sOperacionInv where idOperacion = $idOperacion";
    ...
    $mensajeW = ' Sr(a) *' . $nombre_cliente . '* Se le ha generado un presupuesto por el valor de : *' . $totalNeto . '* *' . $Moneda . '*, para visualizarlo haga Click Aquí: '.$Base.'imprimirPresupuestoFacial?idOperacion=' . $idOperacion . ' para firmar en el siguiente link para confirmar, Link:  '.$Base.'firma/FirmarPresupuesto/' . $idOperacion . ' ';
    $accion = 0;
    $prueba = Whatsapp_sent_cliente($linkkey, $whatsapp, $mensajeW, $idCliente, $usuario_id_op, $whatsapp, $accion);
    echo "<script language='Javascript'> window.location='preliminarPresupuestoFacial?idOperacion={$idOperacion}';</script>";
    exit();
}
include 'header.php';
include 'menu.php';
...
<div class="content-wrapper p-3">
    ...
    <section class="invoice p-3">
        ...
        <div class="row no-print">
            <div class="col-md-6">
                <a class="btn btn-block btn-outline-info rounded-pill shadow m-1" target="_blank" href="<?php echo $Base; ?>imprimirPresupuestoFacial?idOperacion=<?php echo $idOperacion ?>">
                    <i class="fa fa-print"></i> Imprimir Presupuesto
                </a>

                <!-- <a href="imprimirPresupuestoFacial.php?idOperacion=<?php echo $idOperacion ?>" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Imprimir</a> -->
            </div>

            <div class="col-md-6" align="center">
                <a class="btn btn-block btn-outline-info rounded-pill shadow m-1" target="_blank" href="<?php echo $Base; ?>preliminarPresupuestoFacial?idOperacion=<?php echo $idOperacion ?>&send=true">
                    <i class="fa fa-print"></i> Enviar y Firmar Presupuesto al paciente
                </a>
            </div>
        </div>
    </section>
