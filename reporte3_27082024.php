        ...
        <table class="table  table-bordered table-light">
          <thead>
            <tr style="background: #A4A4A4">
              ...
              <th> Método de pago <br>al facturar </th>
            </tr>
          </thead>
          <tbody>
            <?php
            ...
            if ($tipo == 0) {
              ...
              $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv where ((idEmpresa = '{$_SESSION['ID']}' or idEmpresa = '{$_SESSION['ID_principal']}') or (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}')) and fechaOperacion BETWEEN '$desde' and '$hasta' and tipo IN (1, 7) ");
            } elseif ($tipo <> 0) {

              $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv  where (idEmpresa = $ID or idEmpresa = '{$_SESSION['ID_principal']}') and idCliente = $tipo and fechaOperacion BETWEEN '$desde' and '$hasta' and tipo IN (1, 7)");
            }
            ...
            while ($rowMotorizado = mysqli_fetch_array($queryList)) {
              ...
              echo '     <tr>
                  ...
                  <td width="15%"><div align="right">' . $montoPendiente . '</div> </td>
                  <td width="15%"><div align="right">' . $montoPagado . '</div> </td>
...
<?
  ...
  header('Content-Disposition: attachment; filename=ReporteCxC_' . date("Y-m-d") . '.xls');
  ?>
  <!DOCTYPE html>
  <html>
