...
header('Content-Disposition: attachment; filename=RipsAC.xml');
...
$queryE = mysqli_query($conn3, "SELECT * FROM Rips_Entidades where id='$tipoUsuario'");
...
if($paciente == '1'){
    ...
    $queryU=mysqli_query($conn3,"SELECT * FROM cliente where tipo_cliente in ('CC','RC','TI') $where ");
    ...
    $queryU=mysqli_query($conn3,"SELECT * FROM cliente where tipo_cliente not in ('CC','RC','TI') $where");
    while($rowListaU=mysqli_fetch_array($queryU))
    {
        $ArregloCliente[] = $rowListaU['cliente_id'];
    }
...
if ($doctor == 0) {
    $queryListaH = mysqli_query($conn3, "SELECT * FROM Rips_Informacion where cliente_id = '{$value}'  and Fecha BETWEEN '$desde 00:00:00' and '$hasta 23:59:59'");
   // ECHO "SELECT * FROM Rips_Informacion where cliente_id = '{$value}'  and fecha BETWEEN '$desde 00:00:00' and '$hasta 23:59:59'";
}
else {
    $queryListaH = mysqli_query($conn3, "SELECT * FROM Rips_Informacion where (Fecha BETWEEN '$desde 00:00:00' and '$hasta 23:59:59') AND usuario_id=$doctor and  cliente_id = '{$value}' ");
...
while ($rowListaH = mysqli_fetch_array($queryListaH)) {
  ...
  if($tipoUsuario==$tipoUsuarioSistema){
    ...
    */
    ...
    $Arreglo[$cantidadRegistros]["Fecha_Consulta"] = $Fecha1;
    $Arreglo[$cantidadRegistros]["Numero_Autorizacion"] = $Numero_Autorizacion;
    $Arreglo[$cantidadRegistros]["Tipo_Consulta"] = $Tipo_Consulta;
    $Arreglo[$cantidadRegistros]["Finalidad_Consulta"] = $Finalidad_Consulta;
    $Arreglo[$cantidadRegistros]["Causa_Externa"] = $Causa_Externa;
    $Arreglo[$cantidadRegistros]["CIE10_1"] = $CIE10_1;
    $Arreglo[$cantidadRegistros]["CIE10_2"] = $CIE10_2;
    $Arreglo[$cantidadRegistros]["CIE10_3"] = $CIE10_3;
    $Arreglo[$cantidadRegistros]["CIE10_4"] = $CIE10_4;
    $Arreglo[$cantidadRegistros]["Tipo_Diagnostico"] = $Tipo_Diagnostico;
    $Arreglo[$cantidadRegistros]["Valor_Consulta"] = $Valor_Consulta;
    $Arreglo[$cantidadRegistros]["Valor_Cuota_Moderadora"] = $Valor_Cuota_Moderadora;
