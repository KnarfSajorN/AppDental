...
$queryImg = mysqli_query($conn3, "SELECT * FROM archivos  where cliente_id = '$cliente' and  descripcion='$archivo' AND estado = 1");
...
while ($resulImg = mysqli_fetch_array($queryImg)) {
  ...
  $mensaje .=
    'Ver archivo:  '.$Base.'/archivos/' . $resulImg['codigo'] . ' .<br>
...
while ($resulImg = mysqli_fetch_array($queryImg)) {
  ...
  $mensajeW .=
    'Ver archivo:  '.$Base.'/archivos/' . $Producto . ' 
