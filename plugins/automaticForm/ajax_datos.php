...
$replace = [];
$queryprimary = mysqli_query($conn3, "show columns from `{$tabla}` where `Key` = 'PRI'");
...
$datos = [];
$querycolumns = mysqli_query($conn3, "SHOW columns from {$tabla}") or array_push($datos["Error"], "Columns: " . mysqli_error($con));
$querycliente = mysqli_query($conn3, "SELECT * from {$tabla} ".(!empty($condicion) ? "where {$condicion}" : "")) or array_push($datos["Error"], "Query: ". mysqli_error($con));
