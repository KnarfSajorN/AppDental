...
$conn3 = mysqli_connect($host, $user, $pass, $dbname);
...
mysqli_query($conn3, $q) or array_push($datos, ["Error" => "$q -> " . mysqli_error($conn3)]);
$valores = '';
...
$queryInfo = mysqli_query($conn3, $q) or array_push($datos, ["Error" => "$q -> " . mysqli_error($conn3)]);
...
$queryValidar = mysqli_query($conn3, $q) or array_push($datos, ["Error" => "$q -> " . mysqli_error($conn3)]);
if (empty(mysqli_num_rows($queryValidar))) {
    ...
    $auditorQuery = $q;
    $validar = mysqli_query($conn3, $q) or array_push($datos, ["Error" => "$q -> " . mysqli_error($conn3)]);

    $idInsert = ($_GET['type'] == 2 ? $_GET["idUpdate"] : mysqli_insert_id($conn3));
    if (!empty($countSize)) {
        $carpetas = explode("|//|", substr($carpetas, 0, -4));
        foreach ($carpetas as $key => $value) {
            if (!file_exists('../../uploads/'.$_SESSION['ID_principal'].'/' . $value . "/")) {
                mkdir('../../uploads/'.$_SESSION['ID_principal'].'/' . $value . "/", 0777, true);
            }
        }
        ...
        foreach ($_FILES['archivos']['name'] as $key => $value) {
            if ($_FILES['archivos']['size'][$key] > 0) {
                $llave = explode("|", $key);
                $valoresFile2 = explode("= ", $valoresFile[$conn3tador += 1]);
                ...
                if ($ext == "jpg" || $ext == "png" || $ext == "jpeg" || $ext == "gif") {
                    ...
                    comprimirImagen($_FILES['archivos']['tmp_name'][$key], '../../uploads/'.$_SESSION['ID_principal'].'/' . $llave[1] . "/" . str_replace("'", "", str_replace("", "", $valoresFile2[1])), 70);
                } else {
                    // otros
                    // si es otros no hacemos nada y continua
                    move_uploaded_file($_FILES['archivos']['tmp_name'][$key], '../../uploads/'.$_SESSION['ID_principal'].'/' . $llave[1] . "/" . str_replace("'", "", str_replace("", "", $valoresFile2[1])));
                }
