...
//$check = getimagesize($_FILES["upload"]["tmp_name"]);//-------------------------se quita esto para poder subir todo tipo de archivos
$check = $_FILES["upload"]["tmp_name"];
if($check !== false) {
    $uploadOk = 1;
} else {
    echo "<script>alert('111".$uploadimgerrors1."');</script>";
    $uploadOk = 0;
}
...
// Allow certain file formats // ---------------------------------------aqui se le cambia los formatos de subida
if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
&& $imageFileType != "gif" && $imageFileType != "ico" && $imageFileType != "pdf" && $imageFileType != "docx" && $imageFileType != "jfif") {
