...
function loadImages() {
    ...
    if(file_exists($useruploadpath)){
        ...
        $dir = $useruploadpath;
        $files = glob("$dir*.{jpg,jpe,jpeg,png,gif,ico,jfif,pdf,docx,dcm}", GLOB_BRACE);// ---------------------- aqui poner tambien las extensiones para que se vean en el inicio del explorador
        ...
        echo '<div id="folderError">Error '.$useruploadfolder.'<b> Vuelva a abrir el explorador de archivos </b>';
        //echo $useruploadpath;
        if (!file_exists($useruploadpath)) {
