...
$Nombre_Tabla = "Proveedor_TipoPersona";
#Inicio
if (isset($_POST['Guardar_Informacion_Pagina'])) {
    ...
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}?error=Hubo Un Error Al Guardar el Tipo de Persona'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}?msg=Se Guardo el Tipo de Persona Correctamente'</script>";
    }
}
...
if (isset($_POST['Actualizar_Informacion_Pagina'])) {
    ...
    $ruta = str_replace('.php', '', $ruta);
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}?error=Hubo Un Error Al Actualizar el Tipo de Persona'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}?msg=Se Actualizo el Tipo de Persona Correctamente'</script>";
    }
}
///////////////////////////////////////////////////////////////////////////
if ($_GET['Eliminar'] <> "") {
    ...
    $ruta = str_replace('.php', '', $ruta);
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}?error=Hubo Un Error Al Eliminar el Tipo de Persona'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}?msg=Se Elimino el Tipo de Persona Correctamente'</script>";
    }
}
///////////////////////////////////////////////////////////////////////////
if (isset($_GET['Editar'])) {
    $id = $_GET['Editar'];
    $queryList = mysqli_query($conn3, "SELECT * FROM  {$Nombre_Tabla} where (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') and id=$id limit 1");
...
<div class="content-wrapper p-3">
    ...
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href="#"> Registro de Tipo de Persona </a></li>
        </ol>
    </section>
    ...
    <section class="content">
        <div class="">
            <div class="content">
                <h4 class="Titulo_Pagina">Registro de Tipo de Persona </h4>
                <div class="box">
                    <div class="box-body">
                        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
                            ...
                            <div class="form-group col-md-12">
                                <label>Código</label>
                                <input type="text" class="form-control input-lg" name="Arreglo[Codigo]"  value="" maxlength="120" oninput="if(this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" >

                              <!--  <textarea name="Arreglo[Codigo]" class="editorJR" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"></textarea> -->
                            </div>
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-body">
                                    <div class="col-md-12">
                                        <h2 style="text-align: center;font-weight: bold;"> Tipo Persona </h2>
