<?php # Por si se me olvida por favor borrar o comentar este archivo

session_start();

// echo "<pre>" . json_encode($_SESSION, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . "</pre>";
...
function replaceMaster($search, $replace, $folder = "./*", $config = [])
{
    ...
    foreach ($all_files as $route) {
        if (strpos(getenv("SCRIPT_NAME"), basename($route)) === false) {
            if (is_dir($route)) {
                if ($config["dirs"] === true) $change = array_merge($change, replaceMaster($search, $replace, "{$route}/*", $config));
            } else {
