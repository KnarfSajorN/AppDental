            ...
            <table class="table table-striped" border="1">
                ...
                <tbody>
                    ...
                    if ($queryListC) {
                        ...
                        <td width="25%">' . funcionMaster1($cargoID, 'id', 'Nombre', 'empresas_cargos') . ' </td>
...
$queryList1 = mysqli_query($conn3, "SELECT COUNT(DISTINCT(cliente_id)) AS total,cargoU AS cargoU FROM pacientes  WHERE DATE(Fecha) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) group by cargoU");
        ...
        var data = google.visualization.arrayToDataTable([
            ...
            if ($queryList1) {
                while ($row1 = mysqli_fetch_array($queryList1)) {
                    // $count1 += $row1["total"];
                    // echo "['" . $genero . "', " . $row["total"] . "],";
                    if ($row1['cargoU'] != '' && $row1['cargoU'] > 0) {
                        echo "[' " . funcionMaster1($row1["cargoU"], 'id', 'Nombre', 'empresas_cargos') . "', " . $row1["total"] . "],";
                    }
                }
            }
