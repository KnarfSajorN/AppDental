...
<body>
    ...
    <div class="row m-0 p-0">
        <?php $contador = 0 ?>
        <?php $consAntes = mysqli_query($conn3, "SELECT * from archivos2 where cliente_id = $cliente_id and tipo = '0' ".(!empty($historiaClinica1) ? "and id_historia = '$historiaClinica1'" : "")."") ?>
        <?php $countAntes = mysqli_num_rows($consAntes) ?>
        <?php $banderaAntes = (!empty($countAntes) ? true : false) ?>
        <?php $contador += (!empty($countAntes) ? 1 : 0) ?>
        <?php $consDespues = mysqli_query($conn3, "SELECT * from archivos2 where cliente_id = $cliente_id and tipo = '1' ".(!empty($historiaClinica1) ? "and id_historia = '$historiaClinica1'" : "")."") ?>
        <?php $countDespues = mysqli_num_rows($consDespues) ?>
        <?php $banderaDespues = (!empty($countDespues) ? true : false) ?>
        <?php $contador += (!empty($countDespues) ? 1 : 0) ?>

        <?php $consDurante = mysqli_query($conn3, "SELECT * from archivos2 where cliente_id = $cliente_id and tipo = '2' ".(!empty($historiaClinica1) ? "and id_historia = '$historiaClinica1'" : "")."") ?>
        <?php $countDurante = mysqli_num_rows($consDurante) ?>
        <?php $banderaDurante = (!empty($countDurante) ? true : false) ?>
        <?php $contador += (!empty($countDurante) ? 1 : 0) ?>

        <div align="center" class="<?= ($banderaAntes == true ? "col-md-".($contador == 3 ? "4" : ($contador == 2 ? "6" : ($contador == 1 ? "12" : "")))."" : "d-none") ?>">
            <h2 class="center text-center text-white">Antes</h2>
            <div style="margin-left:-7rem;" class="slider">
                <?php foreach ($consAntes as $datAntes) { ?>
                    <div class="slide">
                        <?php $fileAntes = explode("/", mime_content_type("img/historiaImg/".$cliente_id."/".$datAntes['img1'])) ?>
                        <?php if ($fileAntes[0] == "image") { ?>
                            <img src="img/historiaImg/<?= $cliente_id ?>/<?= $datAntes['img1'] ?>" />
                        <?php } else if ($fileAntes[0] == "video") { ?>
                            <video autoplay controls>
                                <source src="img/historiaImg/<?= $cliente_id ?>/<?= $datAntes['img1'] ?>">
                            </video>
                        <?php } else { ?>
                            <strong style="color: red">Formato Invalido: <?= $fileAntes[0].": ".$fileAntes[1] ?></strong>
                        <?php } ?>
                        <h3><?= $datAntes['titulo1'] ?></h3>
                        <p><?= $datAntes['descripcion1'] ?><br><?= $datAntes['fechaRef'] ?></p>
                    </div>
                <?php } ?>
                <div class="slide">
        ...
        <div align="center" class="<?= ($banderaDurante == true ? "col-md-".($contador == 3 ? "4" : ($contador == 2 ? "6" : ($contador == 1 ? "12" : "")))."" : "d-none") ?>">
            <h2 class="center text-center text-white">Durante</h2>
            <div style="margin-left:-7rem;" class="slider3">
                <?php foreach ($consDurante as $datDurante) { ?>
                    <div class="slide">
                        <?php $fileDespues = explode("/", mime_content_type("img/historiaImg/".$cliente_id."/".$datDurante['img1'])) ?>
                        <?php if ($fileDespues[0] == "image") { ?>
                            <img src="img/historiaImg/<?= $cliente_id ?>/<?= $datDurante['img1'] ?>" />
                        <?php } else if ($fileDespues[0] == "video") { ?>
                            <video autoplay controls>
                                <source src="img/historiaImg/<?= $cliente_id ?>/<?= $datDurante['img1'] ?>">
                            </video>
                        <?php } else { ?>
                            <strong style="color: red">Formato Invalido: <?= $datDurante[0].": ".$datDurante[1] ?></strong>
                        <?php } ?>
                        <h3><?= $datDurante['titulo1'] ?></h3>
                        <p><?= $datDurante['descripcion1'] ?><br><?= $datDurante['fechaRef'] ?></p>
                    </div>
                <?php } ?>
                <div class="slide">
                    <!-- <img src="logos/<?= funcionMaster(1, "ID_Usuario", "logoF", "config") ?>" /> -->
                    <h3>Imágenes de Durante</h3>
                    <p><br></p>
                </div>
            </div>
        </div>
        ...
        <div align="center" class="<?= ($banderaDespues == true ? "col-md-".($contador == 3 ? "4" : ($contador == 2 ? "6" : ($contador == 1 ? "12" : "")))."" : "d-none") ?>">
            <h2 class="center text-center text-white">Después</h2>
            <div style="margin-left:-7rem;" class="slider2">
                <?php foreach ($consDespues as $datDespues) { ?>
                    <div class="slide">
                        <?php $fileDespues = explode("/", mime_content_type("img/historiaImg/".$cliente_id."/".$datDespues['img1'])) ?>
                        <?php if ($fileDespues[0] == "image") { ?>
                            <img src="img/historiaImg/<?= $cliente_id ?>/<?= $datDespues['img1'] ?>" />
                        <?php } else if ($fileDespues[0] == "video") { ?>
                            <video autoplay controls>
                                <source src="img/historiaImg/<?= $cliente_id ?>/<?= $datDespues['img1'] ?>">
                            </video>
                        <?php } else { ?>
                            <strong style="color: red">Formato Invalido: <?= $datDespues[0].": ".$datDespues[1] ?></strong>
                        <?php } ?>
                        <h3><?= $datDespues['titulo1'] ?></h3>
                        <p><?= $datDespues['descripcion1'] ?><br><?= $datDespues['fechaRef'] ?></p>
                    </div>
                <?php } ?>
                <div class="slide">
                    <!-- <img src="logos/<?= funcionMaster(1, "ID_Usuario", "logoF", "config") ?>" /> -->
