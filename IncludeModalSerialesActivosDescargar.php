...
function ModalModuloExistenciasActivos_Compras(Detalle_id,SinvDep,caracter_serial,cantidad){
    ...
    $('#DetalleActivoCargar_Modal').val(Detalle_id);
...
function GuardarDatosExistenciasActivos(){
    ...
    data: {
        formulario: formData,
        Tipo_Consulta: "Agregar Seriales Producto Detalles Descargar Activos"
    },
    success: function(response) {
        var Arreglo = JSON.parse(response);
...
function BuscarSerialRegistradoDetalles(campo) {
    ...
    var Detalle_id = $('#DetalleActivoCargar_Modal').val();
    //console.log(campo);
    var Sinvdep = document.getElementById('SinvDep_Modal').value;
    ...
    $.ajax({
        ...
        data: {
            ...
            Sinvdep: Sinvdep,
            Tipo_Consulta: "Buscar Serial Registrado Detalles Descargar Activos"
        },
        success: function(response) {
...
function ModalModuloHistorialExistencias(Detalle_id,caracter_serial){
    ...
    $('#DetalleActivoCargar_ModalHistorial').val(Detalle_id);
...
function EliminarSerialesRegistrados(){
    ...
    var Detalle_id = $('#DetalleActivoCargar_ModalHistorial').val();
...
<div class="modal-content">
  ...
  <div class="modal-body row" id="ModalCamposSerialExistencias">
    ...
    <input type="hidden" name="DetalleActivoCargar" id="DetalleActivoCargar_Modal" >
...
<div class="modal-content">
  ...
  <div class="modal-body row" >
    ...
    <input type="hidden" name="DetalleActivoCargar_ModalHistorial" id="DetalleActivoCargar_ModalHistorial" >
  </div>
...
$(document).ready(function () {
    ...
    if (faltaLlenarSerialElement.length > 0) {
        ...
        //$('#TotalizarCargarActivo').append(nuevoInput);
        var br = $('<br>');

        // Encuentra el botón submit
        var submitButton = $('#TotalizarDescargarActivo :submit');

        // Si existe el botón submit, coloca el nuevo input encima de él
        if (submitButton.length > 0) {
            submitButton.before([br, nuevoInput, br]);
        } else {
            // Si no existe el botón submit, coloca el nuevo input al final del formulario
            $('#TotalizarDescargarActivo').append([br, nuevoInput, br]);
        }
