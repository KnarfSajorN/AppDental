...
if($MinMeses!="" AND $MaxMeses!=""){
    $QueryLista = mysqli_query($conn3, "SELECT * from Tabla_Crecimiento_CDC where Tipo = '{$Tipo}' AND Genero='{$Genero[$G]}' AND Meses BETWEEN '$MinMeses' AND '$MaxMeses'  ORDER BY Meses ASC");
    $min_grafica = $MinMeses;
    $max_grafica = $MaxMeses;
}else{
    $QueryLista = mysqli_query($conn3, "SELECT * from Tabla_Crecimiento_CDC where Tipo = '{$Tipo}' AND Genero='{$Genero[$G]}' ORDER BY Meses ASC");
    $min_grafica = '0';
    $max_grafica = '241';
    $Intervalo = '12';
}
