      ...
      <div class="col-xs-12 table-responsive">
        <table class="table table-striped" border="1">
          ...
          <tbody>
            ...
            if ($tipo == 0) {
              $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv where activo = 1 and (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}') and fechaOperacion BETWEEN '$desde' and '$hasta' and tipo IN (1, 7) order by numeroDoc asc");
            } elseif ($tipo <> 0) {
              $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv  where activo = 1 and (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}') and idCliente = $tipo and fechaOperacion BETWEEN '$desde' and '$hasta' and tipo IN (1, 7) order by numeroDoc asc ");
            }
            $sumtotalNeto = 0;
            $summontoPagado = 0;
          ...
          <table class="table table-striped" border="1">
            <thead>
              <tr style="background: #A4A4A4">
                ...
                <th>
                  <div align="right">Metodo Pago</div>
                </th>
                <th>
                  <div align="right">Abono</div>
                </th>
            ...
            <tbody>
              ...
              if ($tipo == 0) {
                ...
                $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv where (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}') and  tipo IN (1, 7) order by numeroDoc asc ");
              } elseif ($tipo <> 0) {

                $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv  where (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}') and idCliente = $tipo  and tipo IN (1, 7) order by numeroDoc asc");
              }
              ...
              while ($rowMotorizado = mysqli_fetch_array($queryList)) {
                ...
                $queryAbonos = mysqli_query($conn3, "SELECT * FROM  abono where numero_documento = $numeroDoc and (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') and fecha BETWEEN '$desde' and '$hasta' ");
                // echo "SELECT * FROM  abono where numero_documento = $numeroDoc and (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') and fecha BETWEEN '$desde' and '$hasta' ";
                if ($queryAbonos) {
                  while ($rowMetodosPagoAbono = mysqli_fetch_assoc($queryAbonos)) {
                    $id = $rowMetodosPagoAbono['id'];
                    $numero_documento = $rowMetodosPagoAbono['numero_documento'];
                    $cliente_id = $rowMetodosPagoAbono['cliente_id'];
                    $fecha = $rowMetodosPagoAbono['fecha'];
                    $metodo_pagoAbono = $rowMetodosPagoAbono['pago'];
                    $nombre_ClienteAbono = funcionMaster($cliente_id, 'cliente_id', 'nombre_cliente', 'cliente');
                    $metodo_pagoAbono = funcionMaster($metodo_pagoAbono, 'id', 'Nombre', 'Medios_Pago');
                    $montoPagadoAbono = $rowMetodosPagoAbono['valor_abonado'];
                    $abonos[$id][] = [
                      'cliente_id' => $cliente_id,
                      'fecha' => $fecha,
                      'metodo_pago' => $metodo_pagoAbono,
                      'nombre_cliente' => $nombre_ClienteAbono,
                      'montoPagado' => $montoPagadoAbono,
              ...
              if (count($abonos) > 0) {
                ...
                foreach ($abonos as $abono) {
                  ...
                  $fecha = $abono['fecha'];
                  $metodo_pagoAbono = $abono['metodo_pago'];
                  $montoPagadoAbono = $abono['montoPagado'];
                  $numero_documento = $abono['numero_documento'];
                  echo '     <tr>
                  <td width="10%">' . $numero_documento . ' </td>
                  <td width="20%">' . $nombre_cliente . ' </td>
                  <td width="15%">' . $fecha . ' </td> 
                  <td width="15%">' . $metodo_pagoAbono . ' </td>
                  <td width="15%"><div align="right">' . number_format($montoPagadoAbono, 2) . '</div> </td>
                 
                </tr>';
                $abonoTotal += $montoPagadoAbono;
                }
               
              }
                echo '<tr>
                <td colspan="4">
                <div align="right">Total Abonos</div>
              </td>
                <td>
                 <div align="right">' . number_format($abonoTotal, 2) . '</div> </td>
          ...
          <table class="table table-striped" border="1">
            ...
            <tbody>
              ...
              if ($tipo == 0) {
                ...
                WHERE sdmp.fechaRegistro BETWEEN '$desde 00:00:00' AND '$hasta 23:59:59'
                ...
                left join sOperacionInv sin on sdmp.idOperacion = sin.idOperacion
                INNER JOIN Medios_Pago mp ON sdmp.metodo_pago = mp.id 
                WHERE sdmp.fechaRegistro BETWEEN '$desde 00:00:00' AND '$hasta 23:59:59'  and sdmp.id_cliente = $tipo
                and (sin.ID_principal = '{$_SESSION['ID']}' or sin.ID_principal = '{$_SESSION['ID_principal']}')
                and sin.tipo IN (1, 7)
                GROUP BY mp.Nombre");
              }
        ...
        <div class="col-xs-12 table-responsive">
          <div class="col-xs-12 ">
            <label> Control de Gasto/Egreso</label>
          </div>
          <table class="table table-striped" border="1">
            <thead>
              <tr style="background: #A4A4A4">
            ...
            <tbody>
              <?php
              $queryList = mysqli_query($conn3, "SELECT * FROM S_GE_nuevo WHERE fecha_creacion between '$desde' and '$hasta' and  (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') ") or die(mysqli_error($conn3));
              if ($queryList) {
                $totalGasto = 0;
                $totalEgreso = 0;
                while ($rowMedio = mysqli_fetch_array($queryList)) {
                  $fecha = $rowMedio['fecha'];
                  $total = $rowMedio['mes'];
                  $descripcion = $rowMedio['descripcion'];
                  $monto = $rowMedio['monto'];
                  $tipo = $rowMedio['tipo'];
                  ...
                  $categoria = funcionMaster($rowMedio['categoria'], 'id', 'descripcion', 'S_GE_categorias'); //$rowMedio['categoria'];
                  ...
                  <td width="20%">' . number_format($monto, 2) . '</td>
                  <td width="20%">' . $tipo . '</td>
          ...
          <h5> <b>Total Gasto: </b> <?= number_format($totalGasto, 2) ?> </h5>
          <h5> <b>Total Egreso: </b> <?= number_format($totalEgreso, 2) ?> </h5>
      ...
      <div class="col-xs-12 table-responsive">
        <table class="table table-striped" border="1">
          ...
          <tbody>
            ...
            if ($tipo == 0) {
              $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv where activo = 1 and (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}') and fechaOperacion BETWEEN '$desde' and '$hasta' and tipo IN (1, 7) ");
            } elseif ($tipo <> 0) {
              $queryList = mysqli_query($conn3, "SELECT * FROM  sOperacionInv  where activo = 1 and (ID_principal = '{$_SESSION['ID']}' or ID_principal = '{$_SESSION['ID_principal']}') and idCliente = $tipo and fechaOperacion BETWEEN '$desde' and '$hasta' and tipo IN (1, 7) ");
            }
            $sumtotalNeto = 0;
            $summontoPagado = 0;
            if ($queryList) {
