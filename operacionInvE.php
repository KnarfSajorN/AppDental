          ...
          <div class="tab-pane show active" id="tab-eg-0" role="tabpanel">
            ...
            <form id="totalizar-operacionInv-form" class="form-group row">
              ...
              <div class="col-md-12">
                ...
                <hr>
                <button class="btn btn-block btn-outline-info btn-lg rounded-pill shadow" name="GuardarOperacionEntrada" <?= (($Deposito_id != "0" and $Deposito_id != "") ? '' : 'disabled') ?> type="button" onclick="$('#totalizar-operacionInv-form').automaticForm({type: 1,table: 'operacioninvHeader', token: 'si', tokenText: '<?= $ttext ?>', tokenUser: '<?= $tuser ?>', tokenNumbers: '<?= $tnumeros ?>', reload:'', page: 'entradadeinventario', post: true});">Guardar</button>
              </div>
            </form>
