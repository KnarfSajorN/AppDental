...
$Nombre_Tabla = "LB_PaqueteExamenes";
#Inicio
if (isset($_POST['Guardar_Informacion_Pagina'])) {
    ...
    if ($nrowtabla == 0) {
        ...
        {$Campos},
        `Examenes` text DEFAULT 'id de los examenes separados por comas',
    ...
    if ($Examenes != "") {
        ...
        $ExamenesComas = trim($ExamenesComas, ',');
    }

    $queryList = mysqli_query($conn3, "INSERT INTO {$Nombre_Tabla} (usuario_id,Examenes,{$Campos}) VALUES ('$usuario_id','$ExamenesComas', {$Valores});");
...
if (isset($_POST['Actualizar_Informacion_Pagina'])) {
    ...
    $queryList = mysqli_query($conn3, "UPDATE {$Nombre_Tabla} SET {$Campos} ,Examenes='$ExamenesComas' WHERE id = '{$arreglo_id}' limit 1;");
...
<div class="content-wrapper p-3">
    ...
    <section class="content">
        <div class="">
            <div class="content">
                <h4 class="Titulo_Pagina">Registro de Paquetes</h4>
                <div class="box">
                    <div class="box-body">
                        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST" class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select name="Examenes[]" id="Examenes" class="form-control select2" style="width: 100%;" multiple>
                                        ...
                                        <table id="example1" class="table table-bordered table-striped" style="width:100%">
                                            ...
                                            <tbody>
                                                ...
                                                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
                                                    ...
                                                    foreach ($Examenes as $key => $value) {
                                                        $Examenactivo = funcionMaster($value, 'id', 'Activo', 'LB_Examen');
                                                        if($Examenactivo==1){
                                                            $ExamenesComas .= funcionMaster($value, 'id', 'Nombre', 'LB_Examen') . "<br>";
                                                        }
                                                    }
                                                    $ExamenesComas = trim($ExamenesComas, '<br>');

                                                    $ruta = htmlentities($_SERVER['PHP_SELF']);
                                                    $ruta = str_replace('.php', '', $ruta);
                                                    echo "<tr ><th scope='row' width='2%'>{$id}</th>
                                                    <td width='20%' align='center'>{$Nombre}</td>
                                                    <td width='20%' align='center'>{$ExamenesComas}</td>";
