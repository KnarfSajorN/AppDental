...
$queryList = mysqli_query($conn3, "SELECT * FROM  sDetalleOperDevolucion where estado = 1 and id_usuario = $idEmpresa and  id_cliente = $idCliente and idOperacion = $idOperacion ");
...
while ($rowMotorizado = mysqli_fetch_array($queryList)) {
	...
	$ArregloCosto["$id"]["monto_debe"]=$CostoProductoTotal;
...
$ArregloCuentaVenta["1"]["monto_debe"] = $MontoTotalSinImpuesto;
$ArregloCuentaVenta["1"]["monto_haber"] = 0;
...
$ArregloCuentaCosto["1"]["monto_debe"] = 0;
$ArregloCuentaCosto["1"]["monto_haber"] = $MontoTotalCosto;
...
///////////////////////////////
$MontoFinalCuentaCobrar_Filtro = round($sOperacion_TotalNeto - $sOperacion_MontoDevolucion,2);
...
mysqli_query($conn3,"INSERT INTO CCompDiario 
	...
	values 
	('$numerocomprobante','$fechaComprobante',0,0,'Devolucion Factura #{$numeroFactura}','$SumGeneral','$SumGeneral','$Movimientos','','0','$idCentroCosto','4','$idEmpresa', '$idCliente', '1')") or die(mysqli_error($conn3));
...
$MontoFinalCuentaCobrar = round($sOperacion_TotalNeto - $sOperacion_MontoDevolucion,2);
if($idCuentaContable_cuentaporcobrar!="0" AND $MontoFinalCuentaCobrar>0){
	...
	values  
	('$numerocomprobante','$fechaComprobante',0,0,0,'$idCuentaContable_cuentaporcobrar',0,'$NombreCuentaXCobrar','0','$MontoFinalCuentaCobrar','Cuenta por Cobrar Factura #{$numeroFactura}','$ultimoCompDiarios')") or die(mysqli_error($conn3));
}
