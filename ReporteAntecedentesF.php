...
$queryList1 = mysqli_query($conn3, "SELECT COUNT(AntFId) AS total,AntFId AS AntFId FROM Reporte_AntecedentesF WHERE DATE(Fechar_Registro) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) group by AntFId");
    ...
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ...
            while ($row1 = mysqli_fetch_array($queryList1)) {
                // $count1 += $row1["total"];
                // echo "['" . $genero . "', " . $row["total"] . "],";
                if ($row1['AntFId'] != '' && $row1['AntFId'] > 0) {
                    echo "[' " . funcionMaster1($row1["AntFId"], 'id', 'Nombre', 'antecedentes_PF') . "', " . $row1["total"] . "],";
                }
            }
        ...
        var options = {
            title: "ANTECEDENTES FAMILIARES DE LOS TRABAJADORES ",
            width: 1000,
            height: 1000,
            isStacked: 'percent',
            series: {
