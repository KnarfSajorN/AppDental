...
if($_POST["Tipo_Consulta"] == "Agregar Firma"){
  ...
  $NombreTabla  = decrypt($_POST['tabla']);
  $Tabla_id  = decrypt($_POST['tabla_id']);

  $Campo1 = mysqli_query($conn3, "show COLUMNS from {$NombreTabla} WHERE Field = 'Firma';");
  $nrowCampo1 = mysqli_num_rows($Campo1);
  if ($nrowCampo1 == "0") {
      mysqli_query($conn3, "ALTER TABLE `{$NombreTabla}` ADD `Firma` LONGTEXT NULL  COMMENT '*Creado desde modulo de ModuloFirma/firmar.php*'");
  }

  $Campo1 = mysqli_query($conn3, "show COLUMNS from {$NombreTabla} WHERE Field = 'Firma_Datos';");
  $nrowCampo1 = mysqli_num_rows($Campo1);
  if ($nrowCampo1 == "0") {
      mysqli_query($conn3, "ALTER TABLE `{$NombreTabla}` ADD `Firma_Datos` LONGTEXT NULL  COMMENT '*Creado desde modulo de ModuloFirma/firmar.php*'");
  }

  $FirmaDatos = mysqli_real_escape_string($conn3,"Nombres: ".$Nombres."&nbsp;<br>Documento: ".$Documento);
  $query = mysqli_query($conn3, "UPDATE {$NombreTabla} SET Firma = '{$firma_base64}', Firma_Datos = '{$FirmaDatos}'  WHERE id = {$Tabla_id} limit 1");
...
$Tabla_Inicial = $_GET['tb'];
$id_Inicial = $_GET['id'];

$Tabla=decrypt($Tabla_Inicial);
$id_numerico=decrypt($id_Inicial);


$queryList=mysqli_query($conn3,"SELECT Firma FROM {$Tabla} where Firma != '' AND id = '$id_numerico'");
              ...
              <div class="card-front">
                <div class="center-wrap">
                  <div class="section text-center" style="color: white;">
                    ...
                    <h4 class="mb-4 pb-3">Modulo de Firma</h4>
...
?>
  $("#reg-log").addClass("active");
  $("#BotonGuardar").css("display","none");
  $("#MensajeFinal").html("<u>Ya se encuentra registrada la firma</u>");
<?php
}
?>
</script>
