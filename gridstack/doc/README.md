...
- `acceptWidgets` - Accept widgets dragged from other grids or from outside (default: `false`). Can be:
   ...
   * `function (el: Element): boolean` function called before an item will be accepted when entering a grid. the function will be passed the item being dragged, and should return true | false. See [example](https://github.com/gridstack/gridstack.js/blob/master/baseDev/two.html#L62)
- `alwaysShowResizeHandle` - possible values (default: `mobile`) - does not apply to non-resizable widgets
  ...
  * `'mobile'` if running on a mobile device, default to `true` (since there is no hovering per say), else `false`.
  See [mobile](http://gridstack.github.io/gridstack.js/baseDev/mobile.html)
...
- `cellHeight`- one cell height (default?: 'auto'). Can be:
   *  an integer (px)
   *  a string (ex: '100px', '10em', '10rem'). Note: % doesn't right - see [CellHeight](http://gridstackjs.com/baseDev/cell-height.html)
...
- `column` - Integer > 0 (default 12) which can change on the fly with `column(N)` API, or `'auto'` for nested grids to size themselves to the parent grid container (to make sub-items are the same size). See [column](http://gridstackjs.com/baseDev/column.html) and [nested](http://gridstackjs.com/baseDev/nested.html)
...
- `dragOut` to let user drag nested grid items out of a parent or not (default false) See [example](http://gridstackjs.com/baseDev/nested.html)
- `engineClass` - the type of engine to create (so you can subclass) default to GridStackEngine
- `float` - enable floating widgets (default: `false`) See [example](http://gridstackjs.com/baseDev/float.html)
...
- `removable` - if `true` widgets could be removed by dragging outside of the grid. It could also be a selector string, in this case widgets will be removed by dropping them there (default: `false`) See [example](http://gridstackjs.com/baseDev/two.html)
...
- `rtl` - if `true` turns grid to RTL. Possible values are `true`, `false`, `'auto'` (default: `'auto'`) See [example](https://gridstackjs.com/baseDev/right-to-left(rtl).html)
...
* see [nested.html](https://github.com/gridstack/gridstack.js/tree/master/baseDev/nested.html) demo
...
- see [example](http://gridstackjs.com/baseDev/serialization.html)
...
- see [serialization](http://gridstackjs.com/baseDev/serialization.html) and [nested](http://gridstackjs.com/baseDev/nested.html)
