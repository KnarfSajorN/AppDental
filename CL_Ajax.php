    ...
    if ($noLaboral == 0) {
        ...
        if ($lt == 0 and $DiaSemana == 'Lunes') {
            ...
            if ($cuantos > 0) {
                ...
                $queryList = mysqli_query($conn3, "SELECT * from citas where fecha = '$fecha' and doctor = '$doctor' and estado != '5' order by Hora");
                // $nrowl = mysqli_num_rows($queryList);
                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
                    ...
                    $motivoConsulta = funcionMaster($Motivo, 'id', 'descripcion', 'Motivos_Consulta');
                    echo "<tr>
        ...
        if ($tiempoConsulta > 0 and $trabaja == 1) {
            ...
            do {
                ...
                $queryAgenda = mysqli_query($conn3, "SELECT count(idCitas) as agendado FROM  citas  where doctor = '$doctor' and (Hora <= '{$horaM}' and horaF > '{$horaM}')  and fecha = '$fecha' and estado != '5' ");
        ...
        echo "<font color = 'red' size='5'> Dia no Laboral </font>";
    }
        ...
        foreach ($ArregloDias as $key => $value) {
            ...
            if ($value == 1 and $DiaSemana == $key and $NoDisponible == "No") {
                if (($HoraV >= $ArregloDias_1[$cont] and  $HoraV < $ArregloDias_2[$cont]) or ($HoraV >= $ArregloDias_3[$cont] and  $HoraV < $ArregloDias_4[$cont])) {
                    ...
                    $queryList = mysqli_query($conn3, "SELECT count(idCitas) as Cuantoshora from citas where fecha = '$fecha' and (Hora <= '{$HoraV}' and horaF > '{$HoraV}') and doctor = '$doctor' and estado != '5' ");
                    ...
                    if (count($rangoNoPermitido) > 0) {
                        foreach ($rangoNoPermitido as $rango) {
                            if ($HoraV >= $rango[1] and $HoraV <= $rango[2]) {
                                $Cuantoshora = 1;
                            }
                        }
                    }
    ...
    if ($cliente == "0") {
        ...
        <div class="form-group">
            <div class="input-icon">
                <div align="left"><strong>Documento</strong></div>
                <input type="number" class="form-control input-lg" onblur="validaCliente(this.value)" name="CODI_CLIENTE" id="CODI_CLIENTE" placeholder="Documento de Identidad" required>
            </div>
            <div class="input-icon box-info-content">
            </div>
        </div> 
        <div class="form-group">
            <div class="input-icon">
    ...
    if ($noLaboral == 0) {
        ...
        if ($lt == 0 and $DiaSemana == 'Lunes') {
            ...
            if ($cuantos > 0) {
                ...
                $queryList = mysqli_query($conn3, "SELECT * from citas where fecha = '$fecha' and doctor = '$doctor' order by Hora");
                // $nrowl = mysqli_num_rows($queryList);
                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
                    $ContadorCitasPacientes++;
                    $Hora = $rowMotorizado['Hora'];
        ...
        if ($tiempoConsulta > 0 and $trabaja == 1) {
            ...
            do {
                ...
                $queryAgenda = mysqli_query($conn3, "SELECT count(idCitas) as agendado FROM  citas  where doctor = '$doctor' and (Hora <= '{$horaM_1}' and horaF > '{$horaM_1}')  and fecha = '$fecha' and estado <= '3'");
                // $nrowl = mysqli_num_rows($queryAgenda);
                while ($row_agenda = mysqli_fetch_array($queryAgenda)) {
                    $agendado    = $row_agenda['agendado'];
                }
            ...
            do {
                ...
                $agendado = 0;
                //$queryAgenda = mysqli_query($conn3, "SELECT count(idCitas) as agendado FROM  citas  where doctor = '$doctor' and Hora = '$horaM' and fecha = '$fecha' and estado <= '3'");
                $queryAgenda = mysqli_query($conn3, "SELECT count(idCitas) as agendado FROM  citas  where doctor = '$doctor' and (Hora <= '{$horaM}' and horaF > '{$horaM}')  and fecha = '$fecha' and estado <= '3'");
                // $nrowl = mysqli_num_rows($queryAgenda);
                while ($row_agenda = mysqli_fetch_array($queryAgenda)) {
                    $agendado    = $row_agenda['agendado'];
                }
        ...
        foreach ($ArregloDias as $key => $value) {
            ...
            if ($value == 1 and $DiaSemana == $key and $NoDisponible == "No") {
                if (($HoraV >= $ArregloDias_1[$cont] and  $HoraV < $ArregloDias_2[$cont]) or ($HoraV >= $ArregloDias_3[$cont] and  $HoraV < $ArregloDias_4[$cont])) {
                    ...
                    //$queryList = mysqli_query($conn3, "SELECT count(idCitas) as Cuantoshora from citas where fecha = '$fecha' and hora = '$HoraV' and doctor = '$doctor'");
                    $queryList = mysqli_query($conn3, "SELECT count(idCitas) as Cuantoshora from citas where fecha = '$fecha' and (Hora <= '{$HoraV}' and horaF > '{$HoraV}') and doctor = '$doctor'");
