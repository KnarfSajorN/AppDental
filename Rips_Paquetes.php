...
if (isset($_POST['Guardar_Informacion_Pagina'])) {
    ...
    $ruta = htmlentities("Rips_Tarifas?convenio_id=$convenio_id");
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}&error=Hubo Un Error Al Guardar el Paquete'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}&msg=Se Guardo el Paquete Correctamente'</script>";
    }
}
...
if (isset($_POST['Actualizar_Informacion_Pagina'])) {
    ...
    if ($queryList != true) {
        ...
        echo "<script language='Javascript'> window.location='{$ruta}&msg=Se Actualizo el Paquete Correctamente'</script>";
    }
}
///////////////////////////////////////////////////////////////////////////
if ($_GET['Eliminar'] <> "") {
    ...
    //$ruta = str_replace('.php', '', $ruta);
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='{$ruta}&error=Hubo Un Error Al Eliminar el Paquete'</script>";
    } else {
        echo "<script language='Javascript'> window.location='{$ruta}&msg=Se Elimino el Paquete Correctamente'</script>";
    }
}
///////////////////////////////////////////////////////////////////////////
...
<div class="content-wrapper p-3">
    ...
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href="#"> Registro de paquetes convenios </a></li>
        </ol>
    </section>
    ...
    <section class="content">
        <div class="">
            <div class="content">
                <h4 class="Titulo_Pagina">Registro de paquetes convenios </h4>
                <div class="box">
                    <div class="box-body">
                        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
                            <div class="form-group col-md-12">
                                <label>Nombre del Paquete</label>
                                <input type="text" class="form-control input-lg" name="Arreglo[Nombre]" placeholder="Nombre del Paquete" value="" maxlength="120" oninput="if(this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" required>
                            </div>
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-body">
                                    <div class="col-md-12">
                                        <h2 style="text-align: center;font-weight: bold;"> Paquetes </h2>
                                        <table id="example1" class="table table-bordered table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
                                                    ...
                                                    echo "<td width='20%' align='center'><font color='#04CC05'> <a href='{$ruta}?Editar={$id}&convenio_id={$convenio_id}' class='btn btn-block btn-outline-info btn-lg rounded-pill shadow' style='width: 100%;'><i class='fa fa-pencil' title='Editar'> Editar</i></a></font><br>
                                                    <font> <a href='{$ruta}?Eliminar={$id}&convenio_id={$convenio_id}' class='btn btn-block btn-outline-danger btn-lg rounded-pill shadow' style='width: 100%;margin-top:5px;margin-bottom:5px'> <i class='fa fa-close' title='Eliminar'> Eliminar</i></a></font><br>
                                                    </td></tr>";
                                                }
