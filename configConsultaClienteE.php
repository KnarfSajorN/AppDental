      ...
      <div class="card-body">
        <div class="box box-body">
          ...
          <div align="center">
            ...
            if ($idHistoria == 30) {
            ?>
              <button type="button" class="btn btn-block btn-outline-info btn-lg rounded-pill shadow" data-toggle="modal" data-target="#exampleModalScrollable">
                Nuevo ...
              </button>
                ...
                <div class="tab-content tabs">
                  ...
                  Consultas
                        ...
                        while ($rowConsulta = mysqli_fetch_array($queryConsulta)) {
                          ...
                          // SOLICITUD
                    $REGISTRO_EXTERNA = $rowConsulta['xregistroc607'];
                    if ($REGISTRO_EXTERNA <> '') {
                      $REGISTRO_EXTERNA = 'CONSULTA EXTERNA - ANAMNESIS';
                    } else {
                      $REGISTRO_EXTERNA = '';
                    }

                    $REGISTRO_EPICRISIS = $rowConsulta['xregistroe396'];
                    if ($REGISTRO_EPICRISIS <> '') {
                      $REGISTRO_EPICRISIS = 'EPICRISIS';
                    } else {
                      $REGISTRO_EPICRISIS = '';
                    }

                    $REGISTRO_INTERCONSULTA = $rowConsulta['xregistroi329'];
                    if ($REGISTRO_INTERCONSULTA <> '') {
                      $REGISTRO_INTERCONSULTA = 'INTERCONSULTA';
                    } else {
                      $REGISTRO_INTERCONSULTA = '';
                    }

                    $REGISTRO_EMERGENCIA = $rowConsulta['xregistroe605'];
                    ...
                    $REGISTRO_LABORATORIO = $rowConsulta['xregistrol523'];
                    if ($REGISTRO_LABORATORIO <> '') {
                      $REGISTRO_LABORATORIO = 'LABORATORIO';
                    } else {
                      $REGISTRO_LABORATORIO = '';
                    }

                    $REGISTRO_IMAGENOLOGIA = $rowConsulta['xregistroi376'];
                    if ($REGISTRO_IMAGENOLOGIA <> '') {
                      $REGISTRO_IMAGENOLOGIA = 'IMAGENOLOGIA';
                    } else {
                      $REGISTRO_IMAGENOLOGIA = '';
                    }


                    $REGISTRO_053 = $rowConsulta['xtipo968'];
                    $REGISTRO_0532 = $rowConsulta['xreferenci769'];
                    if ($REGISTRO_053 <> '' or $REGISTRO_0532 <> '') {
                      $REGISTRO_053 = 'referencia, derivación, contrareferencia...';
                    } else {
                      $REGISTRO_053 = '';
                    }


                    if ($REGISTRO_EPICRISIS <> '' or $REGISTRO_INTERCONSULTA <> '' or $REGISTRO_EMERGENCIA <> '' or $REGISTRO_LABORATORIO <> '' or $REGISTRO_IMAGENOLOGIA <> '' or $REGISTRO_EXTERNA <> '' or $REGISTRO_053 <> '') {
                      // code...
                      $TIPO1 = '<br><strong>SOLICITUD</strong> - ' . $REGISTRO_EXTERNA . ' ' . $REGISTRO_EPICRISIS . ' ' . $REGISTRO_INTERCONSULTA . ' ' . $REGISTRO_EMERGENCIA . ' ' . $REGISTRO_LABORATORIO . ' ' . $REGISTRO_IMAGENOLOGIA . ' ' . $REGISTRO_053;
                    } else {
                      $TIPO1 = '';
                    }
                    // INFORME
                    $REGISTRO_IMAGENOLOGIA_INFORME  = $rowConsulta['xregistroi298'];
                    if ($REGISTRO_IMAGENOLOGIA_INFORME <> '') {
                      $REGISTRO_IMAGENOLOGIA_INFORME = 'IMAGENOLOGIA';
                    } else {
                      $REGISTRO_IMAGENOLOGIA_INFORME = '';
                    }
                    $REGISTRO_INTERCONSULTA_INFORME = $rowConsulta['xregistroi168'];
                    if ($REGISTRO_INTERCONSULTA_INFORME <> '') {
                      $REGISTRO_INTERCONSULTA_INFORME = 'INTERCONSULTA';
                    } else {
                      $REGISTRO_INTERCONSULTA_INFORME = '';
                    }
                    $REGISTRO_LABORATORIO_INFORME = $rowConsulta['xregistrol692'];
                    if ($REGISTRO_LABORATORIO_INFORME <> '') {
                      $REGISTRO_LABORATORIO_INFORME = 'LABORATORIO';
                    } else {
                      $REGISTRO_LABORATORIO_INFORME = '';
                    }
                    if ($REGISTRO_IMAGENOLOGIA_INFORME <> '' or $REGISTRO_INTERCONSULTA_INFORME <> '' or $REGISTRO_LABORATORIO_INFORME <> '') {
                      $TIPO2 = '<br><strong>INFORME</strong> - ' . $REGISTRO_IMAGENOLOGIA_INFORME . ' | ' . $REGISTRO_INTERCONSULTA_INFORME . ' | ' . $REGISTRO_LABORATORIO_INFORME;
                    } else {
                      $TIPO2 = '';
                    }
                            ...
                            <div class="panel-heading" role="tab" id="heading" style="background-color: #bad8e687;">
                              <h4 class="panel-title">
                                <a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#Historia<?php echo $ID ?>" aria-expanded="false" aria-controls="Historia<?php echo $ID ?>">
                                  ...
                                  <?php if ($Tabla == 'historiaclinicae') : ?>
                              <button onclick="window.location.href='cFinalizadoE?iC=<?php echo  encrypt($ID) ?>&iCr=<?php echo  encrypt($idHistoria) ?>'" title="Imprimir Consulta" target="_blank"><i class="fa fa-print"></i> </button>
                              ||
                              <?php if ($REGISTRO_EXTERNA <> '') : ?>

                                <button title="Agregar Evolucion" onclick="window.location.href='Evolucion_HistoriaE.php?iC=<?php echo encrypt($ID) ?>&iCr=<?php echo encrypt($idHistoria) ?>&cI=<?php echo encrypt($clienteId) ?>'" title="Evolución" target="_blank"><i class="fa fa-file"></i> Agregar Evolución</button>
                                <button title="Imprimir Evolucion" onclick="window.location.href='imprimir002E.php?iC=<?php echo encrypt($ID) ?>&iCr=<?php echo encrypt($idHistoria) ?>&cI=<?php echo encrypt($clienteId) ?>'" title="Evolución" target="_blank"><i class="fa fa-file-o"></i> Ver Evolución</button>
                              <?php endif ?>


                              <?php
                              // if ($TIPO1=='') {
                              //   echo '| <a onclick="window.location.href='configHistoriaClinica.php?clienteId='.$clienteId.'&idHistoria='.$idHistoria.'" title="Agregar Consulta para '.$nombreH.'" role="button"> <i class="fa fa-heartbeat"></i>  Nueva - Solicitud </a>';  
                              // }
                              // if ($TIPO2=='') {
                              //   echo '| <a onclick="window.location.href='configHistoriaClinica.php?clienteId='.$clienteId.'&idHistoria='.$idHistoria.'&solicitud=1" title="Agregar Consulta para '.$nombreH.'" role="button"> <i class="fa fa-heartbeat"></i>  Nuevo - Informe </a>';  
                              // }
                          ...
                          -->
                            ...
                            <div id="Historia<?php echo $ID ?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading" style="padding-top: 20px;background-color: rgb(238, 238, 238);">
                              <div class="panel-body">
                                ...
                                $queryDetalle = mysqli_query($conn3, "SELECT * FROM  configTablaDetalleE where idTabla = $idHistoria order by convert(orden, signed) asc");
