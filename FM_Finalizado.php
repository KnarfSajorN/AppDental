...
$queryList = mysqli_query($conn3, "SELECT * FROM  Historia_Familiograma where id = $Historia_id");
if($queryList){
...
if (isset($_POST['Enviar_Firma'])) {
  ...
  $whatsapp = funcionMaster($cliente_id, 'cliente_id', 'whatsapp', 'cliente');
  $nombre_cliente = funcionMaster($cliente_id, 'cliente_id', 'nombre_cliente', 'cliente');
  $mensajeW = 'Sr(a) *' . $nombre_cliente . '* Se le ha sugerido firmar este familiograma, para proceder abrir el siguiente link ' . $Base . 'firma/firmardocumento/' . $Historia_id . '/Familiograma/' . $cliente_id;
  $accion = 0;
...
{
//$tipo = $_GET['tipo'];
$mensajeW = 'Sr(a) *'.$nombre_cliente.'* Se le envia el siguiente familiograma registrado en el sistema '.$Base.'FM_ImprimirFamiliograma?id='.$Historia_id;
$accion = 0;
Whatsapp_sent_cliente($linkkey,$whatsapp, $mensajeW, $cliente_id, $usuario_id,$whatsapp, $accion);
  ...
  <div class="box">
    ...
    <div align="center">
      ...
      <a class="btn btn-outline-info btn-lg rounded-pill shadow" target="_blank"
        href="<?php echo $Base; ?>FM_ImprimirFamiliograma?id=<?php echo ($Historia_id); ?>">
        <i class="fa fa-print"></i> Imprimir Familiograma
      </a>
    ...
    <a class="btn btn-outline-info btn-lg rounded-pill shadow" href="<?php echo $Base;?>FM_Finalizado?id=<?php echo $Historia_id;?>&tipo=1"> 
      <i class="fa fa-paper-plane"></i> Enviar Familiograma
    </a>
    ...
    <div align="center">
      ...
      $queryList = mysqli_query($conn3, "SELECT firma FROM  firmas where historia_id = $Historia_id and historia_nombre = 'Familiograma'");
      $nrowl = mysqli_num_rows($queryList);
      while ($rowMotorizado = mysqli_fetch_array($queryList)) {
...
<!-- <a class="btn btn-block btn-outline-info btn-lg rounded-pill shadow" target="_blank"  href="<?php echo $Base; ?>firma/firmardocumento/<?php encrypt($Historia_id); ?>/Familiograma/<?php echo $cliente_id; ?>/<?php echo $_SESSION['ID'] ?>" > 
    <i class="fa fa-pencil-square-o" ></i> Solicitar Firma
  </a> -->

        <form action="FM_Finalizado?id=<?php echo ($Historia_id) ?>" method="POST"
