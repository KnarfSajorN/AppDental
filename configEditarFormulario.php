...
$id_historia_registro  = $_POST['id_historia_registro'];
...
$QueryAddColumnHistoria = " ALTER  TABLE {$nombreTabla} 
                            ADD COLUMN IF NOT EXISTS historia_reemplazo_id INT NULL DEFAULT 0 
                            COMMENT 'Referencia a la tabla {$nombreTabla} - Se crea una nueva historia clinica y se oculta la anterior'";

mysqli_query($conn3, $QueryAddColumnHistoria) or die("Error al crear columnas inamicas => " . ( mysqli_error($conn3) ) );
...
while ($rowDetalle = mysqli_fetch_array($queryDetalle)) {
  ...
  if ($tipoCampo == 'selectmultiple') {
    ...
    if ($update != '') {
      ...
      //VALIDAMOS QUE EL INPUT NO LLEGUE VACÍO
      if ($selectmultiple <> "") {
        $query .= ", '";
        for ($i = 0; $i < count($selectmultiple); $i++) {
...
$valores .= " , historia_reemplazo_id";
$query .= ", '$id_historia_registro'";
...
//   mysqli_query($conn3, "UPDATE $nombreTabla SET " . trim($valores, ',') . " WHERE id='" . $update . "'") or die(mysqli_error($conn3));
// } else {
  ...
  //echo "INSERT INTO $nombreTabla ($valores) VALUES  ($query);";
  //echo "INSERT INTO $nombreTabla ($valores) VALUES  ($query);";
  mysqli_query($conn3, "INSERT INTO $nombreTabla ($valores) VALUES  ($query);") or die(mysqli_error($conn3));
  // echo "Este es el query de insertar";
//   echo "INSERT INTO $nombreTabla ($valores) VALUES  ($query)";
// }
...
// HACER LA HISTORIA COMO NO VISIBLE (ELIMINAR)
mysqli_query($conn3,"UPDATE $nombreTabla SET activo = '0' WHERE id ='$id_historia_registro'") or die(mysqli_error($conn3));
...
// Para los RIPS
$RIP_cliente_id = $cliente_id; // por si esta codificado el cliente_id -- importante poner
$RIP_Nombre_Historia = $nombreTabla;
$RIP_historia_id = $historiaClinica1;
