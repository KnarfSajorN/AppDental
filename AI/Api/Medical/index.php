<?php
// api para registro de clientes - citas y pagos de citas
...
if ($pass == md5($token)) {
    // conexion con base de datos sieven
    $hostApiS = 'localhost';
...
('$tipo', '$token', '$pass', '$usuario', now(), '$texto');");
    ...
    // conexion con medical
    $hostApiM = 'localhost';
    $userdbApiM = 'medicaso_rootBase';
    $pass2ApiM = '5qA?o]t6d-h25qA?o]t6d-h2';

    if ($producto <> '3') {
        $DBApiM = 'medicaso_ms_' . $codigoPrincipal;
    }
    if ($producto == '3') {
        $DBApiM = 'medicaso_ps_' . $codigoPrincipal;
    }
    ...
    if (mysqli_connect_error($conn3ApiM)) {
        // se murio la flor
        echo "Errores de Conexión favor intentar mas tarde";
    } else {
        ...
        if ($activo <> 0) {
            ...
            switch ($tipo) {
                ...
                case 2:
                    ...
                    values 
                    ('$doctor','$fechaCita','$horaCita','$nombreCliente','$telefonoCliente','$correoCliente','$motivoConsulta',now(),'1','$tipoConsulta','0','$duracion');");
                ...
                case 4:
                    ...
                    if ($data['telefono'] <> '' and $data['mensaje'] <> '') {
                        ...
                        $sistemaActual[1] = $codigoPrincipal;
                        // $sistemaActual[1] = 'alte';
                        include '../../whatsappPersonalizadoList.php';
                        // actualizar la variable linkkey por la del cliente o se deja el genérico
                        (!is_null($link[$sistemaActual[1]][0])  ? $linkkey = 'http://' . $link[$sistemaActual[1]][1] . (strpos($link[$sistemaActual[1]][1], 'loclx') !== false ? '' : ':' . $link[$sistemaActual[1]][0]) . '/send-message/' . base64_encode($sistemaActual[1]) : $linkkey = $linkkey);
                        // envio de whatsapp
                        $data = [
                        ...
                        $Objeto->mensaje = 'Mensaje enviado correctamente';
                    } else {
                        $Objeto->mensaje = 'Favor de llenar todos los campos requeridos';
                    };
                    $JSON = json_encode($Objeto);
                ...
                default:
                    $Objeto->mensaje = 'No se reconoce el tipo de servicio';
                    $JSON = json_encode($Objeto);
                    echo $JSON;
                    break;
            }
