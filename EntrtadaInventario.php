...
mysqli_query($conn3, "INSERT INTO opracioninvheader (usuario_id, tipoDoc, fechaReg, cantidadReg, totalCosto, 
totalPrecio, catidadTotal, numero, idTercero, nombre, rut{$llaves}) VALUES 
	        ('$usuario_id', '1', '$date', '0', '0', '0', '0', 
            '$entradaInv', '$tercero', '$nombre', '$rut'{$valores});") or die('Error: ' . mysqli_error($conn3));
...
while ($rowinoper = mysqli_fetch_array($queryinoper)) {
    ...
    mysqli_query($conn3, "update operacioninv set estado = 1, tipoDoc = 1, fechaReg = '$date', numero = $entradaInv,operacioninvheader_id = '$idinvheader' where ususario_id = '$usuario_id' and id = '$id'");
...
mysqli_query($conn3, "INSERT INTO opracioninvheader (usuario_id, tipoDoc, fechaReg, cantidadReg, totalCosto, 
totalPrecio, catidadTotal, numero, idTercero, nombre, rut{$llaves}) VALUES 
	        ('$usuario_id', '1', '$date', '$cantidadReg', '$totalCosto', '$totalPrecio', '$catidadTotal', 
            '$entradaInv', '$tercero', '$nombre', '$rut'{$valores});") or die('Error: ' . mysqli_error($conn3));
*/
