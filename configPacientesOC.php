  ...
  columnastablas = [{
    ...
    },
    {
      "data": function(row, type, set) {
        botones = "";
        botones += "<a href='cHistoriaOC?cI=<?=salt()?>" + btoa(row.cliente_id) + "&iCr=<?= encrypt($idHistoria) ?>&tipo=0' title='Agregar <?php echo $nombreH; ?>'><i class='fas fa-file-medical'></i> </a>";
        botones += "<a href='cVerPacienteOC?cI=<?=salt()?>" + btoa(row.cliente_id) + "&iCr=<?= encrypt($idHistoria) ?>&tipo=0' title='Ver <?php echo $nombreH; ?>'><i class='fas fa-book-medical'></i> </a>";
