...
$queryE = mysqli_query($conn3, "SELECT * FROM FE_Entidades where id='$tipoUsuario'");
...
$queryE = mysqli_query($conn3, "SELECT * FROM Rips_Convenio where id='$convenio'");
...
if($paciente == '1'){
    ...
    $queryU=mysqli_query($conn3,"SELECT * FROM cliente where tipo_cliente in ('CC','RC','TI') and entidad_id= '$tipoUsuario' $where");
    while($rowListaU=mysqli_fetch_array($queryU))
    {
    ...
    $queryU=mysqli_query($conn3,"SELECT * FROM cliente where tipo_cliente not in ('CC','RC','TI') and entidad_id= '$tipoUsuario' $where");
    while($rowListaU=mysqli_fetch_array($queryU))
    {
        $ArregloCliente[] = $rowListaU['cliente_id'];
    }
...
if ($doctor == 0) {
    ...
    $queryListaH = mysqli_query($conn3, "SELECT * FROM Rips_Informacion where cliente_id = '{$value}'  and (Fecha BETWEEN '$desde 00:00:00' and '$hasta 23:59:59') AND usuario_id=$doctor");
