...
if (isset($_POST['GuardarDetalleTransferenciaDepositos'])) {
    ...
    $tipoOperacion = "3"; //Operacion Transferencia entre depositos
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if ($queryList != true) {
        echo "<script language='Javascript'> window.location='operacionInvTransferencia.php'</script>";
    } else {
        echo "<script language='Javascript'> window.location='operacionInvTransferencia.php'</script>";
    }
}
...
if (isset($_POST['GuardarOperacionTransferencia'])) {

    /////////////////////////////////////////////////////////////////////
    $Campo1 = mysqli_query($conn3, "show COLUMNS from config WHERE Field = 'TransferenciaInv';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `config` ADD `TransferenciaInv` int(11) NULL DEFAULT '0' COMMENT ' *Creado desde modulo de totalizar transferencia de inventario*';");
    }
    ////////////////////////////////////////////////////////////////////

    /////////////////////////////////////////////////////////////////////
    $Campo1 = mysqli_query($conn3, "show COLUMNS from operacioninv WHERE Field = 'SinvDep_Creado_id';");
    $nrowCampo1 = mysqli_num_rows($Campo1);
    if ($nrowCampo1 == "0") {
        mysqli_query($conn3, "ALTER TABLE `operacioninv` ADD `SinvDep_Creado_id` int(11) NULL DEFAULT '0' COMMENT ' este campo corresponde que si se transfirió un lote, se creara un nuevo registro y aqui quedara el id de SinvDep*Creado desde modulo de totalizar transferencia de inventario*';");
    }
    ////////////////////////////////////////////////////////////////////
    ...
    while ($rowinconfig = mysqli_fetch_array($queryinconfig)) {
        $TransferenciaInv      = $rowinconfig['TransferenciaInv'];
    }
    $TransferenciaInv++;
    ...
    mysqli_query($conn3, "INSERT INTO opracioninvheader (usuario_id, tipoDoc, fechaRegistro, cantidadRegistros, totalCosto, totalPrecio, cantidadTotal, numero, idTercero, nombre, rut,Motivo) VALUES 
            ('$usuario_id', '3', '$date', '0', '0', '0', '0', '$TransferenciaInv', '$tercero', '$nombre', '$rut','$Motivo');") or die(mysqli_error($conn3));
    $idinvheader = mysqli_insert_id($conn3);


    $deposito_destino = $_POST['datos']['deposito_destino'];

    $queryinoper = mysqli_query($conn3, "SELECT * FROM  operacioninv where (usuario_id = '$usuario_id' or usuario_id = '{$_SESSION['ID_principal']}') and estado = 0  AND tipo = 3 ");
    while ($rowinoper = mysqli_fetch_array($queryinoper)) {
        ...
        while ($RowExistencias = mysqli_fetch_array($QueryExistencias)) {
            ...
            ////////////Datos para el lote si aplica
            $Lote_tipo = $RowExistencias['tipo'];
            $Lote_lote = $RowExistencias['lote'];
            $Lote_fechaExpedicion = $RowExistencias['fechaExpedicion'];
            $Lote_fechaVencimiento = $RowExistencias['fechaVencimiento'];
        }
        ...
        /////////////////////////////////////////////////////////////////
        //Aumentar Existencias en Deposito Destino
        $SinvDep_id = $rowinoper['SinvDep_id'];
        $inventario_id = $rowinoper['codigoProd'];
        if ($TipoInventario == "5") {
            //crear el nuevo lote en el deposito seleccionado
            $datehora = date("Y-m-d H:i:s");
            $QueryExistenciasDestino = mysqli_query($conn3, "SELECT * FROM  SinvDep where idSinvetrios = $inventario_id AND idDep=$deposito_destino AND lote='$Lote_lote' AND fechaExpedicion='$Lote_fechaExpedicion' AND fechaVencimiento='$Lote_fechaVencimiento' ORDER BY id DESC LIMIT 1");

            if (mysqli_num_rows($QueryExistenciasDestino) == 1) {
                $row = mysqli_fetch_assoc($QueryExistenciasDestino);
                $Sinvdep_Lote_id = $row['id'];

                $QueryExistenciasDestino = mysqli_query($conn3, "SELECT * FROM  SinvDep where id = $Sinvdep_Lote_id ");
                while ($RowExistenciasDestino = mysqli_fetch_array($QueryExistenciasDestino)) {
                    $ExistenciaSinvDep_Destino = $RowExistenciasDestino['existencia'];
                }
                $ExistenciasFinales_Destino = $ExistenciaSinvDep_Destino + $cantidad;
                mysqli_query($conn3, "UPDATE SinvDep set existencia = $ExistenciasFinales_Destino where id = $Sinvdep_Lote_id");
            } else {

                mysqli_query($conn3, "INSERT INTO SinvDep (tipo, idSinvetrios, idDep, existencia, primeraCarga, primeraCargaF, lote, fechaExpedicion, fechaVencimiento) VALUES 
            ('$Lote_tipo', '$inventario_id', '$deposito_destino', '$cantidad', '1', '$datehora', '$Lote_lote', '$Lote_fechaExpedicion', '$Lote_fechaVencimiento');");

                $SinvDep_Creado_id = mysqli_insert_id($conn3);
                mysqli_query($conn3, "UPDATE operacioninv set SinvDep_Creado_id = $SinvDep_Creado_id WHERE usuario_id = '$usuario_id' and id = '$id'");
            }
        } else if ($TipoInventario == "1") {
            //Actualizacion de la existencias e los productos 
            $QueryExistenciasDestino = mysqli_query($conn3, "SELECT * FROM  SinvDep where idSinvetrios = $inventario_id AND idDep=$deposito_destino ");
            while ($RowExistenciasDestino = mysqli_fetch_array($QueryExistenciasDestino)) {
                $SinvDep_Destino_id = $RowExistenciasDestino['id'];
                $ExistenciaSinvDep_Destino = $RowExistenciasDestino['existencia'];
            }
            $ExistenciasFinales_Destino = $ExistenciaSinvDep_Destino + $cantidad;
            mysqli_query($conn3, "UPDATE SinvDep set existencia = $ExistenciasFinales_Destino where id = $SinvDep_Destino_id");
        }
        ////////////////////////////////////////////////////////////////



        mysqli_query($conn3, "UPDATE operacioninv set estado = 1, tipoDoc = 3, fechaReg = '$date', numero = $TransferenciaInv, operacioninvheader_id = '$idinvheader', Deposito_id_Destino='$deposito_destino' WHERE usuario_id = '$usuario_id' and id = '$id'");
    }

    mysqli_query($conn3, "UPDATE config set TransferenciaInv = $TransferenciaInv WHERE ID_Usuario = '$usuario_id'");
    ...
    echo "<script language='Javascript'> window.location='Historial_ImprimirTransferenciaInventario.php?id={$idinvheader}';</script>";
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//esto se usara para el apartado de la lista del deposito y en un campo hidden para totalizar la factura
$Deposito_id = "0";
$ID_Usuario  =  $_SESSION['ID'];
$resultado = mysqli_query($conn3, "SELECT * FROM  operacioninv where estado = 0 and  (usuario_id =$ID_Usuario or usuario_id ='{$_SESSION['ID_principal']}') AND tipo = '3' order by id");
        ...
        <div class="box box-info" align="center">
            ...
            <div class="card-body">
                ...
                <div class="card-header-title font-size-lg text-capitalize font-weight-normal row">
                    ...
                    <div class="col-md-6">
                        <h2>Transferencias entre Depósitos</h2>
                    </div>
                    <div class="col-md-3">
                        <ul class="nav nav-justified">
                            <li class="nav-item col-md-12" style="display: contents;"><a data-toggle="tab" style="margin-bottom: 10px;" href="#tab-eg-0" class="btn btn-block btn-outline-info btn-lg rounded-pill shadow active"><?= 'Nuevo' ?></a>
                            </li>
                ...
                <div class="tab-content">
                    <div class="tab-pane show active" id="tab-eg-0" role="tabpanel">
                        ...
                        <form id="FormularioSalidaInventario" class="form-group row" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
                            ...
                            <div class="form-group col-md-12">
                                <br>
                                <button type="submit" class="btn btn-block btn-outline-info btn-lg rounded-pill shadow" name="GuardarDetalleTransferenciaDepositos">
                        ...
                        <div class="col-md-12">
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    ...
                                    $queryOpera = "SELECT * from operacioninv where estado = 0 and (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') and tipo = '3' order by id desc";
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td><?= ($rowOpera['Deposito_id_Origen'] != '' ? funcionMaster($rowOpera['Deposito_id_Origen'], 'id', 'descripcion', 'dep') : '-') ?>
                                            </td>
                                            <td><?= $Descripcion ?></td>
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title">Eliminar <?= $rowOpera['id'] ?> -
                                                                    <?= ($rowOpera['Descripcion']) ?></h5>
                                                            ...
                                                            <form id="modal-form<?= $rowOpera['id'] ?>">
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <button type="submit" class="btn btn-danger" onclick="$('#modal-form<?= $rowOpera['id'] ?>').automaticForm({type: 2,table: 'operacioninv', idUpdate: '<?= $rowOpera['id'] ?>', reload:'', page:'operacionInvTransferencia.php'});">Eliminar</button>
                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
                        ...
                        <form id="totalizar-operacionInv-form" class="form-group row">
                            ...
                            <input type="hidden" name="datos[tipo]" value="3">
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <select class="input-lg form-control" name="datos[deposito_destino]" id="deposito_destino" required>
                                    <?php
                                    $queryListDP = mysqli_query($conn3, "SELECT * from dep WHERE (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') and activo = 1 ORDER BY id ASC");
                                    while ($rowMotorizadoDP = mysqli_fetch_array($queryListDP)) {
                                        $idDP = $rowMotorizadoDP['id'];
                                        ...
                                        if ($Deposito_id != "" and $Deposito_id == $idDP) {
                                            echo '<option value="' . $idDP . '" disabled>' . $descripcionDP . '</option>';
                                        } else {
                                            echo '<option value="' . $idDP . '">' . $descripcionDP . '</option>';
                                        }
                                    }
                            ...
                            <div class="col-md-12">
                                ...
                                <hr>
                                <button class="btn btn-block btn-outline-info btn-lg rounded-pill shadow" name="GuardarOperacionTransferencia" <?= (($Deposito_id != "0" and $Deposito_id != "") ? '' : 'disabled') ?> type="submit" onclick="">Guardar</button>
                                <label style="color:red">*Importante si se transfiere lotes, si no existe el producto
                                    tipo lote que tenga el mismo lote, fecha de expedición y fecha de vencimiento, se
                                    creara un nuevo lote con esas mismas características pero con el depósito de destino
                                    elegido y se descontara del lote elegido en el detalle*</label>
                            </div>
                        </form>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                        $queryOpera = "SELECT * from opracioninvheader where tipoDoc = '3' and (usuario_id = '{$_SESSION['ID']}' or usuario_id = '{$_SESSION['ID_principal']}') order by id desc";
                                        $resultOpera = mysqli_query($conn3, $queryOpera);
                                        while ($rowOpera = mysqli_fetch_array($resultOpera)) {
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href='Historial_ImprimirTransferenciaInventario.php?id=<?= $rowOpera['id']; ?>' class='btn btn-block btn-outline-info btn-lg rounded-pill shadow' style='width: 100%;'><i class='fa fa-print' title='Imprimir'>
                                                            Imprimir</i></a>
                                                    ...
                                                    <div class="modal fade" id="modelId_<?= $rowOpera['id']; ?>" tabindex="99999" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
                                                        <div class="modal-dialog modal-lg" role="document" style="z-index: 99999999 !important">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title">Transferencia de Inventario
                                                                        Detallada</h5>
                                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                                        <span aria-hidden="true">&times;</span>
                                                                    </button>
                                                                ...
                                                                <div class="modal-body">
                                                                    <table class="table table-striped table-bordered display responsive">
                                                                        ...
                                                                        <tbody>
                                                                            <?php
                                                                            $queryOpera2 = "SELECT * from operacioninv where estado = 1 and tipo = '3' and operacioninvheader_id = '{$rowOpera['id']}' order by id desc";
                                                                            $resultOpera2 = mysqli_query($conn3, $queryOpera2);
                                                                            while ($rowOpera2 = mysqli_fetch_array($resultOpera2)) {
                                                                            ?>
                                                                                <tr>
                                                                                    <td><?= $rowOpera2['fechaReg'] ?></td>
                                                                                    <td><?= ($rowOpera2['Deposito_id_Origen'] != '' ? funcionMaster($rowOpera2['Deposito_id_Origen'], 'id', 'descripcion', 'dep') : '-') ?>
                                                                                    </td>
                                                                                    <td><?= $rowOpera2['Descripcion'] ?></td>
                                                                                    <td><?= $rowOpera2['cantidad'] ?></td>
                                                                                    <td><?= ($rowOpera2['Deposito_id_Destino'] != '' ? funcionMaster($rowOpera2['Deposito_id_Destino'], 'id', 'descripcion', 'dep') : '-') ?>
                                                                                    </td>
                                                                                </tr>
                                                                            <?php
                                                                            }
...
<script>
                ...
                success: function(response) {
                    ...
                    if (Respuesta.Lista == true && Respuesta.Tipo == "Lotes") {
                        ...
                        Object.keys(Options).forEach(function(key) {
                            ...
                            optionElement.textContent = item.Nombre +
                                ` [${item.Existencia}] [${item.Vencimiento}] `;
                    ...
                    //codigo para el apartado de producto simple
                        ...
                        Object.keys(Options).forEach(function(key) {
                            ...
                            ////////////////////
                            var Mensaje = "Existencia Disponible: <u><b>" + item.Existencia +
                                "</b></u>";
                            document.getElementById('informacion_existencia').innerHTML = Mensaje;
                            //////////////////////
    ...
    document.getElementById('totalizar-operacionInv-form').addEventListener('submit', function(event) {
        ...
        var tipoInventario = "3"; //3-> transferencia
        $.ajax({
            type: "POST",
                ...
                if (Respuesta.Enviar == false) {
                    ...
                    Object.keys(Detalles).forEach(function(key) {
                        ...
                        if (item.Estado == false) {
                            ...
                            Mensaje += "Existencias a descontar en la operacion actual: " + item
                                .Descontar + " . \r\n   ";
                    ...
                    $('#totalizar-operacionInv-form').automaticForm({
                        ...
                        reload: '',
                        page: 'operacionInvTransferencia.php',
                        post: true
                    });
