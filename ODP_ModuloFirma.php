...
if($_POST["Tipo_Consulta"] == "Agregar Firma"){
  ...
  $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE id = '$Tabla_id'");
  while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
    $cliente_id = $RowOdontogramaMaster["cliente_id"];
  }

   
  $QueryRealizado = mysqli_query($conn3,"INSERT INTO ODP_FirmaDetalle (Fecha,Hora,usuario_id,cliente_id,Firma,Firma_Nombre,Firma_Documento,detalle_id)
...
$QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE id = '$detalle_id'");
while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
...
$queryList=mysqli_query($conn3,"SELECT Firma FROM ODP_FirmaDetalle where Firma != '' AND detalle_id = '$detalle_id'");
                ...
                <div class="center-wrap">
                  <div class="section text-center" style="color: white;">
                    ...
                    <div class="form-group">
                      <?php
                        echo "<p class='mb-0 mt-4 text-center'>Diente <br>#{$Numero_Diente}</p>";
                        echo "<p class='mb-0 mt-4 text-center'>Cara: <br>{$NombreCara}</p>";
...
?>
  $("#reg-log").addClass("active");
  $("#BotonGuardar").css("display","none");
  $("#MensajeFinal").html("<u>Ya se encuentra registrada la firma, sobre el estado/procedimiento de la pieza</u>");
<?php
}
?>
</script>
