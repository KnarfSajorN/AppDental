...
$queryList = mysqli_query($conn3, "SELECT * FROM  planesTratamiento where id = $historiaClinica1");
...
if (isset($_POST['Enviar_Firma'])) {
    ...
    $mensajeW = 'Sr(a) *' . $nombre_cliente . '* Se le ha sugerido firmar esta historia clinica, para proceder abrir el siguiente link ' . $Base . 'firma/firmardocumento/' . $historiaClinica1 . '/planesTratamiento/' . $cliente_id;
    $accion = 0;
...
<div class="content-wrapper p-3">
    ...
    <section class="content box p-4">
        ...
        <?php
                                ...
                                $botonesImprimir = [
                                    ['Plan de Tratamiento', base64_encode($Base . 'PT_impresionPlanTratamiento?idCliente=' . $cliente_id . '&FAID=' . base64_encode($historiaClinica1).'&idP='.$ID_principal)],
                                ];
        ...
        <div align="center">
            ...
            $queryList = mysqli_query($conn3, "SELECT firma FROM  firmas WHERE historia_nombre='planesTratamiento' AND historia_id = $historiaClinica1");
            // $nrowl = mysqli_num_rows($queryList);
            if ($queryList) {
                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
