...
$queryList = mysqli_query($conn3, "SELECT * FROM  planesProcedimiento where id = $historiaClinica1");
...
if (isset($_POST['Enviar_Firma'])) {
    ...
    $mensajeW = 'Sr(a) *' . $nombre_cliente . '* Se le ha sugerido firmar esta historia clinica, para proceder abrir el siguiente link ' . $Base . 'firma/firmardocumento/' . $historiaClinica1 . '/planesProcedimiento/' . $cliente_id;
    $accion = 0;
...
<div class="content-wrapper p-3">
    ...
    <section class="content box p-4">
        ...
        <?php
                                ...
                                $botonesImprimir = [
                                    ['Procedimientos', base64_encode($Base . 'PT_imprimirProcedimientos?idCliente=' . $cliente_id . '&FAID=' . base64_encode($historiaClinica1).'&idP='.$ID_principal)],
                                ];
        ...
        <div align="center">
            ...
            $queryList = mysqli_query($conn3, "SELECT firma FROM  firmas WHERE historia_nombre='planesProcedimiento' AND historia_id = $historiaClinica1");
            // $nrowl = mysqli_num_rows($queryList);
            if ($queryList) {
                while ($rowMotorizado = mysqli_fetch_array($queryList)) {
