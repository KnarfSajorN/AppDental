...
$horaF = $hora_actual->modify('+30 minutes');
...
if($fecha_proxima_aplicacion != ''){

    $queryClientInfo = mysqli_query($conn3, "SELECT * FROM cliente WHERE cliente_id = $id_cliente");
    while ($clientInfo = mysqli_fetch_assoc($queryClientInfo)) {
        $nombre = $clientInfo['nombre'];
        $telefono = $clientInfo['telefono'];
        $correo = $clientInfo['correo'];
        
        $idPaticas = $clienteInfo['idPaticas'];
    }
    $ID_H == 0;
    ...
    $estado = 1;
    $P= 1;
    $motivoConsulta = 'Aplicar vacuna';
    
    $agregarProximaAplicacion = mysqli_query($conn3, "INSERT INTO citas (doctor, fecha, Hora, nombre, telefono, correo, motivoConsulta, estado, registrado, usuario_id, tipo, 
    idCliente,duracion,HoraF,evento_googlecalendar_id_anterior,sucursal,ID_H,idPaticas,validoConductor,conductorAsignado,fechaRecogida,horaRecogida,lugarRecogida,lugarDestino) 
    
    VALUES ('$doctor' ,'$fecha_proxima_aplicacion' ,'$Hora' ,'$nombre' ,'$telefono' ,'$correo' ,'$motivoConsulta' ,'$estado' ,'$registrado', '$usuario_id', '$P', 
    '$id_cliente','$duracion','$horaF','$evento_googlecalendar_id_anterior','$sucursal','$ID_H','$idPaticas','$validoConductor','$conductorAsignado','$fechaRecogida','$horaRecogida','$lugarRecogida','$lugarDestino')");
...
echo "<script language='Javascript'> window.location='vacunacionAplicar?cI=".encrypt($id_cliente)."';</script>";
