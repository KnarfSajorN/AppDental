        ...
        <div class="col-xs-12 table-responsive">
            <table class="table table-striped" border="1">
                ...
                <tbody>
                    ...
                    $queryListC = mysqli_query($conn3, "SELECT * FROM cliente WHERE DATE(Fechar) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) GROUP BY cliente_id");
                    // $queryListC = mysqli_query($conn3, "SELECT * FROM  cliente WHERE DATE(Fecharr) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) $andtipo ");
...
$queryList1 = mysqli_query($conn3, "SELECT COUNT(RiesgoId) AS total,RiesgoId AS RiesgoId FROM Reporte_Riesgos  WHERE DATE(Fechar_Registro) BETWEEN CAST('$desde' AS DATE) AND CAST('$hasta' AS DATE) group by RiesgoId");
    ...
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ...
            while ($row1 = mysqli_fetch_array($queryList1)) {
                // $count1 += $row1["total"];
                // echo "['" . $genero . "', " . $row["total"] . "],";
                if ($row1['RiesgoId'] != '' && $row1['RiesgoId'] > 0) {
                    echo "[' " . funcionMaster1($row1["RiesgoId"], 'id', 'Nombre', 'empresas_cargos') . "', " . $row1["total"] . "],";
                }
            }
        ...
        var options = {
            title: "PERSONAL EXPUESTO A FACTORES DE RIESGOS ",
            width: 1000,
            height: 1000,
            bar: {
                groupWidth: "50%"
