...
if($_GET['Tipo']=="1"){
    echo "<tr><td colspan='16' style='font-size: 50px;'> <div style='width: 100%;text-align-last: center;padding-top: 20px;padding-bottom: 20px;'> Odontograma Pediatrico <u>Temporales</u>  </div> </td></tr>";
}else if($_GET['Tipo']=="2"){
    echo "<tr><td colspan='16' style='font-size: 50px;'> <div style='width: 100%;text-align-last: center;padding-top: 20px;padding-bottom: 20px;'> Odontograma Pediatrico <u>Permanentes</u>     </div> </td></tr>";
}
else if($_GET['Tipo']=="3"){
    echo "<tr><td colspan='16' style='font-size: 50px;'> <div style='width: 100%;text-align-last: center;'> Odontograma Pediatrico <u>Temporales/Permanentes</u>  </div> </td></tr>";
}
foreach ($ArregloDientes as $keyDiente => $valueDiente) {
    ...
    echo "<tr style='display: block;height: 280px;width: 100%;{$EstiloTR}' id='Tr_{$Contador}'>";
    ...
    foreach ($valueDiente as $keyPieza => $valuePieza) {
        ...
        if($valuePieza!="Vacio"){
            ...
            $QueryOdontogramaMaster = mysqli_query($conn3, "SELECT * FROM ODP_OdontogramaMasterDetalle WHERE cliente_id = '$cliente_id' AND usuario_id = '$usuario_id' AND Numero_Diente='$valuePieza' ORDER BY id DESC LIMIT 1 ");
            while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
                $Procedimiento = $RowOdontogramaMaster["Procedimiento"];
            }

            $SVG = funcionMasterAntiguo(funcionMasterAntiguo($Procedimiento,'id','Icono','ODP_Procedimiento'),'id','SVG','ODP_Iconos_SVG');
            $Color = funcionMasterAntiguo($Procedimiento,'id','Color','ODP_Procedimiento');
            ...
            while ($RowOdontogramaMaster= mysqli_fetch_array($QueryOdontogramaMaster)) {
                ...
                foreach (json_decode($ArregloD) as $key => $value) {
                    ...
                    $SVG = funcionMasterAntiguo(funcionMasterAntiguo($value,'id','Icono','ODP_Procedimiento'),'id','SVG','ODP_Iconos_SVG');
                    $Color = funcionMasterAntiguo($value,'id','Color','ODP_Procedimiento');
                    $SVG = str_replace('fill="currentColor"', "fill='{$Color}'", $SVG);
