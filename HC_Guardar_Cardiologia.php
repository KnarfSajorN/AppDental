...
// 	$Paraclinicos .= '<b>Laboratorio #' . $key . '</b>:<br>';
// 	foreach ($value as $key1 => $value1) {
...
//exit();
    ...
    mysqli_query($conn3, "INSERT INTO  Historia_Cardiologia (fecha, cliente_id, usuario_id, InformacionAcudiente,    EnfermedadActual, Checks_Antecedentes, AntecentesGinecobstetricos , AntecedentesFamiliares, Checks_Revision, SignosVitales, Paraclinicos, Imagenologia_Examen, Laboratorio_Examenes, ExamenFisico, OrganoSentidos, SintomasGenerales, DiagnosticoAcupuntura, DiagnosticoConsulta, Impresion, PlanManejo, Incapacidades, Insumos, RecetaId,imgEscalaTanner,textEscalaTanner,CIE10_1) VALUES  
    ('$fecha', '$clienteId', '$idusuario', '$InformacionAcudiente',  '$EnfermedadActual', '$Checks_Antecedentes', '$AntecentesGinecobstetricos', '$AntecedentesFamiliares',  '$Checks_Revision', '$SignosVitales', '$Paraclinicos','$Imagenologia_Examen', '$Laboratorio_Examenes','$ExamenFisico','$OrganoSentidos', '$SintomasGenerales', '$DiagnosticoAcupuntura','$DiagnosticoConsulta', '$Impresion', '$PlanManejo', '$Incapacidades','$Insumos','$RecetaId', '$imgEscalaTanner', '$textEscalaTanner','$CIE10');") or die(mysqli_error($conn3));
...
if ($_POST['SignosVitales']["Peso Corporal"] <> "" or $_POST['SignosVitales']["Altura"] <> "" or $_POST['SignosVitales']["IMC"] <> "" or $_POST['SignosVitales']["Perimetro_Cefalico"] <> "") {
    ...
    if ($nrowsignosvitales > 0) {
        mysqli_query($conn3, "
            ...
            Presion_Arterial = '{$_POST['SignosVitales']["Presion Arterial"]}',
			WHERE cliente_id = '$clienteId'
			");
    } else {
        mysqli_query($conn3, "INSERT INTO  SignosVitalesYAntropometria (Peso_Corporal, Altura, IMC, Perimetro_Cefalico, Frecuencia_Respiratoria, Frecuencia_Cardiaca, Presion_Arterial_Diastolica, Presion_Arterial_Sistolica, Temperatura_Corporal, Saturacion_Oxigeno, Porcentaje_Grasa_Corporal, Circunferencia_Cintura, Circunferencia_Abdominal, Tension_Arterial_Media,Presion_Arterial, cliente_id, usuario_id) 
			VALUES ('{$_POST['SignosVitales']["Peso Corporal"]}','{$_POST['SignosVitales']["Altura"]}','{$_POST['SignosVitales']["IMC"]}','{$_POST['SignosVitales']["Perimetro Cefalico"]}','{$_POST['SignosVitales']["Frecuencia Respiratoria"]}','{$_POST['SignosVitales']["Frecuencia Cardiaca"]}','{$_POST['SignosVitales']["Presion Arterial Diastolica"]}','{$_POST['SignosVitales']["Presion Arterial Sistolica"]}','{$_POST['SignosVitales']["Temperatura Corporal"]}','{$_POST['SignosVitales']["Saturacion Oxigeno"]}','{$_POST['SignosVitales']["Porcentaje de Grasa Corporal"]}','{$_POST['SignosVitales']["Circunferencia de Cintura"]}','{$_POST['SignosVitales']["Circunferencia Abdominal"]}','{$_POST['SignosVitales']["Tension Arterial Media"]}','{$_POST['SignosVitales']["Presion Arterial"]}','$clienteId','$idusuario')");
    }
}

$queryList = mysqli_query($conn3, "SELECT * FROM  Laboratorio_Imagen where cliente_id = $clienteId");
if ($nrowlaboratorio > 0) {
    mysqli_query($conn3, "
			UPDATE Laboratorio_Imagen
			SET Imagenologia_P = '{$_POST['Paraclinicos']}',
			
			WHERE cliente_id = '$clienteId'
			");
} else {
    mysqli_query($conn3, "INSERT INTO  Laboratorio_Imagen (Imagenologia_P, cliente_id, usuario_id) 
			VALUES ('{$_POST['Paraclinicos']}','$clienteId','$idusuario')");
}


// Para los RIPS
$RIP_Nombre_Historia = "Historia_Cardiologia";
...
echo "<script language='Javascript'> window.location='HC_Finalizado_Historias?historiaClinica1=$id_historia';</script>";
