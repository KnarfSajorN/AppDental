...
$queryList=mysqli_query($conn3,"SELECT firma FROM  abonoC where firma != '' AND id = '$idAbono'");
// echo "SELECT firma FROM  abonoC where firma != '' AND id = '$idAbono'";
...
$montoAbonado = funcionMaster($idAbono,'id','valor_abonado','abonoC');
$usuario = funcionMaster($idAbono,'id','usuario_id','abonoC');
...
?>
  <div id="signature-pad" class="signature-pad">
    <h1>El monto cancelado es de : <?= number_format($montoAbonado,2).' '.$moneda?></h1>
...
<form action="<?= $Base ?>firma/AbonoFirmadoC" method="POST">
 

<input type="hidden"  name="tarea" id="tarea"    required>
<input type="hidden"  name="idAbono" id="idAbono"  value="<?= $idAbono;?>"  required>
<input type="hidden"  name="idOperacion" id="idOperacion"  value="<?= $idOperacion;?>"  required>
