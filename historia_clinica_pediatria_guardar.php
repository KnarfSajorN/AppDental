...
foreach (array_filter($_POST['Sospecha'], 'removeEmptyElements') as $key => $value) {
	$Sospecha .= $key . " : " . $value . '<br>';
}
//echo $PlanManejo.'<br>';
...
$idusuario = $_POST['ID'];
$Peso_edad = $_POST['Peso_edad'];
$peso_talla = $_POST['peso_talla'];
$talla_edad = $_POST['talla_edad'];
$estado_nutricional = $_POST['estado_nutricional'];

$Observaciones_Imagen = $_POST['Observaciones_Imagen'];

$Observaciones_Labora = $_POST['Observaciones_Labora'];
...
//validacion para no enviar la Historia Vacia
if(trim($InformacionAcudiente) || trim($EnfermedadActual) || trim($Checks_Antecedentes) || trim($AntecentesGinecobstetricos)
|| trim($AntecedentesFamiliares) || trim($Checks_Revision) || trim($SignosVitales) || trim($Paraclinicos) || trim($Imagenologia_Examen)
|| trim($Laboratorio_Examenes) || trim($ExamenFisico) || trim($OrganoSentidos) || trim($SintomasGenerales) || trim($DiagnosticoAcupuntura)
|| trim($DiagnosticoConsulta) || trim($Impresion) || trim($PlanManejo) || trim($Incapacidades) || trim($Insumos) || trim($receta_id)
|| trim($imgEscalaTanner) || trim($textEscalaTanner)){


	$query = "Historia_Clinica_Pediatria (fecha, cliente_id, usuario_id, InformacionAcudiente,    EnfermedadActual, Checks_Antecedentes, AntecentesGinecobstetricos , AntecedentesFamiliares, Checks_Revision, SignosVitales, Paraclinicos, Imagenologia_Examen, Laboratorio_Examenes, ExamenFisico, OrganoSentidos, SintomasGenerales, DiagnosticoAcupuntura, DiagnosticoConsulta, Impresion, PlanManejo, Incapacidades, Insumos, RecetaId,imgEscalaTanner,textEscalaTanner, Peso_edad, peso_talla, talla_edad, estado_nutricional, Observaciones_Imagen, Observaciones_Labora, Sospecha) VALUES  
($fecha, $clienteId, $idusuario, $InformacionAcudiente,  $EnfermedadActual, $Checks_Antecedentes, $AntecentesGinecobstetricos, $AntecedentesFamiliares,  $Checks_Revision, $SignosVitales, $Paraclinicos,$Imagenologia_Examen, $Laboratorio_Examenes,$ExamenFisico,$OrganoSentidos, $SintomasGenerales, $DiagnosticoAcupuntura,$DiagnosticoConsulta, $Impresion, $PlanManejo, $Incapacidades,$Insumos,$RecetaId, $imgEscalaTanner, $textEscalaTanner, $Peso_edad, $peso_talla, $talla_edad, $estado_nutricional, $Observaciones_Imagen, $Observaciones_Labora, $Sospecha);";
...
mysqli_query($conn3, "INSERT INTO  Historia_Clinica_Pediatria (fecha, cliente_id, usuario_id, InformacionAcudiente,    EnfermedadActual, Checks_Antecedentes, AntecentesGinecobstetricos , AntecedentesFamiliares, Checks_Revision, SignosVitales, Paraclinicos, Imagenologia_Examen, Laboratorio_Examenes, ExamenFisico, OrganoSentidos, SintomasGenerales, DiagnosticoAcupuntura, DiagnosticoConsulta, Impresion, PlanManejo, Incapacidades, Insumos, RecetaId,imgEscalaTanner,textEscalaTanner, Peso_edad, peso_talla, talla_edad, estado_nutricional, Observaciones_Imagen, Observaciones_Labora, Sospecha) VALUES  
('$fecha', '$clienteId', '$idusuario', '$InformacionAcudiente',  '$EnfermedadActual', '$Checks_Antecedentes', '$AntecentesGinecobstetricos', '$AntecedentesFamiliares',  '$Checks_Revision', '$SignosVitales', '$Paraclinicos','$Imagenologia_Examen', '$Laboratorio_Examenes','$ExamenFisico','$OrganoSentidos', '$SintomasGenerales', '$DiagnosticoAcupuntura','$DiagnosticoConsulta', '$Impresion', '$PlanManejo', '$Incapacidades','$Insumos','$RecetaId', '$imgEscalaTanner', '$textEscalaTanner', '$Peso_edad', '$peso_talla', '$talla_edad', '$estado_nutricional', '$Observaciones_Imagen', '$Observaciones_Labora', '$Sospecha');");
$id_historia = mysqli_insert_id($conn3);

}else{
	echo "<script language='Javascript'> alert('No puedes enviar una Historia vacía')</script>";
	echo "<script language='Javascript'> window.location='hcp?cI=".encrypt($clienteId)."';</script>";
}
...
// Para los RIPS
$RIP_Nombre_Historia = "Historia_Clinica_Pediatria";
$RIP_historia_id = $id_historia;
include 'IR_GuardarRips.php';

// Para el Recetario
$Recetario_Nombre_Historia = "Historia_Clinica_Pediatria";
...
mysqli_query($conn3, "INSERT INTO examenesaRealizar (usuario_id,cliente_id,historia_id,laboratorio,ecografia,otros,fechaHora, Observaciones_Labora, Observaciones_Imagen) 
    VALUES 
    ('$idusuario','$clienteId','$id_historia', '$Laboratorio_Examenes', '$Imagenologia_Examen', '$otros', '$fecha', '$Observaciones_Labora', '$Observaciones_Imagen');");


echo "<script language='Javascript'> window.location='hcpFinalizado?hC=".encrypt($id_historia)."';</script>";
