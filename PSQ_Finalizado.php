...
if (isset($_GET['tipo'])) {
  ...
  $mensajeW = 'Sr(a) *' . $nombre_cliente . '* Se le envia el siguiente documento registrado en la cita ' . $Base . 'PSQ_Imprimir?historiaClinica1=' . $historiaClinica1 . '';
  $accion = 0;
  Whatsapp_sent_cliente($linkkey, $whatsapp, $mensajeW, $cliente_id, $usuario_id, $whatsapp, $accion);

  echo "<script language='Javascript'> window.location='PSQ_Finalizado?historiaClinica1=" . $historiaClinica1 . "';</script>";
...
<a class="btn btn-outline-info btn-lg rounded-pill shadow" href="<?php echo $Base;?>PSQ_Pacientes"> 
...
<a class="btn btn-outline-info btn-lg rounded-pill shadow" target="_blank" href="<?php echo $Base;?>PSQ_Imprimir?historiaClinica1=<?php echo $historiaClinica1;?>"> 
  <i class="fa fa-print"></i> Imprimir Consulta Psiquiátrica
</a>

<?php 
        $botonesImprimir = [
          ['Consulta Psiquiátrica', base64_encode($Base.'PSQ_Imprimir_plantilla.php?historiaClinica1='.encrypt($historiaClinica1).'')],       
        ];
...
<a class="btn btn-outline-info btn-lg rounded-pill shadow" href="<?php echo $Base; ?>PSQ_Finalizado?historiaClinica1=<?php echo $historiaClinica1; ?>&tipo=consulta">
