<?php
 ...
 if ($procedimiento <> '') {$procedimiento1 = '<b> Padecimiento Actual:</b> '.$procedimiento.'';}

 $interrogatorios                = reem($_POST['interrogatorios']); 
 if ($interrogatorios  <> '') {$interrogatorios1 = '<b>Interrogatorios por Aparatos y Sistemas:</b> '.$interrogatorios.'';}

 $estado               = reem($_POST['estado']); 
 if ($estado <> '') {$estado1 = '<b>Estado de Salud:</b> '.$estado.'';}

 $pronostico               = reem($_POST['pronostico']); 
 if ($pronostico <> '') {$pronostico1 = '<b>Pronóstico:</b> '.$pronostico.'';}

 $diagnostico1               = reem($_POST['diagnostico']);
 if ($diagnostico1 <> '') {$diagnostico11 = '<b>Pronóstico:</b> '.$diagnostico1.'';}
  ...
  // *********************************************************    TABLA  antecedentes Herofamiliares *********************************************************
        // *********************************************************     *********************************************************


 $padre               = reem($_POST['padre']);
 if ($padre <> '') {$padre1 = ' Papa: '.$padre.'<trong> | </trong>';}

 $madre        = reem($_POST['madre']);
 if ($madre <> '') {$madre1 = ' Madre: '.$madre.'<trong> | </trong>';}

        $abuelos1        = reem($_POST['abuelos1']);
 if ($abuelos1<> '') {$abuelos11 = ' Abuelos Paternos: '.$abuelos1.'<trong> | </trong>';}


        $abuelos2        = $_POST['abuelos2'];
 if ($abuelos2<> '') {$abuelos21 = ' Abuelos Maternos: '.$abuelos1.'<trong> | </trong>';}


        $hermanos          = reem($_POST['hermanos']);
 if ($$hermanos<> '') {$hermanos1 = ' Hermanos: '.$hermanos.'<trong> | </trong>';}


        $otros          = reem($_POST['otros']);
 if ($otros <> '') {$otros1 = ' Otros ANTECEDENTES: '.$otros.'<trong> | </trong>';}


        $ant_nopatologico        = reem($_POST['ant_nopatologicos']);

        $ant_patologicos         = reem($_POST['ant_paatologicos']);

 $antecedentesFami=$padre1.$madre1.$abuelos11.$abuelos21.$hermanos1.$otros1; 
...
$procedimiento               = reem($_POST['procedimiento']); 
 ...
 if ($diagnostico1 <> '') {$diagnostico11 = '<b>Pronóstico:</b> '.$diagnostico1.'';}
        ...
        mysqli_query($conn3,"INSERT INTO historiaClinica14_cardiologia (cliente_id, usuario_id, Fecha, Hora, antecedentes_cardio,   patologicos_cardio,nopatologicos_cardio,procedimiento,interroga,estado,pronostico,diagnostico) VALUES  ('$clienteId', '$ID', '$fechar', '$hora','$antecedentesFami','$ant_patologicos','$ant_nopatologico','$procedimiento1','$interrogatorios1','$estado1','$pronostico1','$diagnostico11');");
        ...
        // *********************************************************    TABLA  historiaClinica1 *********************************************************
              ...
              $queryListhc=mysqli_query($conn3,"SELECT MAX(ID) as historiaClinica1 from historiaClinica14_cardiologia where cliente_id= $clienteId");
            ...
            // *********************************************************    TABLA  examenFisico *********************************************************
            // *********************************************************    TABLA  examenFisico *********************************************************
            mysqli_query($conn3,"INSERT INTO examenFisicoCardiologia (usuario_id, cliente_id, historiaClinicacardiologia_id, peso, altura, imc, ComposicionCorporal, estadoGeneral, estadoConciencia, ojos, otoscopia, cavidadOral, cuello, torax, corazon, abdomen, genitoUrinario, extremidades, vacularPeriferico, sistemaNervioso, pielAnexos, examenPartesdCuerpo, tart, temperatura, fcard, sat, fechaHora, procedimiento_cardio, interrogatorios_cardio, estado_salud_cardio, pronostico_cardio, diagnostico_cardio) 

                VALUES ('$ID', '$clienteId','$historiaClinica1', '$peso', '$altura', '$imc', '$ComposicionCorporal', '$estadoGeneral', '$estadoConciencia', '$ojos', '$otoscopia', '$cavidadOral', '$cuello', '$torax', '$corazon  ', '$abdomen', '$genitoUrinario', '$extremidades', '$vacularPeriferico', '$sistemaNervioso', '$pielAnexos', '$examenPartesdCuerpo', '$tart', '$temp', '$fcard', '$sat', '$Afechar','$procedimiento', '$interrogatorios','$estado','$pronostico','$diagnostico1');");
            // *********************************************************    TABLA  examenFisico *********************************************************
            // *********************************************************    TABLA  examenFisico *********************************************************
...
'.$NOMBRE_USUARIO.'</p> 
   ...
   echo "<script language='Javascript'> window.location='finalizar_cardiologia.php?historiaClinica1=$historiaClinica1';</script>"; 
