...
if (isset($_POST['keyUnlock'])) {
    ...
    $selectFrom = Desencriptar($option->selectFrom);
    $option->order = json_decode(Desencriptar($option->order));
    foreach ($option->order as $key => $value) {
        $order .= $key . " " . $value . " ";
    }
    $option->clausula->data = Desencriptar($option->clausula->data);
    ...
    if (isset($_POST["Nueva_Opcion_Select_Global"]) && $_POST['Tipo'] != "cargar") {
        ...
        $nombreCreador = Desencriptar($campoCreador->nombreCreador);
        $conditionInsert = Desencriptar($campoCreador->conditionInsert);
        $conditionSelect = Desencriptar($campoCreador->conditionSelect);
        $TipoSelect = Desencriptar($option->Tipo_Select_Global);
        ...
        $likeWhere = explode(" || ", Desencriptar($option->likeWhere));
        ...
        $regAdicional['data'] = ((gettype($regAdicional['data']) === "string") ? json_decode(Desencriptar($regAdicional['data'])) : $regAdicional['data']);
...
<script type="text/javascript">
    function Select2Dinamico(Nombre, TipoSelect, creador, selected = false) {
        $(Nombre).select2({
            // placeholder: {
